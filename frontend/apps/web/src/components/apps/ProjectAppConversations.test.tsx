...
import { OmnaraClientProvider, useCreateAppSubscription } from '@omnara/react'
import { type AppSubscription, createOmnaraClient, type ProjectApp } from '@omnara/sdk'
...
import { ProjectAppConversations } from './ProjectAppConversations'

const orgId = fakeId('org'),
  projectId = fakeId('proj')
const app = projectApp({ state: 'active', provider_tenant_id: 'T123' })
const path = `/api/v1/orgs/${orgId}/projects/${projectId}/apps/${app.id}/subscriptions`
...
const second: AppSubscription = {
  ...subscription,
  id: `asub_${'b'.repeat(26)}`,
  agent_name: '',
...
function conversations(currentApp: ProjectApp = app, canManage = true) {
  return (
...
it('allows stopping while disconnected, keeps failed deletions visible, and respects canceled confirmation and lost manage permission', async () => {
  ...
  const offline = { ...app, state: 'disconnected' } satisfies ProjectApp
...
function Attach() {
  const create = useCreateAppSubscription(orgId, projectId, app.id)
  return (
    <button
      onClick={() => {
        create.mutate({
...
it('refreshes the app list after an explicit API attachment without changing config', async () => {
  let attached = false
  const api = fakeApi([
    {
      method: 'GET',
