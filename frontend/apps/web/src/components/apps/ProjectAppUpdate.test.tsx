...
it.each(['same setup', 'new revision', 'disconnected'] as const)(
  ...
  async (scenario) => {
    ...
    const detail = { ...app, runtime_failure: failure }
    const updated = projectApp({
      ...app,
      settings: {},
      updated_at: '2026-09-23T11:00:00Z',
      setup_revision: scenario === 'new revision' ? app.setup_revision + 1 : app.setup_revision,
    ...
    const api = fakeApi([
      ...
      { method: 'PUT', path: appPath, respond: () => Response.json(updated) },
      {
        method: 'GET',
      ...
      ...['cron-triggers', `apps/${app.id}/subscriptions`].map((resource) => ({
        method: 'GET',
        path: `${projectPath}/${resource}`,
        respond: () => Response.json({ data: [], next_cursor: null }),
      })),
    ])
    const { cache, client } = renderProjectApp(
      root,
      api,
    ...
    expect(api.requestsTo('PUT', appPath)[0]?.body).toEqual({
      name: app.name,
    ...
    expect(api.requestsTo('GET', appPath)).toHaveLength(3)
