...
import { type AppCapabilityDefinition, schemas } from '@omnara/sdk'
...
import { projectApp } from '@/test/fixtures'
...
it('uses the declared schedule capability, defaults, order and constraints without thread fields', async () => {
  ...
  expect(api.requestsTo('POST', cronPath)[0]?.body).toEqual({
    ...
    target: { type: 'app', app_id: app.id, settings: { queue: 'job-2', note: '🚀🚀' } },
  })
...
it.each(['nested schema', 'additional saved settings'])(
  ...
  async (scenario) => {
    ...
    const properties: AppCapabilityDefinition['input_schema'] = { queue: { type: 'string' } }
    ...
    const saved = trigger({ target: { type: 'app', app_id: app.id, settings } })
    let fail = true
    ...
    expect(api.requestsTo('PATCH', cronPath + '/' + saved.id)[1]?.body).toEqual({
      ...
      target: { type: 'app', app_id: app.id, settings: changed },
    })
  },
)
