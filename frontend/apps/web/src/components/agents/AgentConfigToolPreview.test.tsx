...
it.each([
  ...
  await vi.waitFor(() => {
    expect(otherToolsTrigger()).not.toBeNull()
  })
  clickLabel('Other tools')
  const trigger = otherToolsTrigger()
  ...
  expect(otherToolsTrigger()).toBe(trigger)
...
it('clears the retained tool preview when switching projects', async () => {
  ...
  expect(otherToolsTrigger()).toBeNull()
...
const otherToolsTrigger = () =>
  [...container.querySelectorAll('button')].find((el) => el.textContent === 'Other tools') ?? null
