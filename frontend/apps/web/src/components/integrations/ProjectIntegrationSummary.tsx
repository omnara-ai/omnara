...
import type { IntegrationLauncher, ProjectIntegration } from '@omnara/sdk'
...
export function ProjectIntegrationSummary({
  ...
  return (
    <>
      <p className="text-muted-foreground">
        {launchMoment(launcher, integration.integration_type)},{' '}
...
function launchMoment(
  ...
  integrationType: ProjectIntegration['integration_type'],
) {
  if (integrationType === 'discord_thread')
    return 'When someone mentions the bot in any server where it has access'
  const where =
    integrationType === 'github_pr' && launcher.scope_kind === 'installation'
  ...
  return integrationType === 'github_pr'
