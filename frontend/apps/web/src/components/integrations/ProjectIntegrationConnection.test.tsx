...
it.each(['discord_thread', 'github_pr'] as const)(
  ...
  async (integrationType) => {
    ...
    expect(
      button(integrationType === 'github_pr' ? 'Choose a profile' : 'Choose profiles'),
    ).toBeDefined()
  },
)
...
async function beginSlackAuthorization({ setupRevision = 1, callbackError = false } = {}) {
  const draft = projectIntegration()
  ...
  const connected: ProjectIntegration = {
    ...draft,
    state: 'active',
    provider_tenant_id: 'T123',
    provider_account_ref: 'A123',
  ...
  return {
    ...
    draft,
    connected,
    refreshIntegration: async (integration: ProjectIntegration) => {
      current = integration
      await act(async () => {
...
it('invalidates only this project’s cached integration list before finishing Slack authorization', async () => {
  ...
  const listKey = (projectID: string) =>
    listProjectIntegrationsInfiniteOptions({ path: { orgID: orgId, projectID }, client }).queryKey
