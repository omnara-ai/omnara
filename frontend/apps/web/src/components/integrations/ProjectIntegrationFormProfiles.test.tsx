...
const mixedIntegration: ProjectIntegration = {
  ...projectIntegration({ name: 'shared-support', state: 'active', provider_tenant_id: 'T123' }),
  settings: {
    launcher: {
      trigger: 'mention',
      scope_kind: 'channel',
...
it('keeps off-page and existing-agent slots and retries a failed profile save', async () => {
    ...
    },
    {
      ...
      respond: ({ body }) => {
        ...
        return Response.json({
          ...mixedIntegration,
          ...schemas.zSaveProjectIntegrationRequest.parse(body),
        })
      },
    },
  ])
  ...
  expect(updates[1]?.body).toEqual({
    name: mixedIntegration.name,
    integration_type: mixedIntegration.integration_type,
    settings: {
      ...mixedIntegration.settings,
      launcher: {
        ...mixedIntegration.settings.launcher,
        slots: [
          { key: 'off-page', agent_profile_id: reviews.id },
          { key: 'profile_1', agent_id: fakeId('agt') },
  ...
  async (existingCount) => {
    ...
    const api = fakeApi([
      ...
      },
      {
        ...
        respond: ({ body }) =>
          Response.json({ ...integration, ...schemas.zSaveProjectIntegrationRequest.parse(body) }),
      },
    ])
    const closed = vi.fn()
    render(
    ...
    if (existingCount === 15) {
      ...
      await submit()
      expect(api.requestsTo('PUT', path + '/integrations/' + integration.id)).toHaveLength(0)
    } else {
      expect(button('Save changes').disabled).toBe(false)
    }
    act(() => {
    ...
    expect(api.requestsTo('PUT', path + '/integrations/' + integration.id)[0]?.body).toEqual({
...
it('requires a public key for even one Discord launch profile', async () => {
  const integration = {
    ...mixedIntegration,
    integration_type: 'discord_thread' as const,
    settings: {
      launcher: {
        trigger: 'mention',
        ...
        launcher: {
          ...
          slots:
            scenario === 'saved channel'
              ? mixedIntegration.settings.launcher?.slots
