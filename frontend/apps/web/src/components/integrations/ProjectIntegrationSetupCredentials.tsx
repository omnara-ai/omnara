...
import { IntegrationCredentialFields } from './ProjectIntegrationFormCredentials'

export function ProjectIntegrationSetupCredentials({
  orgId,
  projectId,
  integrationType,
  name,
        ...
        <>
          ...
          {newCredential ? (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <IntegrationCredentialFields integrationType={integrationType} />
            </div>
          ) : (
...
export function ProjectIntegrationCredentialPicker({
  orgId,
  projectId,
  integrationType,
  value,
  ...
  const secretsQuery = useProjectAvailableSecrets(orgId, projectId, {
    filters: { kind: integrationType === 'github_pr' ? 'github_app_credentials' : 'generic' },
  })
