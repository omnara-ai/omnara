...
import { installFailureTracking, installIntegrationFailureTracking } from '../../e2e/fixtures'

const origin = 'https://app.omnara.test'
const integrationPath =
  '/api/v1/orgs/org_test/projects/proj_test/integrations/itg_aaaaaaaaaaaaaaaaaaaaaaaaaa'
...
function request(method: string, path = integrationPath, error = 'net::ERR_ABORTED') {
  return {
    method: () => method,
    url: () => `${origin}${path}`,
...
it.each(['POST', 'PUT', 'PATCH', 'DELETE'])(
  ...
  (method) => {
    ...
    expect(failures).toEqual([`request: ${method} ${origin}${integrationPath} (net::ERR_ABORTED)`])
  },
)
...
it.each([200, 404, 500])('records an aborted DELETE after a %i', (status) => {
  ...
  page.emit('requestfailed', deletion)
  expect(failures).toEqual([
    ...(status >= 400 ? [`response: ${status} ${integrationPath}`] : []),
    `request: DELETE ${origin}${integrationPath} (net::ERR_ABORTED)`,
  ])
})
...
it('does not borrow a 204 from a different request to the same URL', () => {
  ...
  expect(failures).toEqual([`request: DELETE ${origin}${integrationPath} (net::ERR_ABORTED)`])
})

it('records other DELETE failures even after a 204', () => {
  const { page, failures } = tracker()
  const deletion = request('DELETE', integrationPath, 'net::ERR_CONNECTION_RESET')
  page.emit('response', { request: () => deletion, status: () => 204, url: deletion.url })
  page.emit('requestfailed', deletion)
  expect(failures).toEqual([
    `request: DELETE ${origin}${integrationPath} (net::ERR_CONNECTION_RESET)`,
  ])
})
...
it('cannot use a broad ignore pattern to suppress a write or an uncanceled read failure', () => {
  ...
  page.emit('requestfailed', request('GET', integrationPath, 'net::ERR_CONNECTION_RESET'))
  expect(failures).toEqual([
    `request: PUT ${origin}${integrationPath} (net::ERR_ABORTED)`,
    `request: POST ${origin}${integrationPath} (net::ERR_ABORTED)`,
    `request: GET ${origin}${integrationPath} (net::ERR_CONNECTION_RESET)`,
  ])
})
