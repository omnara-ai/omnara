...
export async function exerciseGuidedGitHubSetup(page: Page, projectID: string) {
  ...
  await page.route(`**/integrations/${integration.id}`, (route) =>
    route.fulfill({ json: integration }),
  )
  await page.route(`**/integrations/${integration.id}/subscriptions*`, (route) =>
    route.fulfill({ json: { data: [], next_cursor: null } }),
  )
  await page.route(`**/projects/${projectID}/secrets*`, (route) =>
    route.fulfill({ json: { data: [], next_cursor: null } }),
  )
  await page.route(`**/integrations/${integration.id}/github-setup`, async (route) => {
  ...
  await page.route(
    `**/integrations/${integration.id}/github-setup/installations`,
    async (route) => {
      inspections++
      expect(
