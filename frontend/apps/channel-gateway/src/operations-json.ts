 ...
 * It never represents payload numbers as JS numbers. Multipart framing is
 * handled separately by the maintained streaming parser.
 */
export function parseObjectFields(raw: string, maxBytes: number): Map<string, string> {
  ...
  const value = (depth: number): void => {
    if (depth > 128 || ++nodes > 16_384) throw new InvalidOperationError()
    whitespace()
    ...
    while (raw[offset] !== close) {
      ...
      if (object) {
        ...
        if (!isBoundedTextKey(decoded) || seen.has(decoded)) throw new InvalidOperationError()
        key = decoded
        seen.add(key)
        whitespace()
...
function isBoundedTextKey(value: unknown): value is string {
