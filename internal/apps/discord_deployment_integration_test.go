...
func testDiscordDeploymentHandoff(t *testing.T, scenario string) {
	...
	require.NoError(t, err)
	f.appSetup, err = f.store.Apps().ConfigureProjectApp(ctx, appstore.ConfigureProjectAppInput{
	...
	provider.apps, provider.secrets = f.store.Apps(), f.store.Secrets()
	...
	require.NoError(t, err)
	f.appSetup, err = f.store.Apps().UpdateProjectApp(ctx, f.appSetup.ID, appstore.SaveProjectAppInput{
		...
		Settings: appstore.ProjectAppSettings{Launcher: &appstore.AppLauncher{
			Trigger: "mention",
			Slots:   []appstore.AppLaunchSlot{{Key: "helper", AgentProfileID: &profile.ID}},
		}},
	})
	require.NoError(t, err)
	...
	newRuntime := func() *DiscordRuntime {
		return &DiscordRuntime{
			Apps: f.store.Apps(), Secrets: f.store.Secrets(), Redis: redis, HTTPClient: client,
		}
	}
	newWorker := func() *AppInboxWorker {
		router := NewAppRouter(f.store.Execution(), f.store.Apps())
		consumer := NewAppInboxConsumer(router, f.store.Apps(), nil,
			map[string]AppInboxProvider{"discord": provider}, nil, testAppLaunchWorkflow(router))
		return NewAppInboxWorker(f.store.Apps(), consumer, AppInboxWorkerOptions{})
	}
	...
	first, found, err := f.store.Apps().ClaimAppRuntime(ctx, revision, discordRuntimeLease)
	require.NoError(t, err)
	...
	case "crash":
		...
		_, found, err = f.store.Apps().ClaimAppRuntime(ctx, revision, discordRuntimeLease)
		require.NoError(t, err)
	...
	require.NoError(t, f.pool.QueryRow(ctx, `SELECT count(*),count(*) FILTER (WHERE state='pending')
		FROM app_inbox WHERE app_id=$1`, f.appSetup.ID).Scan(&captured, &pending))
	...
	var second appstore.AppRuntimeClaim
	require.Eventually(t, func() bool {
		second, found, err = f.store.Apps().ClaimAppRuntime(ctx, revision, discordRuntimeLease)
	...
	require.ErrorIs(t, f.store.Apps().RenewAppRuntime(ctx, first.Lease, discordRuntimeLease),
		appstore.ErrAppRuntimeLeaseLost)
	require.ErrorIs(t, f.store.Apps().CommitAppRuntime(ctx, first.Lease,
		json.RawMessage(`{"sequence":999}`), &appstore.VerifiedAppReceipt{
			ProjectID: f.appSetup.ProjectID, AppID: f.appSetup.ID, ReceiptKey: "discord:stale",
			Payload: discordInboxPayload(t, reply),
		}), appstore.ErrAppRuntimeLeaseLost)
	require.ErrorIs(t, f.store.Apps().ReleaseAppRuntime(ctx, first.Lease, 0, ""),
		appstore.ErrAppRuntimeLeaseLost)
	...
	require.NoError(t, f.pool.QueryRow(ctx, `SELECT count(*),count(*) FILTER (WHERE state='completed')
		FROM app_inbox WHERE app_id=$1`, f.appSetup.ID).Scan(&receipts, &completed))
