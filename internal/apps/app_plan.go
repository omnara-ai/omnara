...
type appEventCandidates struct {
	...
	scopes     []appstore.ConversationAddress
...
func (r *AppRouter) Freeze(
	...
	appSetup, err := r.apps.GetProjectAppByID(ctx, receipt.AppID)
	if err != nil {
		return nil, err
	}
	...
	err = r.apps.WithAppInboxLease(
		...
		func(work *appstore.AppInboxLeaseTx) error {
			...
			for i := range requests {
				request := &requests[i]
				var err error
				request.candidates, err = r.apps.AppRoutingCandidatesForInbox(
					ctx,
					work,
					request.address,
					request.scopes,
	...
	err = r.apps.WithAppInboxLease(
		...
		func(work *appstore.AppInboxLeaseTx) error {
			...
			for _, request := range requests {
				current, err := r.apps.AppRoutingCandidatesForInbox(
					ctx,
					work,
					request.address,
					request.scopes,
	...
	for order, event := range events {
		...
		for _, address := range addresses {
			request.scopes = append(
				request.scopes,
				appstore.ConversationAddress{Kind: address.Kind, Ref: address.Ref},
			)
		}
...
func (r appEventCandidates) matchesSubscriptionAddress(address appstore.ConversationAddress) bool {
...
func sameAppCandidates(a, b appstore.AppRoutingCandidates) bool {
	canonical := func(c appstore.AppRoutingCandidates) appstore.AppRoutingCandidates {
		c.Subscriptions, c.Selections = slices.Clone(c.Subscriptions), slices.Clone(c.Selections)
		slices.SortFunc(
			c.Subscriptions,
			func(a, b appstore.AppSubscriptionRecord) int { return bytes.Compare(a.ID[:], b.ID[:]) },
		)
		slices.SortFunc(
			c.Selections,
			func(a, b appstore.AppTargetRecord) int { return bytes.Compare(a.ID[:], b.ID[:]) },
		)
		return c
	}
...
func (r *AppRouter) buildAppPlan(
	...
	var planned []appstore.AppSubscriptionRecord
	...
	for _, request := range requests {
		...
		for _, intent := range event.Launches {
			...
			for _, subscription := range subscriptions {
				planned = append(planned, appstore.AppSubscriptionRecord{
...
func resolveAppLaunchIntent(
	...
	candidates appstore.AppRoutingCandidates,
) (appstore.ProjectAppRecord, uuid.UUID, error) {
	unavailable := func() (appstore.ProjectAppRecord, uuid.UUID, error) {
		return appstore.ProjectAppRecord{}, uuid.Nil,
	...
	if app == nil || app.ID != intent.AppID || app.ProjectID != projectID ||
		app.State != appstore.ProjectAppStateActive || app.Settings.Launcher == nil {
		return unavailable()
	}
...
func deriveAppLaunch(
	base executionstore.AgentConfigRecord, app appstore.ProjectAppRecord, scope appdefinition.Scope,
) (executionstore.CreateAgentConfigInput, []appstore.AppSubscriptionAttachment, error) {
...
func deriveAppLaunchConfig(
	base executionstore.AgentConfigRecord, app appstore.ProjectAppRecord,
) (executionstore.CreateAgentConfigInput, error) {
	if base.ProjectID != app.ProjectID || app.State != appstore.ProjectAppStateActive {
...
) ([]appstore.AppSubscriptionAttachment, error) {
