...
func runInboxLoad(t *testing.T, capacity int, poolSize int32, holdSnapshot bool, duration time.Duration) {
 ...
 FROM generate_series(1,100000) g`, ids.ProjectID, apps, payload)
	require.NoError(t, err)
	_, err = pool.Exec(ctx, "VACUUM (ANALYZE) app_inbox")
	require.NoError(t, err)
	...
	if holdSnapshot {
		...
		var rows int
		require.NoError(t, snapshot.QueryRow(ctx, "SELECT count(*) FROM app_inbox").Scan(&rows))
	...
	consumer := appWorkerConsumerFunc(func(
		callCtx context.Context, lease appstore.AppInboxLease,
	) ([]AppSlotAdmission, error) {
		started := time.Now()
		var created time.Time
		err := inbox.WithAppInboxLease(callCtx, lease, func(work *appstore.AppInboxLeaseTx) error {
	...
	for range 8 {
		group.Go(func() error {
			for phase.Err() == nil && workCtx.Err() == nil {
				n := sequence.Add(1)
				begin := time.Now()
				_, created, err := inbox.AcceptAppReceipt(workCtx, appstore.VerifiedAppReceipt{
	...
	group.Go(func() error {
		...
		for phase.Err() == nil && workCtx.Err() == nil {
			...
			for time.Now().Before(until) && phase.Err() == nil {
				begin := time.Now()
				n, err := inbox.CleanupTerminalAppInbox(workCtx, 7*24*time.Hour, 100)
				if err != nil {
					return err
				}
				cleanupCalls++
	...
	group.Go(func() error {
		...
		for {
			...
			case <-workCtx.Done():
				return workCtx.Err()
			case <-ticker.C:
				oldest, err := inbox.OldestReadyAppInboxLag(workCtx)
				if err != nil {
					return err
				}
 ...
 FROM app_inbox`).Scan(&live, &done, &pending, &expired))
	...
	oldest, err := inbox.OldestReadyAppInboxLag(ctx)
	require.NoError(t, err)
	...
	if snapshot != nil {
		var visible int
		require.NoError(t, snapshot.QueryRow(ctx, "SELECT count(*) FROM app_inbox").Scan(&visible))
...
func logInboxLoadStorage(t *testing.T, ctx context.Context, pool *pgxpool.Pool, label string) {
 ...
 pg_total_relation_size(relid),n_live_tup,n_dead_tup FROM pg_stat_user_tables WHERE relname='app_inbox'`).
