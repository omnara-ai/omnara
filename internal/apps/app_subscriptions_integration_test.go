...
func TestAppRouterSameExpansionSubscriptionAdmitsMessagesAndMedia(t *testing.T) {
	...
	_, err = store.Apps().UpdateProjectApp(ctx, appID, appstore.SaveProjectAppInput{
		...
		Settings: appstore.ProjectAppSettings{Launcher: &appstore.AppLauncher{
			...
			Slots: []appstore.AppLaunchSlot{{Key: "review", AgentProfileID: &profile.ID}},
		}},
	})
	require.NoError(t, err)
	...
	subscriptions, err := store.Apps().ListAppSubscriptions(ctx, appstore.ListAppSubscriptionsInput{
...
func createTestAppSubscription(
	t *testing.T, store *storage.Store, app appstore.ProjectAppRecord, agentID uuid.UUID,
	subscriptionType string, conversation string, events ...string,
) appstore.AppSubscriptionRecord {
	t.Helper()
	subscription, err := store.Apps().CreateAppSubscription(
		t.Context(), appstore.CreateAppSubscriptionInput{
...
func removeTestAgentSubscriptions(
	...
	t.Helper()
	input := appstore.ListAppSubscriptionsInput{ProjectID: app.ProjectID, AppID: app.ID, Limit: 100}
	for {
		result, err := store.Apps().ListAppSubscriptions(t.Context(), input)
		require.NoError(t, err)
		for _, subscription := range result.Subscriptions {
			if subscription.AgentID == agentID {
				require.NoError(
					t,
					store.Apps().DeleteAppSubscription(t.Context(), app.OrgID, app.ProjectID, app.ID, subscription.ID),
				)
			}
		}
		if !result.HasMore {
