...
func slackInboxTestApp() appstore.ProjectAppRecord {
...
func (s *slackInboxTestAccess) GetConversationDisplayName(
	context.Context, uuid.UUID, uuid.UUID, appstore.ConversationAddress,
) (string, error) {
	return "", nil
}
...
func TestAppEventsDiscordGuildDiffersFromAppApplication(t *testing.T) {
	...
	require.Equal(t, appstore.ConversationAddress{Kind: "thread", Ref: "456:789"}, requests[0].address)
}
	...
	for _, change := range []string{"disabled", "reconfigured", "rotated", "grant_revoked"} {
		t.Run(change, func(t *testing.T) {
			...
			access.afterRead = func() {
				switch change {
				case "disabled":
					access.appSetup.State = appstore.ProjectAppStateDisconnected
				case "reconfigured":
