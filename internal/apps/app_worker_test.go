...
type appWorkerConsumerFunc func(context.Context, appstore.AppInboxLease) ([]AppSlotAdmission, error)
...
type appWorkerTestStore struct {
	...
	recovered    int
	ready        bool
	retried      []appstore.AppInboxLease
	apps         []appstore.AppInboxApp
	claimOrder   []uuid.UUID
	scans        int
	noClaim      bool
	discover     func(appstore.AppInboxApp) appstore.AppInboxAppPage
	recoverBatch func(context.Context, int) (int64, error)
	sampleLag    func(context.Context) (time.Duration, error)
	sampled      int
}

func (s *appWorkerTestStore) RecoverAppInbox(ctx context.Context, limit int) (int64, error) {
	s.mu.Lock()
	s.recovered++
	s.mu.Unlock()
...
func (s *appWorkerTestStore) OldestReadyAppInboxLag(ctx context.Context) (time.Duration, error) {
	s.mu.Lock()
	s.sampled++
	s.mu.Unlock()
	if s.sampleLag != nil {
...
func (s *appWorkerTestStore) ListReadyAppInboxApps(
	_ context.Context,
	...
	if !s.ready {
		return appstore.AppInboxAppPage{}, nil
	}
	if len(s.apps) > 0 {
		return appstore.AppInboxAppPage{
			Apps: append([]appstore.AppInboxApp(nil), s.apps...),
		}, nil
	}
	return appstore.AppInboxAppPage{
		Apps: []appstore.AppInboxApp{{ProjectID: uuid.New(), AppID: uuid.New()}},
	}, nil
}

func (s *appWorkerTestStore) ClaimAppInbox(
	_ context.Context,
	input appstore.ClaimAppInboxInput,
	...
	if s.noClaim {
		return appstore.AppInboxRecord{}, false, nil
	}
	return appstore.AppInboxRecord{
		ID:           uuid.New(),
...
func TestAppInboxWorkerRotatesAppsBeforeRevisitingHotApp(t *testing.T) {
	...
	worker := NewAppInboxWorker(
		...
		appWorkerConsumerFunc(
			func(context.Context, appstore.AppInboxLease) ([]AppSlotAdmission, error) {
				return nil, nil
			},
		),
...
func TestAppInboxWorkerAdvancesPastUnavailableAppsAndWraps(t *testing.T) {
	first := appstore.AppInboxApp{ProjectID: uuid.New(), AppID: uuid.New()}
	last := appstore.AppInboxApp{ProjectID: uuid.New(), AppID: uuid.New()}
	var cursors []appstore.AppInboxApp
	store := &appWorkerTestStore{discover: func(
		after appstore.AppInboxApp,
	) appstore.AppInboxAppPage {
		cursors = append(cursors, after)
		if after == (appstore.AppInboxApp{}) {
			return appstore.AppInboxAppPage{NextCursor: first}
		}
		require.Equal(t, first, after)
		return appstore.AppInboxAppPage{Apps: []appstore.AppInboxApp{last}}
	}}
	...
	require.Equal(t, []appstore.AppInboxApp{{}, first, {}, first}, cursors)
...
func TestAppInboxWorkerBoundsUnavailableAppDiscovery(t *testing.T) {
	for _, complete := range []bool{false, true} {
		t.Run(fmt.Sprintf("complete=%t", complete), func(t *testing.T) {
			store := &appWorkerTestStore{discover: func(
				appstore.AppInboxApp,
			) appstore.AppInboxAppPage {
				if complete {
					return appstore.AppInboxAppPage{}
				}
				return appstore.AppInboxAppPage{
					NextCursor: appstore.AppInboxApp{ProjectID: uuid.New(), AppID: uuid.New()},
				}
			}}
			worker := NewAppInboxWorker(store, appWorkerConsumerFunc(
				func(context.Context, appstore.AppInboxLease) ([]AppSlotAdmission, error) {
			...
			if complete {
				...
				require.Equal(t, appstore.AppInboxMaxBatch, store.scans, "bound a large traversal")
			}
		})
	}
}
...
func TestAppInboxWorkerRecoveryContinuesWhileAllConsumersAreBusy(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		defer cancel()
		appSetup := appstore.AppInboxApp{ProjectID: uuid.New(), AppID: uuid.New()}
		store := &appWorkerTestStore{
			ready: true,
			apps:  []appstore.AppInboxApp{appSetup},
		}
		started := make(chan struct{}, 1)
		consumer := appWorkerConsumerFunc(
			func(ctx context.Context, _ appstore.AppInboxLease) ([]AppSlotAdmission, error) {
				started <- struct{}{}
				<-ctx.Done()
				return nil, ctx.Err()
			},
...
func (s *appWorkerTestStore) WithAppInboxLease(
	ctx context.Context,
	lease appstore.AppInboxLease,
	_ func(*appstore.AppInboxLeaseTx) error,
) error {
...
func TestAppInboxWorkerBoundedConcurrencyAndShutdown(t *testing.T) {
	...
	started := make(chan appstore.AppInboxLease, 4)
	consumer := appWorkerConsumerFunc(
