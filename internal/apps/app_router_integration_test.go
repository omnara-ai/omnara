...
func TestAppRouterConcurrentFreezePartialRecoveryAndPinnedConfig(t *testing.T) {
	...
	apps := store.Apps()
	router := NewAppRouter(store.Execution(), apps)
	...
	app, err := store.Apps().UpdateProjectApp(ctx, appSetup, setup)
	require.NoError(t, err)
	claim := func(key string) appstore.AppInboxRecord {
		_, _, err := store.Apps().
			AcceptAppReceipt(
				ctx,
	...
	receipts := []appstore.AppInboxRecord{claim("first"), claim("second")}
	...
	require.ErrorIs(t, failures[1-winner], appstore.ErrAppSelectionReserved)
	plan, receipt := plans[winner], receipts[winner]
	require.Len(t, plan, 2)
	frozen, err := apps.GetAppInbox(ctx, ids.ProjectID, receipt.ID)
	require.NoError(t, err)
	...
	_, err = store.Apps().UpdateProjectApp(ctx, app.ID, setup)
	require.NoError(t, err)
	...
	consumer := NewAppInboxConsumer(
		router, apps, nil, map[string]AppInboxProvider{"slack": provider}, nil, testAppLaunchWorkflow(router),
	)
...
func TestAppRouterPlainFollowupWaitsForReservedConversation(t *testing.T) {
	for _, ownerState := range []appstore.AppInboxState{
		appstore.AppInboxProcessing,
		appstore.AppInboxPending,
		appstore.AppInboxFailed,
	} {
		t.Run(string(ownerState), func(t *testing.T) {
				...
				appstore.SaveProjectAppInput{
					...
					Settings: appstore.ProjectAppSettings{
						Launcher: &appstore.AppLauncher{
							...
							ScopeKind: "workspace",
							ScopeRef:  "T123",
							Slots:     []appstore.AppLaunchSlot{{Key: "reviewer", AgentProfileID: &profile.ID}},
						},
					},
				},
			)
			require.NoError(t, err)
			claim := func() appstore.AppInboxRecord {
				r, found, err := inbox.ClaimAppInbox(
					ctx,
					appstore.ClaimAppInboxInput{
						ProjectID:     ids.ProjectID,
						AppID:         appSetup,
			...
			if ownerState != appstore.AppInboxProcessing {
				require.NoError(
					t,
					inbox.WithAppInboxLease(
						ctx,
						owner.Lease(),
						func(work *appstore.AppInboxLeaseTx) error {
							if ownerState == appstore.AppInboxPending {
			...
			if ownerState == appstore.AppInboxFailed {
				frozen, err := inbox.GetAppInbox(ctx, ids.ProjectID, owner.ID)
				require.NoError(t, err)
				for i := range 12 {
					if i > 0 {
					...
					require.NoError(t, err)
					completed, err := inbox.GetAppInbox(ctx, ids.ProjectID, follow.ID)
					require.NoError(t, err)
				...
				unchanged, err := inbox.GetAppInbox(ctx, ids.ProjectID, owner.ID)
				require.NoError(t, err)
				...
				unplanned, err := inbox.GetAppInbox(ctx, ids.ProjectID, follow.ID)
				require.NoError(t, err)
				require.Empty(t, unplanned.Plan, "plain follow-up must not freeze empty")
				require.Equal(t, appstore.AppInboxProcessing, unplanned.State)
			}
			...
			if ownerState == appstore.AppInboxPending {
				_, err := pool.Exec(ctx, `UPDATE app_inbox SET available_at=now() WHERE id=$1`, owner.ID)
				require.NoError(t, err)
			}
			if ownerState != appstore.AppInboxProcessing {
				owner = claim()
