...
func (p *DiscordAppInboxProvider) NotifyInboxFailure(ctx context.Context,
	app appstore.ProjectAppRecord, receipt appstore.AppInboxRecord, text string,
) error {
	if err := checkInboxFailureReceipt(app, receipt, appdefinition.ProviderDiscord); err != nil {
		return err
	}
	...
	switch {
	case receipt.Source == appstore.AppInboxSourceScheduled:
		known, ok, err := scheduledInboxFailureScope(receipt, appdefinition.ProviderDiscord)
		if err != nil || !ok {
			return err
		}
	...
	case receipt.Source != appstore.AppInboxSourceProvider:
		return nil
	case len(receipt.Events) != 0:
		event, err := inboxFailureSelectedEvent(receipt, appdefinition.ProviderDiscord)
		if err != nil {
			return err
		}
	...
	default:
		message, ok, err := discordInboxMessage(app, receipt.Payload)
		if err != nil || !ok {
			return err
		}
	...
	client, _, err := p.requestAccess(ctx, app, nil)
	if err != nil {
		return err
	}
	if source != nil {
