...
func TestAppRouterOverlappingSlackSetupsLaunchAndContinueIndependently(t *testing.T) {
	...
	app, err := inbox.UpdateProjectApp(ctx, appID, appstore.SaveProjectAppInput{
	...
	require.NoError(t, err)
	apps := []appstore.ProjectAppRecord{app, seedIndependentApp(t, pool, app, "other-chat")}
	capture := func(app appstore.ProjectAppRecord, key string) appstore.AppInboxRecord {
		t.Helper()
		_, created, err := inbox.AcceptAppReceipt(ctx, appstore.VerifiedAppReceipt{
	...
	_, err = inbox.UpdateProjectApp(ctx, apps[0].ID, appstore.SaveProjectAppInput{
	...
	unchanged, err := inbox.GetAppInbox(ctx, ids.ProjectID, receipt.ID)
	require.NoError(t, err)
	require.Empty(t, unchanged.Plan)
	attempts := 0
	worker := NewAppInboxWorker(inbox, appWorkerConsumerFunc(func(
		ctx context.Context, lease appstore.AppInboxLease,
	...
	_, err = pool.Exec(ctx, `UPDATE app_inbox SET available_at=now() WHERE id=$1`, receipt.ID)
	require.NoError(t, err)
...
func TestAppRouterDirectedSettledIntentWithoutSubscription(t *testing.T) {
	...
	require.NoError(t, err)
	app, err := inbox.UpdateProjectApp(ctx, appSetup, appstore.SaveProjectAppInput{
		OrgID: ids.OrgID, ProjectID: ids.ProjectID, Name: "chat", AppType: appdefinition.SlackThread,
	...
	require.Len(t, results, 1)
	require.True(t, results[0].Launch.Created)
	agentID, targetID := results[0].Launch.Agent.ID, results[0].Launch.AppTarget.ID
	...
	for _, directed := range []bool{false, true} {
		...
		if directed {
			...
			require.Equal(t, targetID, results[0].Input.AgentInput.AppTargetID)
		} else {
