...
func feedbackReceipt(app appstore.ProjectAppRecord, payload []byte) appstore.AppInboxRecord {
	return appstore.AppInboxRecord{
		ID: uuid.New(), ProjectID: app.ProjectID, AppID: app.ID, State: appstore.AppInboxFailed,
		Source: appstore.AppInboxSourceProvider, Payload: payload, LastError: "private failure details",
	}
}
...
func feedbackPlan(t *testing.T, scope appdefinition.Scope) json.RawMessage {
	...
	return githubEventJSON(t, AppInboxPlan{"scheduled": {
		Scope: scope, AgentID: uuid.New(), Selection: &appstore.InboxAppSelection{},
...
func TestSlackNotifyInboxFailureDestination(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.scheduled {
				receipt.Source = appstore.AppInboxSourceScheduled
...
func TestInboxFailureSkipsUnknownScheduledOpeningsAndAutomatedMessages(t *testing.T) {
	...
	for _, plan := range []json.RawMessage{nil, json.RawMessage(`{}`)} {
		...
		slackReceipt.Source, discordReceipt.Source = appstore.AppInboxSourceScheduled,
			appstore.AppInboxSourceScheduled
	...
	unknownSlack.Source, unknownDiscord.Source = appstore.AppInboxSourceScheduled,
		appstore.AppInboxSourceScheduled
