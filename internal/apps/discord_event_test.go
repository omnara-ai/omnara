...
func discordInboxApp() appstore.ProjectAppRecord {
	return appstore.ProjectAppRecord{
		...
		State: appstore.ProjectAppStateActive, ProviderAgentDisplayName: "Helper",
	}
}
...
func TestDiscordInboxRoutesBeforeIdentityAndAttachments(t *testing.T) {
	t.Parallel()
	for _, routeErr := range []error{nil, appstore.ErrAppSelectionReserved} {
...
func TestDiscordInboxRejectsUnprovenScope(t *testing.T) {
	...
	for _, raw := range [][]byte{{0xff}, bytes.Repeat([]byte(" "), appstore.AppInboxMaxPayloadBytes+1)} {
...
func (f *discordInboxFixture) GetProjectApp(
	...
	if projectID != f.appSetup.ProjectID || appID != f.appSetup.ID {
		return appstore.ProjectAppRecord{}, storeerr.ErrUnauthorized
	}
...
func TestDiscordInboxAccessIsRevalidated(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"app disabled", func(f *discordInboxFixture) {
			f.appSetup.State = appstore.ProjectAppStateDisconnected
		}},
