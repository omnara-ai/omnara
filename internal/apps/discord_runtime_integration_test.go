...
func newDiscordRuntimeFixture(t *testing.T) discordRuntimeFixture {
	...
	require.NoError(t, err)
	appSetup, err := store.Apps().ConfigureProjectApp(ctx, appstore.ConfigureProjectAppInput{
		OrgID: ids.OrgID, ProjectID: ids.ProjectID, AppID: app.ID,
		InstalledByUserID: ids.ProviderAdminUserID, Provider: appstore.AppProviderDiscord,
...
func TestDiscordRuntimePersistsResumeAndFencesRevokedCredentials(t *testing.T) {
	...
	claim, found, err := store.Apps().ClaimAppRuntime(ctx, revision, discordRuntimeLease)
	require.NoError(t, err)
	require.True(t, found)
	updated, err := store.Apps().UpdateProjectApp(ctx, appSetup.ID, appstore.SaveProjectAppInput{
	...
	require.NoError(
		t,
		pool.QueryRow(ctx, `SELECT count(*) FROM app_inbox WHERE app_id=$1`, appSetup.ID).Scan(&count),
	)
	require.Equal(t, 1, count)
	_, found, err = store.Apps().ClaimAppRuntime(ctx, revision, discordRuntimeLease)
	require.NoError(t, err)
	...
	r.runShard = func(
		...
		require.NoError(t, err)
		require.ErrorIs(t, config.BeforeConnect(ctx), appstore.ErrAppRuntimeLeaseLost)
		checkpoint.Sequence++
		require.ErrorIs(
			t,
			commit(
			...
			appstore.ErrAppRuntimeLeaseLost,
		)
...
func TestDiscordRuntimePersistsOnlySafeFailureMessage(t *testing.T) {
	...
	for _, cause := range []error{
		...
		f := newDiscordRuntimeFixture(t)
		ctx := t.Context()
		claim, found, err := f.store.Apps().ClaimAppRuntime(ctx, appstore.AppRuntimeRevision{
		...
		require.True(t, found)
		r := DiscordRuntime{
			Apps: f.store.Apps(), Secrets: f.store.Secrets(),
		...
		failure, err := f.store.Apps().GetAppRuntimeFailure(ctx,
...
func TestDiscordRuntimeScanClaimsAvailableAppsWithinCapacity(t *testing.T) {
	...
	defer cancel()
	apps := []appstore.ProjectAppRecord{f.appSetup}
	for i := range 3 {
		app, err := f.store.Apps().CreateProjectApp(ctx, appstore.SaveProjectAppInput{
			OrgID: f.appSetup.OrgID, ProjectID: f.appSetup.ProjectID,
			Name: fmt.Sprintf("discord-%d", i), AppType: appdefinition.DiscordThread,
		})
		require.NoError(t, err)
		appSetup, err := f.store.Apps().ConfigureProjectApp(ctx, appstore.ConfigureProjectAppInput{
			OrgID: f.appSetup.OrgID, ProjectID: f.appSetup.ProjectID, AppID: app.ID,
			InstalledByUserID: f.appSetup.InstalledByUserID, Provider: appstore.AppProviderDiscord,
	...
	owner, found, err := f.store.Apps().ClaimAppRuntime(ctx, revision, discordRuntimeLease)
	require.NoError(t, err)
	require.True(t, found)
	...
	require.NoError(t, f.store.Apps().RenewAppRuntime(t.Context(), owner.Lease, discordRuntimeLease))
	var total, owned int
...
func TestDiscordRuntimeCheckpointDoesNotPreventCredentialOrProjectDeletion(t *testing.T) {
	for _, remove := range []string{"credential", "project"} {
		t.Run(remove, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.True(t, found)
			require.NoError(t, f.store.Apps().CommitAppRuntime(ctx, claim.Lease,
				json.RawMessage(`{"session_id":"retired"}`), nil))
			require.NoError(t, f.store.Apps().ReleaseAppRuntime(ctx, claim.Lease, 0, ""))
			actor := identitystore.NewUserPrincipal(f.appSetup.InstalledByUserID)
			if remove == "credential" {
				require.NoError(t, f.store.Apps().DeleteProjectApp(ctx,
...
func TestDiscordRuntimeScanPassesOwnedPageAndWraps(t *testing.T) {
	...
	var firstOwner appstore.AppRuntimeClaim
	...
	require.NoError(t, f.store.Apps().ReleaseAppRuntime(ctx, firstOwner.Lease, 0, ""))
	close(releaseLast)
