...
func CleanupTerminalAppInboxArtifacts(
	...
	defer cancel()
	receipt, err := inbox.GetAppInbox(ctx, projectID, receiptID)
	if err != nil {
		return err
	}
	...
	if receipt.State != appstore.AppInboxFailed &&
		receipt.State != appstore.AppInboxCompleted {
		return nil
	}
	...
	for key, slot := range plan {
		...
		if receipt.State == appstore.AppInboxCompleted && outcome.Committed == nil {
			return storeerr.ErrStateTransitionConflict
		}
