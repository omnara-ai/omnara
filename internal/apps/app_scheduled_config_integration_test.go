...
func TestScheduledLaunchRetryReusesConfigFrozenBeforeProfileEdit(t *testing.T) {
	for _, provider := range []string{appdefinition.ProviderSlack, appdefinition.ProviderDiscord} {
		t.Run(provider, func(t *testing.T) {
			...
			frozen, err := f.store.Apps().GetAppInbox(t.Context(), f.ids.ProjectID, receipt.ID)
			require.NoError(t, err)
			require.Equal(t, appstore.AppInboxPending, frozen.State)
			...
			completed, err := f.store.Apps().GetAppInbox(t.Context(), f.ids.ProjectID, receipt.ID)
			require.NoError(t, err)
