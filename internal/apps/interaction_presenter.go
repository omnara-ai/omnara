...
func (p InteractionPresenter) access(
	...
	appSetup, err := p.Store.Apps().GetProjectApp(ctx, projectID, destination.AppID)
	if err != nil {
		return interactionAccess{}, err
	}
	if appSetup.State != appstore.ProjectAppStateActive || appSetup.AppType != destination.AppType {
	...
	kind, err := appstore.ProjectAppCredentialKind(appSetup.Provider)
	if err != nil {
		return interactionAccess{}, err
	}
...
func (p InteractionPresenter) recheck(
	...
	current, err := p.Store.Apps().
		GetProjectApp(ctx, access.appSetup.ProjectID, access.appSetup.ID)
	if err != nil {
		return err
	}
	if current.State != appstore.ProjectAppStateActive ||
...
func DiscordInteractionScope(destination executionstore.InteractionDestination) (discord.Scope, error) {
	...
	if destination.Address != (appstore.ConversationAddress{Kind: kind, Ref: ref}) {
		return discord.Scope{}, storeerr.ErrUnauthorized
	}
...
func SlackInteractionPromptPayload(
	...
	targetID, err := publicid.Encode(publicid.KindAppTarget, destination.AppTargetID)
	if err != nil {
		return nil, err
	}
	...
	text, blocks := slack.InteractionFormPromptBlocks(form, slack.PromptActionValue{Type: slack.PromptType,
		InteractionID: interactionID, AgentID: agent, AppTargetID: targetID})
