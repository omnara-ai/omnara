...
type AppProfileChoiceProvider interface {
	PresentProfileChoice(context.Context, appstore.ProjectAppRecord,
		appstore.AppProfileChoiceRecord, func(context.Context) error) (string, string, error)
	DismissProfileChoice(context.Context, appstore.ProjectAppRecord,
		appstore.AppProfileChoiceRecord, string) error
}
...
func NewChatAppLauncher(
	store *appstore.Store, profiles AppProfileNames, providers map[string]AppInboxProvider,
) *ChatAppLauncher {
...
func (l *ChatAppLauncher) decideProfiles(
	...
	var options []appstore.AppProfileChoiceOption
	if exists {
		...
		for _, intent := range profiles {
			...
			options = append(options, appstore.AppProfileChoiceOption{
	...
	if errors.Is(err, appstore.ErrAppSelectionSettled) {
		return nil, nil
	}
	if err != nil {
		return nil, err
	...
	check := func(ctx context.Context) error {
		if err := l.store.WithAppInboxLease(ctx, input.Receipt.Lease(),
			func(*appstore.AppInboxLeaseTx) error { return nil }); err != nil {
			return err
		}
		...
		if app.State != appstore.ProjectAppStateActive {
...
func (l *ChatAppLauncher) selectedChoiceIntent(
	ctx context.Context, choice appstore.AppProfileChoiceRecord,
) ([]AppLaunchIntent, error) {
	...
	if receipt.State == appstore.AppInboxPending ||
		receipt.State == appstore.AppInboxProcessing {
		return nil, &appstore.AppSelectionReservationError{ReceiptID: receipt.ID, State: receipt.State}
	}
	return nil, nil
}

func choiceIntent(choice appstore.AppProfileChoiceRecord) []AppLaunchIntent {
...
func SelectChatAppProfile(
	ctx context.Context, store *appstore.Store, appSetup appstore.ProjectAppRecord,
	id uuid.UUID, key, actorID, channelID, messageID string,
) (appstore.AppProfileChoiceRecord, error) {
	for range 3 {
		...
		result, err := store.ChooseAppProfile(ctx, appstore.ChooseAppProfileInput{
	...
	return appstore.AppProfileChoiceRecord{}, storeerr.ErrConflict
}
