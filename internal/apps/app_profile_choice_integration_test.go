...
type choiceTestProvider struct {
	appConsumerProvider
	menus             []appstore.AppProfileChoiceRecord
	notices           []string
	presentationError error
}

func (p *choiceTestProvider) PresentProfileChoice(
	ctx context.Context, _ appstore.ProjectAppRecord,
	choice appstore.AppProfileChoiceRecord, check func(context.Context) error) (string, string, error) {
...
func (p *choiceTestProvider) DismissProfileChoice(_ context.Context, _ appstore.ProjectAppRecord,
	_ appstore.AppProfileChoiceRecord, text string) error {
	p.notices = append(p.notices, text)
	return nil
}

func (p *choiceTestProvider) NotifyInboxFailure(_ context.Context, _ appstore.ProjectAppRecord,
	_ appstore.AppInboxRecord, _ string) error {
...
func newChoiceJourney(t *testing.T, profileCount int) *choiceJourney {
	...
	ctx := t.Context()
	appSetup, err := store.Apps().GetProjectAppByID(ctx, appID)
	...
	for _, name := range []string{"light", "heavy"}[:profileCount] {
		...
		slots = append(slots, appstore.AppLaunchSlot{Key: name, AgentProfileID: &profile.ID})
	}
	f.app, err = store.Apps().UpdateProjectApp(ctx, appID, appstore.SaveProjectAppInput{
...
func (f *choiceJourney) restart() {
	...
	router := NewAppRouter(f.store.Execution(), f.store.Apps())
	launcher := NewChatAppLauncher(f.store.Apps(), f.store.Execution(), providers)
	workflow := NewAppLaunchWorkflow(router, map[appdefinition.Type]AppLauncher{
		appdefinition.SlackThread: launcher.Decide,
	})
	f.consumer = NewAppInboxConsumer(router, f.store.Apps(), &appConsumerUploads{}, providers, nil, workflow)
}

func (f *choiceJourney) receive(key string, event AppEvent) []AppSlotAdmission {
	f.t.Helper()
	_, _, err := f.store.Apps().AcceptAppReceipt(f.t.Context(),
...
func (f *choiceJourney) claim() appstore.AppInboxRecord {
	f.t.Helper()
	receipt, found, err := f.store.Apps().ClaimAppInbox(f.t.Context(),
...
func (f *choiceJourney) choose(choice appstore.AppProfileChoiceRecord, key string) {
	f.t.Helper()
	selected, err := SelectChatAppProfile(f.t.Context(), f.store.Apps(), f.appSetup, choice.ID,
...
func TestChatProfileChoiceLaunchesSelectedProfileAfterRestart(t *testing.T) {
	...
	_, found, err := f.store.Apps().ClaimAppInbox(ctx, appstore.ClaimAppInboxInput{
		ProjectID: f.ids.ProjectID, AppID: f.appSetup.ID, LeaseDuration: time.Minute,
	})
	require.NoError(t, err)
...
func TestChatProfileChoiceConcurrentSiblingDoesNotPostAnotherMenu(t *testing.T) {
	...
	launcher := NewChatAppLauncher(f.store.Apps(), f.store.Execution(), map[string]AppInboxProvider{"slack": p})
	capture := func(key string) appstore.AppInboxRecord {
		_, _, err := f.store.Apps().AcceptAppReceipt(ctx, appstore.VerifiedAppReceipt{
...
func TestUnavailableChatSetupDoesNotDropOtherLaunchesOrSubscriptions(t *testing.T) {
	...
	f.consumer = NewAppInboxConsumer(router, f.store.Apps(), nil,
...
func TestChatProfileChoiceEditedSlotCannotLaunchReplacement(t *testing.T) {
	...
	_, err := f.store.Apps().UpdateProjectApp(t.Context(), f.app.ID, appstore.SaveProjectAppInput{
	...
	require.NoError(t, f.pool.QueryRow(t.Context(),
		`SELECT state,attempt_count FROM app_inbox WHERE project_id=$1 AND events IS NOT NULL`,
...
func TestChatProfileChoiceLateFilesReachEachChosenAgentWithoutSubscription(t *testing.T) {
	...
	require.NoError(t, err)
	for _, app := range []appstore.ProjectAppRecord{f.app, other} {
		f.appSetup = app
...
func TestChatProfileChoiceStaleMenuDoesNotBlockFreshSelection(t *testing.T) {
	...
	settings := f.app.Settings
	settings.Launcher.Slots[1].AgentProfileID = &f.profiles[0].ID
	_, err := f.store.Apps().UpdateProjectApp(ctx, f.app.ID, appstore.SaveProjectAppInput{
		OrgID: f.ids.OrgID, ProjectID: f.ids.ProjectID, AppType: f.app.AppType,
		Name: f.app.Name, Settings: settings,
	})
	require.NoError(t, err)
	_, err = SelectChatAppProfile(ctx, f.store.Apps(), f.appSetup, old.ID,
...
func TestChatProfileChoiceFailedSourceCannotLaunchAgain(t *testing.T) {
	...
	f.choose(f.provider.menus[0], "heavy")
	original := f.claim()
	require.NoError(t, f.store.Apps().WithAppInboxLease(ctx, original.Lease(),
		func(work *appstore.AppInboxLeaseTx) error { return work.Fail(ctx, "before planning") }))
...
func TestChatProfileChoiceSiblingCannotRestartFailedLaunch(t *testing.T) {
	...
	for _, frozen := range []bool{false, true} {
		t.Run(fmt.Sprintf("frozen=%t", frozen), func(t *testing.T) {
			...
			require.ErrorIs(t, err, appstore.ErrAppSelectionReserved)
			waiting, err := inbox.GetAppInbox(ctx, f.ids.ProjectID, late.ID)
			require.NoError(t, err)
			require.Empty(t, waiting.Plan, "the sibling cannot acquire independent launch authority")
			require.NoError(t, inbox.WithAppInboxLease(ctx, selected.Lease(),
