...
func NormalizeDiscordAppEvent(
	appSetup appstore.ProjectAppRecord, raw []byte, channel discord.Channel,
) (AppEvent, bool, error) {
...
func discordInboxMessage(
	appSetup appstore.ProjectAppRecord, raw []byte,
) (discord.MessageEvent, bool, error) {
	if appSetup.Provider != "discord" || appSetup.State != appstore.ProjectAppStateActive ||
	...
	if len(raw) > appstore.AppInboxMaxPayloadBytes || !utf8.Valid(raw) ||
...
func (p *DiscordAppInboxProvider) requestAccess(
	ctx context.Context, appSetup appstore.ProjectAppRecord, authority func(context.Context) error,
...
func (p *DiscordAppInboxProvider) requestClient(
	...
	checkAppSetup := func(ctx context.Context) error {
		...
		if latest.State != appstore.ProjectAppStateActive || latest.Provider != "discord" ||
...
func (p *DiscordAppInboxProvider) Expand(
	ctx context.Context, appSetup appstore.ProjectAppRecord, raw []byte,
...
func (p *DiscordAppInboxProvider) DownloadFile(
	ctx context.Context, appSetup appstore.ProjectAppRecord, raw []byte, fileID string,
) (AppInboxFile, error) {
	ctx, cancel := context.WithTimeout(ctx, discord.OperationTimeout)
	defer cancel()
	message, ok, err := discordInboxMessage(appSetup, raw)
...
func discordConversationName(appSetup appstore.ProjectAppRecord) string {
