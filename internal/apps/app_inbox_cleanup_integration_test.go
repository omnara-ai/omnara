...
VALUES($1,$2,$3,'active',$4,now(),now())`,
	...
	require.NoError(t, err)
	receipt, _, err := inbox.AcceptAppReceipt(ctx, appstore.VerifiedAppReceipt{
		ProjectID: ids.ProjectID, AppID: appID, ReceiptKey: "failure-files", Payload: []byte(`{}`),
	})
	require.NoError(t, err)
	claimed, found, err := inbox.ClaimAppInbox(ctx, appstore.ClaimAppInboxInput{
		ProjectID: ids.ProjectID, AppID: appID, LeaseDuration: time.Minute,
	})
	require.NoError(t, err)
	require.True(t, found)
	require.NoError(t, inbox.WithAppInboxLease(ctx, claimed.Lease(),
	...
	retained, err := inbox.GetAppInbox(ctx, ids.ProjectID, receipt.ID)
	require.NoError(t, err)
	require.Equal(t, appstore.AppInboxFailed, retained.State)
...
func TestAppInboxSkippedUploadsCleanupAndPreparationErrors(t *testing.T) {
	for _, scenario := range []string{
		...
		t.Run(scenario, func(t *testing.T) {
			...
			var launchSlots []appstore.AppLaunchSlot
			for i, key := range []string{"archived", "active"} {
				...
				launchSlots = append(launchSlots, appstore.AppLaunchSlot{Key: key, AgentID: &agents[i]})
			}
			inbox := store.Apps()
			...
			if scenario == "live sibling failure" {
				...
				pending, err := inbox.GetAppInbox(ctx, ids.ProjectID, receipt.ID)
				require.NoError(t, err)
				require.Equal(t, appstore.AppInboxProcessing, pending.State)
			...
			latest, err := inbox.GetAppInbox(ctx, ids.ProjectID, receipt.ID)
			require.NoError(t, err)
			require.Equal(t, appstore.AppInboxCompleted, latest.State)
			if scenario == "cleanup failure leaves orphan" {
				_, found, err := inbox.ClaimAppInbox(ctx, appstore.ClaimAppInboxInput{
					ProjectID: ids.ProjectID, AppID: appID, LeaseDuration: time.Minute,
				})
				require.NoError(t, err)
