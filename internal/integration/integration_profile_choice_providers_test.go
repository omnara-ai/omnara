...
func providerProfileChoice(t *testing.T) integrationstore.IntegrationProfileChoiceRecord {
	t.Helper()
...
func TestIntegrationProfileChoiceDiscordProviderCreatesThreadAndPostsNativeMenu(t *testing.T) {
	t.Parallel()
	...
	channel, message, err := provider.PresentProfileChoice(t.Context(), f.integrationSetup, choice, check)
	require.NoError(t, err)
	...
	require.NoError(t, provider.DismissProfileChoice(t.Context(), f.integrationSetup, choice, "Selected Reviewer."))
	dismissal := <-requests
	...
	f.revoked = true
	f.mu.Unlock()
	_, _, err = provider.PresentProfileChoice(t.Context(), f.integrationSetup, choice, check)
	require.ErrorIs(t, err, storeerr.ErrUnauthorized)
	require.Empty(t, requests)
}

func TestIntegrationProfileChoiceSlackProviderPreservesRetryHint(t *testing.T) {
	t.Parallel()
	for _, operation := range []string{"post", "update"} {
		t.Run(operation, func(t *testing.T) {
			...
			provider := NewSlackIntegrationInboxProvider(slack.OAuthConfig{APIURL: server.URL, HTTPClient: server.Client()},
				access, access, nil)
			choice := providerProfileChoice(t)
			...
			if operation == "post" {
				...
				channel, message, err = provider.PresentProfileChoice(t.Context(), integrationSetup, choice, nil)
				require.Empty(t, channel)
				require.Empty(t, message)
			} else {
				choice.MessageChannelID, choice.MessageID = "C123", "2.3"
				err = provider.DismissProfileChoice(t.Context(), integrationSetup, choice, "Selected Reviewer.")
			}
			var apiErr *slack.APIError
			require.ErrorAs(t, err, &apiErr)
			require.True(t, apiErr.Result.RateLimited)
			require.Equal(t, 15*time.Minute, apiErr.RetryDelay())
			require.Equal(t, 15*time.Minute, integrationInboxRetryDelay(1, err))
			require.EqualValues(t, 1, attempts.Load(), "long provider throttles must return to the durable worker")
		})
	}
}

func TestIntegrationProfileChoiceSlackProviderPostsAndClearsNativeMenu(t *testing.T) {
	t.Parallel()
	integrationSetup := slackInboxTestIntegration()
	access := &slackInboxTestAccess{integrationSetup: integrationSetup, version: uuid.New()}
	...
	var checks atomic.Int32
	check := func(context.Context) error { checks.Add(1); return nil }
	channel, message, err := provider.PresentProfileChoice(t.Context(), integrationSetup, choice, check)
	require.NoError(t, err)
	...
	require.Positive(t, checks.Load())
	choice.MessageChannelID, choice.MessageID = channel, message
	require.NoError(t, provider.DismissProfileChoice(t.Context(), integrationSetup, choice, "Selected Reviewer."))
	dismissal := <-requests
	...
	require.ErrorIs(t, err, storeerr.ErrUnauthorized)
	require.Empty(t, requests, "revoked integration authority must prevent provider I/O")
}
