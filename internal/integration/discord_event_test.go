...
func TestDiscordInboxNormalizesMentionAndThreadReply(t *testing.T) {
	t.Parallel()
	definition, _ := integrationdefinition.Lookup(integrationdefinition.DiscordThread)
...
func TestDiscordInboxChannelFailuresClassifyOnlyInaccessibleTargets(t *testing.T) {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			expansion, err := p.ExpandRouted(t.Context(), f.integrationSetup, discordInboxPayload(t, f.message),
				func(IntegrationEvent) (bool, error) {
					t.Fatal("failed channel lookup reached routing")
					return true, nil
				})
			...
			if test.permanent {
				...
				if errors.Is(err, ErrIntegrationInboundPermanent) || !errors.As(err, &apiErr) || apiErr.StatusCode != test.status {
	...
	t.Run("stored authority remains retryable", func(t *testing.T) {
		...
		if !errors.Is(err, storeerr.ErrUnauthorized) || errors.Is(err, ErrIntegrationInboundPermanent) ||
			len(f.requests) != 0 {
...
func TestDiscordInboxRejectsUnprovenScope(t *testing.T) {
	...
	for _, raw := range []string{"null", "[]", "{}", "{", `{"type":"MESSAGE_CREATE","sequence":-1}`,
		...
		if _, _, err := discordInboxMessage(discordInboxIntegration(), []byte(raw)); err == nil {
	...
	for _, raw := range [][]byte{{0xff}, bytes.Repeat([]byte(" "), integrationstore.IntegrationInboxMaxPayloadBytes+1)} {
		if _, _, err := discordInboxMessage(discordInboxIntegration(), raw); err == nil {
...
func (f *discordInboxFixture) ReadProjectAvailableSecretPayload(
	...
	f.mu.Lock()
	defer f.mu.Unlock()
	if input.OrgID != f.integrationSetup.OrgID || input.ProjectID != f.integrationSetup.ProjectID ||
		input.SecretID != f.integrationSetup.CredentialSecretID || input.Kind != secrets.KindGeneric || f.revoked {
...
func (f *discordInboxFixture) GetProjectAvailableSecret(
	...
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.revoked || orgID != f.integrationSetup.OrgID || projectID != f.integrationSetup.ProjectID ||
		secretID != f.integrationSetup.CredentialSecretID {
...
func newDiscordInboxFixture(t *testing.T) (*discordInboxFixture, *DiscordIntegrationInboxProvider) {
	t.Helper()
	...
	p := NewDiscordIntegrationInboxProvider(discord.Config{HTTPClient: client, APIURL: server.URL + "/api/v10"}, f, f)
	return f, p
}
...
func TestDiscordInboxAccessIsRevalidated(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"integration identity", func(f *discordInboxFixture) { f.integrationSetup.ProviderTenantID = "111" }},
		{"bot identity", func(f *discordInboxFixture) { f.integrationSetup.ProviderAccountRef = "222" }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			f, p := newDiscordInboxFixture(t)
			integrationSetup := f.integrationSetup
			f.afterRead = func() { tc.edit(f) }
			_, err := p.Expand(t.Context(), integrationSetup, discordInboxPayload(t, f.message))
...
func TestDiscordInboxPrepareCreatesOneThreadOnlyAfterAuthority(t *testing.T) {
	...
	f, p := newDiscordInboxFixture(t)
	raw := discordInboxPayload(t, f.message)
	scope := integrationdefinition.DiscordScope{GuildID: "100", ChannelID: "300", ThreadID: "500"}
	if err := p.PrepareConversation(t.Context(), f.integrationSetup, raw,
		integrationdefinition.Scope{Discord: &scope}, nil); err == nil || len(f.requests) != 0 {
	...
	for range 2 {
		if err := p.PrepareConversation(t.Context(), f.integrationSetup, raw,
			integrationdefinition.Scope{Discord: &scope}, authority); err != nil {
			t.Fatal(err)
		}
	}
	...
	wrong := scope
	wrong.ThreadID = "600"
	err := p.PrepareConversation(
		t.Context(),
		f.integrationSetup,
		raw,
		integrationdefinition.Scope{Discord: &wrong},
		authority,
	)
...
func TestDiscordInboxAttachmentsUseExistingFrozenArtifactFlow(t *testing.T) {
	...
	if _, err := consumer.prepareFiles(
		t.Context(),
		p,
		f.integrationSetup,
		raw,
	...
	requestCount = len(f.requests)
	if _, err := p.DownloadFile(t.Context(), f.integrationSetup, raw, "702"); err == nil ||
		len(f.requests) != requestCount {
...
func TestDiscordInboxAttachmentRecoveryRefusesChangedMetadata(t *testing.T) {
	...
	requestCount := len(f.requests)
	if _, err := p.DownloadFile(t.Context(), f.integrationSetup, raw, "../701"); err == nil ||
		len(f.requests) != requestCount {
...
func TestDiscordInboxContextAndMissingContent(t *testing.T) {
	...
	cancel()
	if _, err := p.Expand(ctx, f.integrationSetup, discordInboxPayload(t, f.message)); !errors.Is(err, context.Canceled) ||
		len(f.requests) != 0 {
...
func TestDiscordInboxSettingsEditPreservesSetupAccess(t *testing.T) {
	...
	f.afterRead = func() { f.integrationSetup.UpdatedAt = integration.UpdatedAt.Add(time.Hour) }
	_, check, err := provider.requestAccess(t.Context(), integration, nil)
	if err != nil {
		t.Fatal(err)
	}
