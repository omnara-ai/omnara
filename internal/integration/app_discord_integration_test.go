...
func TestAppDiscordEarlyReplyWaitsForFrozenLaunch(t *testing.T) {
	...
	require.NoError(t, err)
	app, err = store.Integrations().UpdateProjectApp(ctx, appID, integrationstore.SaveProjectAppInput{
		...
		Settings: integrationstore.ProjectAppSettings{Launcher: &integrationstore.AppLauncher{
			Trigger: "mention", Slots: []integrationstore.AppLaunchSlot{{Key: "review", AgentProfileID: &profile.ID}},
		}},
	})
	require.NoError(t, err)
	f, provider := newDiscordInboxFixture(t)
