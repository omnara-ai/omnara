...
func TestGitHubNormalizeIntegrationEvents(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.eventType, func(t *testing.T) {
			...
			event, ok, err := NormalizeGitHubIntegrationEvent(
				integrationSetup,
				githubEventJSON(t, githubEventFixture(tc.eventType)),
			)
			if err != nil || !ok {
...
func TestGitHubSynchronizeQueuesAndUsesTransitionIdentity(t *testing.T) {
	...
	other, ok, err := NormalizeGitHubIntegrationEvent(githubEventIntegration(), githubEventJSON(t, payload))
...
func TestGitHubNormalizeIgnoresNonInputs(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.edit(&payload)
			_, ok, err := NormalizeGitHubIntegrationEvent(githubEventIntegration(), githubEventJSON(t, payload))
			if err != nil || ok {
	...
	for _, raw := range []string{
		...
		integrationSetup := githubEventIntegration()
		integrationSetup.ProviderIdentity = nil
		_, ok, err := NormalizeGitHubIntegrationEvent(integrationSetup, []byte(raw))
		if err != nil || ok {
...
func TestGitHubNormalizeRejectsWrongIdentity(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.edit(&payload, &integrationSetup)
			_, ok, err := NormalizeGitHubIntegrationEvent(integrationSetup, githubEventJSON(t, payload))
			if err == nil || ok {
...
func TestGitHubNormalizationStableOnRenameAndHeaderReplay(t *testing.T) {
	...
	integrationSetup := githubEventIntegration()
	payload := githubEventFixture("pull_request_review_comment")
	before, _, err := NormalizeGitHubIntegrationEvent(integrationSetup, githubEventJSON(t, payload))
	if err != nil {
		t.Fatal(err)
	}
	...
	after, ok, err := NormalizeGitHubIntegrationEvent(integrationSetup, raw)
	...
	other, ok, err := NormalizeGitHubIntegrationEvent(integrationSetup, githubEventJSON(t, payload))
...
func TestGitHubInboxAdapter(t *testing.T) {
	...
	cancel()
	if _, err = provider.Expand(ctx, integrationSetup, []byte(`{}`)); !errors.Is(err, context.Canceled) {
	...
	for _, raw := range [][]byte{nil, []byte("null"), []byte("[]"), []byte("{"), {0xff},
		...
		if _, _, err = NormalizeGitHubIntegrationEvent(integrationSetup, raw); err == nil {
