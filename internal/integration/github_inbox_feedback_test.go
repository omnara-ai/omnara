...
func (f *githubFeedbackFixture) ReadProjectAvailableSecretPayload(_ context.Context,
	...
	f.mu.Lock()
	defer f.mu.Unlock()
	if input.ProjectID != f.integration.ProjectID || input.OrgID != f.integration.OrgID ||
		input.SecretID != f.integration.CredentialSecretID || input.Kind != secrets.KindGitHubAppCredentials || f.revoked {
...
func (f *githubFeedbackFixture) GetProjectAvailableSecret(_ context.Context, orgID, projectID, secretID uuid.UUID) (
	...
	f.mu.Lock()
	defer f.mu.Unlock()
	if projectID != f.integration.ProjectID || orgID != f.integration.OrgID ||
		secretID != f.integration.CredentialSecretID || f.revoked {
...
func newGitHubFeedbackFixture(t *testing.T) (*githubFeedbackFixture, *GitHubIntegrationInboxProvider) {
	t.Helper()
	...
	f.integration.OrgID, f.integration.CredentialSecretID, f.integration.SetupRevision = uuid.New(), uuid.New(), 3
	...
	provider := NewGitHubIntegrationInboxProvider(github.Config{HTTPClient: server.Client(), APIURL: server.URL}, f, f)
	return f, provider
}
	...
	for _, eventType := range []string{"issue_comment", "pull_request_review_comment", "pull_request_review"} {
		for _, source := range []string{"unplanned mention", "planned followup"} {
			t.Run(eventType+"/"+source, func(t *testing.T) {
				...
				receipt := feedbackReceipt(f.integration, githubEventJSON(t, event))
				if source == "planned followup" {
					normalized, ok, err := NormalizeGitHubIntegrationEvent(f.integration, receipt.Payload)
					require.NoError(t, err)
					require.True(t, ok)
...
func TestGitHubNotifyInboxFailureSkipsUnplannedHumanComments(t *testing.T) {
	...
	for _, eventType := range []string{"issue_comment", "pull_request_review_comment", "pull_request_review"} {
		t.Run(eventType, func(t *testing.T) {
			...
			receipt := feedbackReceipt(integration, githubEventJSON(t, event))
...
func TestGitHubNotifyInboxFailureSkipsAutomaticAndBotEvents(t *testing.T) {
	...
	for _, action := range []string{"opened", "synchronize"} {
		...
		receipt := feedbackReceipt(integration, githubEventJSON(t, event))
		require.NoError(t, p.NotifyInboxFailure(t.Context(), integration, receipt, inboxFailureMessage))
		normalized, ok, err := NormalizeGitHubIntegrationEvent(integration, receipt.Payload)
		require.NoError(t, err)
		require.True(t, ok)
		receipt.Plan = feedbackPlan(t, normalized.Event.Scope)
		require.NoError(t, p.NotifyInboxFailure(t.Context(), integration, receipt, inboxFailureMessage))
	...
	require.NoError(t,
		p.NotifyInboxFailure(
			t.Context(),
			integration,
			feedbackReceipt(integration, githubEventJSON(t, event)),
			inboxFailureMessage,
		))
}
...
func TestGitHubNotifyInboxFailureAuthorityAndScope(t *testing.T) {
	for _, change := range []string{"revision", "credential version", "revoked", "wrong App", "wrong repository"} {
		t.Run(change, func(t *testing.T) {
			...
			err := p.NotifyInboxFailure(t.Context(), integration,
				feedbackReceipt(integration, githubEventJSON(t, githubEventFixture("issue_comment"))), inboxFailureMessage)
			require.Error(t, err)
			f.mu.Lock()
			defer f.mu.Unlock()
...
func TestGitHubNotifyInboxFailureDoesNotRetryUnknownSend(t *testing.T) {
	...
	err := p.NotifyInboxFailure(t.Context(), f.integration,
		feedbackReceipt(f.integration, githubEventJSON(t, githubEventFixture("issue_comment"))), inboxFailureMessage)
