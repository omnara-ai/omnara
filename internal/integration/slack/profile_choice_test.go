...
func profileChoiceTestID(t *testing.T) string {
	t.Helper()
	id, err := publicid.Encode(publicid.KindAppProfileChoice, uuid.New())
...
func TestProfileChoicePromptRejectsInvalidMenu(t *testing.T) {
	...
	for _, scenario := range []string{
		...
		t.Run(scenario, func(t *testing.T) {
			...
			case "wrong identity":
				id = strings.Replace(id, "apc_", "aint_", 1)
...
func TestProfileChoiceFromActionsRejectsMalformedSelections(t *testing.T) {
	...
	for _, scenario := range []string{
		...
		t.Run(scenario, func(t *testing.T) {
			...
			case "nil identity":
				envelope.Actions[0].ActionID = ProfileChoiceActionPrefix + "apc_" + strings.Repeat("a", 26)
			}
