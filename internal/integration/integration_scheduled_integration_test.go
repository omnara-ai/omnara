...
type scheduledJourney struct {
	...
	handler       *ThreadIntegrationScheduledHandler
}

func newScheduledJourney(t *testing.T) *scheduledJourney {
	t.Helper()
	return newScheduledProviderJourney(t, integrationdefinition.ProviderSlack)
}
...
func newScheduledProviderJourney(t *testing.T, providerName string) *scheduledJourney {
	...
	if providerName == integrationdefinition.ProviderDiscord {
	...
	pool, store, ids, integrationID := integrationProviderFixture(t, providerName, tenant, account)
	...
	trigger, err := store.Execution().CreateCronTrigger(t.Context(), executionstore.CreateCronTriggerInput{
		...
		Target: executionstore.CronTriggerTarget{
			Kind: executionstore.CronTriggerTargetIntegration, ID: integrationID, Settings: settings,
		},
	})
	require.NoError(t, err)
	provider := &scheduledTestProvider{root: root}
	router := NewIntegrationRouter(store.Execution(), store.Integrations())
	handler := NewThreadIntegrationScheduledHandler(router, store.Integrations(), provider)
	...
	return &scheduledJourney{
		t: t, pool: pool, store: store, ids: ids, integrationID: integrationID,
...
func (f *scheduledJourney) fire() integrationstore.IntegrationInboxRecord {
	...
	f.firings++
	queued, err := f.store.Execution().CreateCronTriggerIntegrationEvent(f.t.Context(), claims.Claimed[0])
	require.NoError(f.t, err)
...
func TestScheduledLaunchHasConversationContextWithoutMentionLauncher(t *testing.T) {
	...
	context, found, err := f.store.Integrations().GetAgentIntegrationConversation(
		t.Context(), f.ids.ProjectID, launched.Agent.ID, f.integrationID,
	)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, "C123:100.1", context.Ref)
	require.Contains(t, raw, "int__chat__post_message")
	subscriptions, err := f.store.Integrations().ListIntegrationSubscriptions(
		t.Context(), integrationstore.ListIntegrationSubscriptionsInput{
...
func TestScheduledLaunchRetriesFrozenPlanAndBlocksEarlyFollowup(t *testing.T) {
	...
	follow, _, err := f.store.Integrations().AcceptIntegrationReceipt(
		...
		integrationstore.VerifiedIntegrationReceipt{
			ProjectID: f.ids.ProjectID, IntegrationID: f.integrationID, ReceiptKey: "early-reply", Payload: []byte(`{}`),
		},
	)
	require.NoError(t, err)
	claimed := f.claim()
