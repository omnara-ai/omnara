...
func TestDiscordScheduledRootAndThreadRecovery(t *testing.T) {
	...
	launch := integrationdefinition.ScheduledThreadLaunch{ChannelID: "300", OpeningMessage: "Daily update"}
	check := func(context.Context) error { return nil }
	root, err := provider.PublishScheduledRoot(t.Context(), f.integrationSetup, launch, receipt, check)
	...
	require.NoError(t, provider.EnsureScheduledThread(t.Context(), f.integrationSetup, root, check))
	require.NoError(t, provider.EnsureScheduledThread(t.Context(), f.integrationSetup, root, check))
...
func TestDiscordScheduledUnknownPublicationIsTerminal(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.Equal(t, integrationdefinition.Scope{}, root)
