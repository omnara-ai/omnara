...
[integration contribution guide](../CONTRIBUTING.md). Setup instructions belong in the
...
and check them again in READY. Guild ID belongs to a destination, not application identity.
...
under the integration runtime lease. This also covers READY/RESUMED. A callback failure
...
Independent saved integrations may share a physical bot but keep separate leases and
...
Callback owner IDs are untrusted routing hints. The captured integration's public key must
...
setup. Independent integrations sharing a bot cannot borrow one another's callback authority.
