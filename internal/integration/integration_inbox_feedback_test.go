...
func feedbackReceipt(
	...
	return integrationstore.IntegrationInboxRecord{
		...
		Source:        integrationstore.IntegrationInboxSourceProvider,
		Payload:       payload,
		LastError:     "private failure details",
	}
}
...
func TestSlackNotifyInboxFailureDestination(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			receipt := feedbackReceipt(integration, slackInboxTestPayload(t, event))
			...
			p := NewSlackIntegrationInboxProvider(slack.OAuthConfig{APIURL: server.URL, HTTPClient: server.Client()},
				access, access, nil)
			require.NoError(t, p.NotifyInboxFailure(t.Context(), integration, receipt, inboxFailureMessage))
...
func TestSlackNotifyInboxFailureRechecksAuthority(t *testing.T) {
	for _, change := range []string{"revision", "credential version", "revoked"} {
		t.Run(change, func(t *testing.T) {
			...
			p := NewSlackIntegrationInboxProvider(slack.OAuthConfig{APIURL: server.URL, HTTPClient: server.Client()},
				access, access, nil)
			receipt := feedbackReceipt(integration, slackInboxTestPayload(t, slack.Event{
				Type: "app_mention", User: "U123", Channel: "C123", TS: "1.2",
			}))
			require.ErrorIs(
				t,
				p.NotifyInboxFailure(t.Context(), integration, receipt, inboxFailureMessage),
...
func TestSlackNotifyInboxFailureMentionSiblings(t *testing.T) {
	for _, thread := range []string{"", "1.1"} {
		for _, planned := range []bool{false, true} {
			...
			t.Cleanup(server.Close)
			p := NewSlackIntegrationInboxProvider(
			...
			for _, kind := range []string{"message", "app_mention"} {
				...
				if planned {
					receipt.Plan = feedbackPlan(t, integrationdefinition.Scope{
						Slack: &integrationdefinition.SlackScope{ChannelID: "C123", ThreadTS: "1.1"},
					})
				}
				require.NoError(t, p.NotifyInboxFailure(t.Context(), integration, receipt, inboxFailureMessage))
			}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.selected {
				...
				receipt.Plan = feedbackPlan(t, integrationdefinition.Scope{Discord: &integrationdefinition.DiscordScope{
			...
			require.NoError(t, p.NotifyInboxFailure(t.Context(), f.integrationSetup, receipt, inboxFailureMessage))
			f.mu.Lock()
			defer f.mu.Unlock()
...
func TestDiscordNotifyInboxFailureRechecksAuthority(t *testing.T) {
	for _, change := range []string{"revision", "credential version", "revoked", "wrong guild"} {
		t.Run(change, func(t *testing.T) {
			f, p := newDiscordInboxFixture(t)
			integration := f.integrationSetup
			...
			err := p.NotifyInboxFailure(
				t.Context(), integration, feedbackReceipt(integration, discordInboxPayload(t, f.message)), inboxFailureMessage)
			require.Error(t, err)
			f.mu.Lock()
			defer f.mu.Unlock()
...
func TestInboxFailureSkipsUnplannedOrdinaryHumanMessages(t *testing.T) {
	...
	for _, thread := range []string{"", "1.1"} {
		receipt := feedbackReceipt(slackIntegration, slackInboxTestPayload(t, slack.Event{
			Type: "message", User: "U123", Channel: "C123", TS: "1.2", ThreadTS: thread, Text: "ordinary conversation",
		}))
		require.NoError(t, slackProvider.NotifyInboxFailure(t.Context(), slackIntegration, receipt, inboxFailureMessage))
	}
	for _, channel := range []string{"300", "400"} {
		message := discordInboxMessageFixture()
		message.ChannelID, message.Mentions = channel, nil
		receipt := feedbackReceipt(discordIntegration, discordInboxPayload(t, message))
		require.NoError(t, discordProvider.NotifyInboxFailure(t.Context(), discordIntegration, receipt, inboxFailureMessage))
	}
}
...
func TestInboxFailureSkipsUnknownScheduledOpeningsAndAutomatedMessages(t *testing.T) {
	...
	for _, plan := range []json.RawMessage{nil, json.RawMessage(`{}`)} {
		...
		require.NoError(t, slackProvider.NotifyInboxFailure(t.Context(), slackIntegration, slackReceipt, inboxFailureMessage))
		require.NoError(
			t,
			discordProvider.NotifyInboxFailure(t.Context(), discordIntegration, discordReceipt, inboxFailureMessage),
		)
	}
	unknownSlack, unknownDiscord := feedbackReceipt(slackIntegration, nil), feedbackReceipt(discordIntegration, nil)
	unknownSlack.Source, unknownDiscord.Source = integrationstore.IntegrationInboxSourceScheduled,
		integrationstore.IntegrationInboxSourceScheduled
	unknownSlack.Plan = feedbackPlan(
		t,
		integrationdefinition.Scope{Slack: &integrationdefinition.SlackScope{ChannelID: "C123"}},
	)
	unknownDiscord.Plan = feedbackPlan(t, integrationdefinition.Scope{Discord: &integrationdefinition.DiscordScope{
		GuildID: "100", ChannelID: "300",
	}})
	require.NoError(t, slackProvider.NotifyInboxFailure(t.Context(), slackIntegration, unknownSlack, inboxFailureMessage))
	require.NoError(
		t,
		discordProvider.NotifyInboxFailure(t.Context(), discordIntegration, unknownDiscord, inboxFailureMessage),
	)
	slackReceipt := feedbackReceipt(slackIntegration, slackInboxTestPayload(t, slack.Event{
		Type: "message", User: "UBOT", BotID: "B123", Channel: "C123", TS: "1.2",
	}))
	require.NoError(t, slackProvider.NotifyInboxFailure(t.Context(), slackIntegration, slackReceipt, inboxFailureMessage))
	message := discordInboxMessageFixture()
	message.Author.Bot = true
	require.NoError(t, discordProvider.NotifyInboxFailure(t.Context(), discordIntegration,
		feedbackReceipt(discordIntegration, discordInboxPayload(t, message)), inboxFailureMessage))
	slackReceipt.IntegrationID = uuid.New()
	require.ErrorIs(
		t,
