...
func (s *scheduledPlanResponseFailure) WithIntegrationInboxLease(
	...
	var froze bool
	err := s.IntegrationRoutingStore.WithIntegrationInboxLease(
...
func (s *scheduledPlanResponseFailure) GetIntegrationInbox(
	...
	return s.IntegrationRoutingStore.GetIntegrationInbox(ctx, projectID, receiptID)
}
...
func TestScheduledLaunchReusesCommittedPlanAfterLostResponse(t *testing.T) {
	for _, failRead := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			store := &scheduledPlanResponseFailure{IntegrationRoutingStore: f.store.Integrations()}
			if failRead {
...
func newScheduledDiscordJourney(t *testing.T) (*scheduledJourney, *discordInboxFixture) {
	t.Helper()
	f := newScheduledProviderJourney(t, integrationdefinition.ProviderDiscord)
	remote, provider := newDiscordInboxFixture(t)
	var err error
	remote.integrationSetup, err = f.store.Integrations().GetProjectIntegrationByID(t.Context(), f.integrationID)
	require.NoError(t, err)
	remote.identity.ApplicationID = remote.integrationSetup.ProviderTenantID
