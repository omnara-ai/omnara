...
func TestScheduledLaunchRejectsDeletedProfileBeforePublication(t *testing.T) {
	...
	require.NoError(t, err, "deleting an integration-owned profile input must not delete the schedule")
}
...
func TestScheduledLaunchRejectsForeignProjectProfileBeforePublication(t *testing.T) {
	for _, provider := range []string{integrationdefinition.ProviderSlack, integrationdefinition.ProviderDiscord} {
		t.Run(provider, func(t *testing.T) {
			...
			require.NoError(t, err)
			var settings integrationdefinition.ThreadScheduleSettings
			...
			require.NoError(t, err, "the integration settings schema permits a syntactically valid profile ID")
			receipt := f.fire()
			worker := NewIntegrationInboxWorker(f.store.Integrations(), f.consumer, IntegrationInboxWorkerOptions{})
