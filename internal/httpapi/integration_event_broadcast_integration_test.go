...
func TestSlackEventsThreadBroadcastContinuesAssignedThreadOnce(t *testing.T) {
	...
	_, found, err := f.Project.Store.Execution().GetIntegrationTargetInputByIdempotency(ctx,
		executionstore.GetIntegrationTargetInputByIdempotencyInput{
			IntegrationID: f.Install.ID, IntegrationTargetID: target.ID, IdempotencyKey: "slack:message:T123:C123:333.444",
		})
	require.NoError(t, err)
	...
	require.NoError(t, pool.QueryRow(ctx, `SELECT
		...
		(SELECT count(*) FROM integration_inbox WHERE project_id=$1)`, f.Project.ProjectUUID).
