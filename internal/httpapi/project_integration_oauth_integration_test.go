...
func newProjectSlackOAuthFixture(
	...
	f.integration = createSetupHTTPIntegration(t, f.handler, f.project, "slack", integrationdefinition.SlackThread)
	return f
}
...
func (f *projectSlackOAuthFixture) start(
	...
	require.NotContains(t, projectIntegrationHTTPJSON(t, setup), "client-secret")
	require.NotContains(t, projectIntegrationHTTPJSON(t, setup), "signing-secret")
	...
	require.NoError(t, err)
	require.Equal(t, f.integration.ID, state.IntegrationID)
	require.Equal(t, f.integration.SetupRevision, state.SetupRevision)
	require.EqualValues(t, state.SetupRevision, setup["setup_revision"])
	require.Equal(t, testPublicID(t, publicid.KindProjectIntegration, f.integration.ID), setup["integration_id"])
...
func projectSlackSetupRequest(
	...
	t.Helper()
	path := project.ProjectPath + "/integrations/" + integrationRef + "/oauth/setup"
	body := map[string]any{
		"client_id":      "client-123",
		"client_secret":  "client-secret",
		"signing_secret": "signing-secret",
	...
	if manifest {
		path = project.ProjectPath + "/integrations/" + integrationRef + "/slack-setup"
		body = map[string]any{
	...
	body["return_to"] = "/projects/" + project.ProjectID + "/integrations/new/slack?draft=keep#settings"
...
func (f *projectSlackOAuthFixture) complete(
	...
	require.Equal(t, "/projects/"+f.project.ProjectID+"/integrations/new/slack", location.Path)
	require.Equal(t, "settings", location.Fragment)
	id, err := publicid.Decode(publicid.KindProjectIntegration, location.Query().Get("integration_id"))
	require.NoError(t, err)
	require.Equal(t, url.Values{
		"draft": {"keep"}, "integration_oauth": {"success"},
		"integration_id": {location.Query().Get("integration_id")},
	}, location.Query())
	integration, err := f.project.Store.Integrations().
		GetProjectIntegration(t.Context(), f.project.ProjectUUID, id)
	require.NoError(t, err)
	...
	detail := requestJSONWithHeaders(
		...
		f.handler,
		http.MethodGet,
		f.project.ProjectPath+"/integrations/"+location.Query().
			Get("integration_id"),
		"",
		"",
		http.StatusOK,
	...
	require.Equal(
		t,
		testPublicID(t, publicid.KindIntegrationOAuthFlow, integration.LastOAuthFlowID),
...
func (f *projectSlackOAuthFixture) assertOneIntegration(t *testing.T) {
	t.Helper()
	page, err := f.project.Store.Integrations().
		ListProjectIntegrations(t.Context(), integrationstore.ListProjectIntegrationsInput{
			ProjectID: f.project.ProjectUUID, Limit: 100,
		})
	require.NoError(t, err)
	require.Len(t, page.Integrations, 1)
	require.Equal(t, f.integration.ID, page.Integrations[0].ID)
}
...
func TestProjectSlackOAuthSetupWithoutProfiles(t *testing.T) {
	...
	for _, manifest := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, integrationstore.ProjectIntegrationStateActive, first.State)
			if manifest {
...
func TestProjectSlackOAuthStaleDeletionAndNameReuse(t *testing.T) {
	...
	require.NoError(
		...
		f.project.Store.Integrations().
			DeleteProjectIntegration(t.Context(), f.project.OrgUUID, f.project.ProjectUUID, first.ID),
	)
	replacement := createSetupHTTPIntegration(t, f.handler, f.project, "slack", integrationdefinition.SlackThread)
	...
	require.Equal(t, integrationstore.ProjectIntegrationStateDisconnected, replacement.State)
}
...
func TestProjectSlackOAuthDisconnectInvalidatesPendingSetup(t *testing.T) {
	...
	for _, active := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			path := strings.TrimSuffix(integrationSetupPath(t, f.project, f.integration), "/setup") + "/disconnect"
...
func TestProjectSlackOAuthCommitRaceCleansCredential(t *testing.T) {
	...
	for _, change := range []string{"disconnect", "delete"} {
		t.Run(change, func(t *testing.T) {
			...
			mutate = func() {
				if change == "delete" {
					assert.NoError(
						t,
						f.project.Store.Integrations().
							DeleteProjectIntegration(t.Context(), f.project.OrgUUID, f.project.ProjectUUID, f.integration.ID),
					)
				} else {
			...
			if change == "delete" {
				f.assertFailure(t, rec, "integration_deleted")
			} else {
				f.assertFailure(t, rec, "integration_setup_changed")
				current, err := f.project.Store.Integrations().
					GetProjectIntegration(t.Context(), f.project.ProjectUUID, f.integration.ID)
				require.NoError(t, err)
				require.Equal(t, integrationstore.ProjectIntegrationStateDisconnected, current.State)
...
func TestProjectSlackOAuthIndependentIntegrationsAndReconnect(t *testing.T) {
	t.Parallel()
	f := newProjectSlackOAuthFixture(t, nil)
	token, _ := f.start(t, false)
	first := f.complete(t, token, "first")
	other := bootstrapPublicHTTPProject(t, f.handler, "other-slack")
	otherIntegration := createSetupHTTPIntegration(t, f.handler, other, "slack", integrationdefinition.SlackThread)
	otherRef := testPublicID(t, publicid.KindProjectIntegration, otherIntegration.ID)
	...
	require.NoError(t, err)
	require.Equal(t, otherRef, location.Query().Get("integration_id"))
	otherIntegration, err = other.Store.Integrations().
		GetProjectIntegration(t.Context(), other.ProjectUUID, otherIntegration.ID)
	require.NoError(t, err)
	require.Equal(t, first.ProviderTenantID, otherIntegration.ProviderTenantID)
	require.Equal(t, first.ProviderAccountRef, otherIntegration.ProviderAccountRef)
	require.NotEqual(t, first.CredentialSecretID, otherIntegration.CredentialSecretID)
	f.assertFailure(t, projectSlackCallback(f.handler, token, "replay", f.project.AdminSession),
		"integration_setup_changed")
	newToken, _ := f.start(t, false)
	...
	unchanged, err := other.Store.Integrations().
		GetProjectIntegration(t.Context(), other.ProjectUUID, otherIntegration.ID)
	require.NoError(t, err)
	require.Equal(t, otherIntegration, unchanged)
}
	...
	for range 2 {
		...
		if outcome := location.Query().Get("integration_oauth_error"); outcome != "" {
			...
			require.Equal(
				t,
				testPublicID(t, publicid.KindProjectIntegration, f.integration.ID),
				location.Query().Get("integration_id"),
			)
			outcomes = append(outcomes, location.Query().Get("integration_oauth"))
		}
	}
	require.ElementsMatch(t, []string{"success", "integration_setup_changed"}, outcomes)
	require.Equal(
		t,
		1,
		countSlackOAuthCredentialSecrets(t, t.Context(), f.project.Store, f.project),
