...
func TestSlackDisconnectedEventsVerifyRetainedCredentialsAndReconnect(t *testing.T) {
	...
	apps := f.Project.Store.Apps()
	applied, err := apps.DisconnectProjectApp(ctx, appstore.DisconnectProjectAppInput{
	...
	request := func(body, signingSecret string, status int) map[string]any {
		t.Helper()
		return requestJSONWithHeaders(t, f.Handler, http.MethodPost, appEventsPath, body, "", status,
	...
	require.NoError(t, err)
	setup := appstore.ConfigureProjectAppInput{
	...
	var captured []byte
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT payload FROM app_inbox WHERE app_id=$1`, f.Install.ID).Scan(&captured))
...
func TestSlackDisconnectedSiblingCannotAuthorizeIntakeOrPoisonActiveApp(t *testing.T) {
	...
	apps := f.Project.Store.Apps()
	for _, app := range []appstore.ProjectAppRecord{different, same} {
		applied, err := apps.DisconnectProjectApp(ctx, appstore.DisconnectProjectAppInput{
	...
	assertReceipts := func(activeCount int) {
		t.Helper()
		for _, app := range []appstore.ProjectAppRecord{f.Install, different, same} {
			var count int
			require.NoError(t, pool.QueryRow(ctx,
	...
	r := httptest.NewRequest(http.MethodPost, appEventsPath, strings.NewReader(body))
	...
	response := performRequest(http.HandlerFunc(server.appEventsRoute), r)
