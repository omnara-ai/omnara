...
func TestIntegrationSetupNameConflictAndMalformedID(t *testing.T) {
	t.Parallel()
	...
	invalidID := "itg_" + strings.Repeat("a", 26)
	...
	for _, suffix := range []string{"/setup", "/disconnect"} {
		...
		response := requestJSONWithHeaders(t, f.handler, http.MethodPost,
			f.project.ProjectPath+"/integrations/"+invalidID+suffix, body, "", http.StatusBadRequest,
			authHeaders(f.project.AdminToken))
		require.Equal(t, "invalid_request", response["code"])
	}
...
func TestIntegrationOAuthDeletedCallbackUsesOnlyValidatedSealedReturnTo(t *testing.T) {
	t.Parallel()
	f := newProjectSlackOAuthFixture(t, nil)
