...
func TestSlackDisconnectedSiblingCannotAuthorizeIntakeOrPoisonActiveIntegration(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	pool := openIntegrationDB(t, ctx)
	provider := newSlackEventsTestServer(t)
	...
	assertReceipts := func(activeCount int) {
		...
		for _, integration := range []integrationstore.ProjectIntegrationRecord{f.Install, different, same} {
			...
			want := 0
			if integration.ID == f.Install.ID {
				want = activeCount
			}
			require.Equal(t, want, count, "each integration must verify its own credentials and be active before admission")
		}
	}
	request(disconnectedSlackEvent, "wrong-signing-secret", http.StatusUnauthorized)
	...
	assertReceipts(2)
	require.Contains(t, logs.String(), "provider event integration failed")
	require.Contains(t, logs.String(), different.ID.String())
	require.Contains(t, logs.String(), `"integration_state":"disconnected"`)
