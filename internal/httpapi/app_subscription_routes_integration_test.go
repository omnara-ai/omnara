...
func subscriptionHTTPAgent(t *testing.T, f appLaunchHTTPFixture) string {
	t.Helper()
	launched := requestJSONWithHeaders(t, f.handler, http.MethodPost, f.project.ProjectPath+"/agents",
		projectAppHTTPJSON(t, map[string]any{"config": f.configID, "name": "Subscribed agent"}),
...
func TestAppSubscriptionsHTTPPaginationAndDetachReplay(t *testing.T) {
	t.Parallel()
	f := newAppLaunchHTTPFixture(t, "subscription-pages")
	...
	create := func(channel string) map[string]any {
		...
		return requestJSONWithHeaders(t, f.handler, http.MethodPost, path,
			projectAppHTTPJSON(t, subscriptionHTTPBody(agentID, channel)), "", http.StatusCreated, headers)
	}
	first := create("C100")
	firstID := testutil.RequireType[string](t, first["id"])
	mustPublicHTTPID(t, publicid.KindAppSubscription, firstID)
	require.Equal(t, f.project.ProjectID, first["project_id"])
	require.Equal(t, f.appID, first["app_id"])
	...
	require.NoError(t, err)
	mustPublicHTTPID(t, publicid.KindAppSubscription, decoded.ID)
	...
	requestJSONWithHeaders(t, f.handler, http.MethodDelete, path+"/"+firstID, "", "", http.StatusNoContent, headers)
	requestJSONWithHeaders(t, f.handler, http.MethodPost, f.project.ProjectPath+"/apps/"+f.appID+"/disconnect",
		"", "", http.StatusOK, headers)
...
func TestAppSubscriptionsHTTPPermissionsAndIsolation(t *testing.T) {
	t.Parallel()
	f := newAppLaunchHTTPFixture(t, "subscription-auth")
	agentID := subscriptionHTTPAgent(t, f)
	path := f.project.ProjectPath + "/apps/" + f.appID + "/subscriptions"
	body := projectAppHTTPJSON(t, subscriptionHTTPBody(agentID, "C123"))
	headers := authHeaders(f.project.AdminToken)
	...
	otherApp := createSlackHTTPApp(t, t.Context(), other, "AOTHER", "TOTHER", "Other")
	otherAppID := testPublicID(t, publicid.KindProjectApp, otherApp.ID)
	otherPath := other.ProjectPath + "/apps/" + otherAppID + "/subscriptions"
	for _, prefix := range []string{
		other.ProjectPath + "/apps/" + f.appID, f.project.ProjectPath + "/apps/" + otherAppID,
	} {
	...
	secondApp := createSlackHTTPApp(t, t.Context(), f.project, "ASECOND", "TSECOND", "Second")
	secondID := testPublicID(t, publicid.KindProjectApp, secondApp.ID)
	secondPath := f.project.ProjectPath + "/apps/" + secondID + "/subscriptions"
	...
	page := requestJSONWithHeaders(t, f.handler, http.MethodGet, path, "", "", http.StatusOK, headers)
	require.Equal(t, []any{attached}, page["data"], "foreign app DELETE cannot delete a scoped subscription")
}

func TestAppSubscriptionsHTTPValidation(t *testing.T) {
	t.Parallel()
	f := newAppLaunchHTTPFixture(t, "subscription-invalid")
	agentID := subscriptionHTTPAgent(t, f)
	path := f.project.ProjectPath + "/apps/" + f.appID + "/subscriptions"
	headers := authHeaders(f.project.AdminToken)
	for _, tc := range []struct {
		...
		requestJSONWithHeaders(t, f.handler, http.MethodPost, path,
			projectAppHTTPJSON(t, body), "", http.StatusBadRequest, headers)
	}
	...
	requestJSONWithHeaders(t, f.handler, http.MethodGet,
		f.project.ProjectPath+"/apps/"+agentID+"/subscriptions", "", "", http.StatusBadRequest, headers)
	page := requestJSONWithHeaders(t, f.handler, http.MethodGet, path, "", "", http.StatusOK, headers)
	require.Empty(t, page["data"])
}

func TestAppSubscriptionsHTTPEventSelectionConflict(t *testing.T) {
	t.Parallel()
	...
	config := createPublicHTTPAgentConfig(t, f.handler, f.project, "subscription-config", "json",
		projectAppHTTPJSON(t, projectAppHTTPSource(nil)), f.project.AdminToken, http.StatusCreated)
	launched := requestJSONWithHeaders(t, f.handler, http.MethodPost, f.project.ProjectPath+"/agents",
		projectAppHTTPJSON(t, map[string]any{"config": config["id"]}),
		"subscription-agent", http.StatusCreated, authHeaders(f.project.AdminToken))
	agentID := testutil.RequireType[map[string]any](t, launched["agent"])["id"]
	path := f.project.ProjectPath + "/apps/" + testPublicID(t, publicid.KindProjectApp, f.app.ID) + "/subscriptions"
	body := map[string]any{
	...
	first := requestJSONWithHeaders(t, f.handler, http.MethodPost, path,
		projectAppHTTPJSON(t, body), "", http.StatusCreated, headers)
	require.Equal(t, []any{"commit"}, first["events"])
	var ref string
	require.NoError(t, integrationPoolForHandler(t, f.handler).QueryRow(t.Context(),
		`SELECT scope_ref FROM app_subscriptions WHERE id=$1`,
		mustPublicHTTPID(t, publicid.KindAppSubscription, testutil.RequireType[string](t, first["id"]))).Scan(&ref))
	...
	requestJSONWithHeaders(t, f.handler, http.MethodPost, path,
		projectAppHTTPJSON(t, body), "", http.StatusConflict, headers)
	page := requestJSONWithHeaders(t, f.handler, http.MethodGet, path, "", "", http.StatusOK, headers)
