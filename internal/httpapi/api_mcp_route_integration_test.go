...
func TestAPIMCPRoute(t *testing.T) {
	...
	t.Run("viewer key only discovers read tools", func(t *testing.T) {
		...
		for _, want := range []string{"agents_list", "agents_get", "projects_list", "profiles_list"} {
			if !names[want] {
				t.Errorf("viewer key should see %s, got %v", want, names)
			}
		}
		hidden := []string{"whoami", "orgs_list", "agents_launch", "agents_input", "agents_cancel", "agents_archive"}
		...
		launch, err := session.CallTool(ctx, &mcp.CallToolParams{Name: "agents_launch", Arguments: map[string]any{
			"orgID": project.OrgID, "projectID": project.ProjectID,
		}})
		if err != nil {
			t.Fatalf("call hidden tool: %v", err)
		}
		if !launch.IsError {
