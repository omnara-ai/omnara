...
const appEventsPath = "/api/integrations/slack/events"

func (s *Server) appEventsRoute(w http.ResponseWriter, r *http.Request) {
	ctx, cancel := context.WithTimeout(r.Context(), appIntakeTimeout)
	defer cancel()
	r = r.WithContext(ctx)
	raw, ok := readAppCallbackBody(w, r, slack.EventBodyMaxBytes)
	if !ok {
		return
	}
	...
	response := ""
	var rejected error
	result, err := fanoutApps(ctx, s.store.Apps().ListProjectAppsForProviderEventVerification,
		appstore.AppProviderSlack, envelope.TeamID, envelope.APIAppID,
		func(ctx context.Context, app appstore.ProjectAppRecord) (bool, error) {
			credentials, err := s.appSlackCredentials(ctx, app)
			if err != nil {
				return false, err
			}
...
func (s *Server) acceptSlackEvent(
	ctx context.Context, app appstore.ProjectAppRecord, envelope slack.EventsEnvelope, raw []byte,
) (string, error) {
	...
	if slack.DisabledInstallEvent(identity.BotUserID, envelope.Event) {
		_, err := s.store.Apps().DisconnectProjectApp(ctx, appstore.DisconnectProjectAppInput{
	...
	if update, ok := slack.EventNameUpdate(envelope); ok {
		return "updated", s.applyAppNameUpdate(ctx, app, update)
	}
	...
	_, _, err = s.store.Apps().AcceptAppReceipt(ctx, appstore.VerifiedAppReceipt{
		ProjectID: app.ProjectID, AppID: app.ID,
		ReceiptKey: "slack:" + envelope.EventID, Payload: raw,
	})
	if err == nil {
		logent.AppEvent(ctx, app, "received", envelope.Event.Type)
	}
	return "received", err
}

func (s *Server) applyAppNameUpdate(
	ctx context.Context,
	install appstore.ProjectAppRecord,
	update slack.NameUpdate,
) error {
	if update.ConversationID != "" {
		return s.store.Apps().UpdateAppTargetDisplayNamesByProviderRefPrefix(
			ctx,
			install.ProjectID,
			install.ID,
...
func (s *Server) appSlackCredentials(
	ctx context.Context,
	install appstore.ProjectAppRecord,
...
func writeAppProviderError(w http.ResponseWriter, err error) {
	switch {
