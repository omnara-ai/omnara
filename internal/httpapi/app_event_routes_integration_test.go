...
func TestSlackEventsAppMentionCreatesAppTargetInputAndDedupesMessageEvent(
	t *testing.T,
) {
	t.Parallel()
	ctx := context.Background()
	...
	require.NoError(
		...
		pool.QueryRow(
			ctx,
			`SELECT payload,state FROM app_inbox WHERE project_id=$1 AND receipt_key=$2`,
	...
	appTarget, err := slackJourneyTarget(t, pool, fixture.Project.Store.Apps(),
		ctx,
		fixture.Project.ProjectUUID,
		fixture.Install.ID,
		"C123:111.222",
	...
	if err != nil {
		t.Fatalf("get app target: %v", err)
	}
	if string(appTarget.ProviderMetadata) != "{}" {
		t.Fatalf("app target metadata = %s want {}", appTarget.ProviderMetadata)
	}
	input, found, err := fixture.Project.Store.Execution().GetAppTargetInputByIdempotency(
		ctx,
	...
	if err != nil {
		t.Fatalf("get app target input: %v", err)
	}
	if !found || input.ActorID == uuid.Nil ||
		input.AppTargetID != appTarget.ID {
		t.Fatalf(
	...
	if agent.AppTargetID != appTarget.ID {
		t.Fatalf(
			"app target target = %s want %s",
			agent.AppTargetID,
			appTarget.ID,
		)
	}
	...
	if _, found, err := fixture.Project.Store.Execution().GetAppTargetInputByIdempotency(
		ctx,
		executionstore.GetAppTargetInputByIdempotencyInput{
			AppID:          fixture.Install.ID,
			AppTargetID:    appTarget.ID,
	...
	threadInput, found, err := fixture.Project.Store.Execution().GetAppTargetInputByIdempotency(
		ctx,
		executionstore.GetAppTargetInputByIdempotencyInput{
			AppID:          fixture.Install.ID,
			AppTargetID:    appTarget.ID,
	...
	_, err = slackJourneyTarget(t, pool, fixture.Project.Store.Apps(),
		ctx,
		fixture.Project.ProjectUUID,
		fixture.Install.ID,
		"C123:999.000",
	...
	if !storeerr.IsNotFound(err) {
		t.Fatalf(
			"unmapped thread reply should not create app target, err=%v",
			err,
		)
	}
}
...
func TestSlackEventsReusesStoredDisplayNames(t *testing.T) {
	...
	if appTarget.DisplayName != "general" {
		t.Fatalf(
			"stored conversation display name = %q, want general",
			appTarget.DisplayName,
		)
	}
	input, found, err := fixture.Project.Store.Execution().GetAppTargetInputByIdempotency(
		ctx,
...
func testSlackEventsOpenInteractionContinuesWithNewMessage(t *testing.T, kind string) {
	...
	require.NoError(
		t,
		(apps.InteractionPresenter{Store: fixture.Project.Store, HTTPClient: fixture.Slack.HTTPClient}).Present(
			ctx,
			fixture.Project.ProjectUUID,
			appTarget.AgentID,
			interaction.ID,
...
func TestSlackEventsLifecycleDisablesInstall(t *testing.T) {
	...
	updated, err := fixture.Project.Store.Apps().GetProjectApp(
		ctx,
		fixture.Project.ProjectUUID,
		fixture.Install.ID,
	)
	...
	if updated.State != appstore.ProjectAppStateActive {
	...
	if updated.State != appstore.ProjectAppStateDisconnected {
	...
	if !storeerr.IsNotFound(err) {
		t.Fatalf(
			"disabled runtime event should not create app target, err=%v",
			err,
		)
	}
}
...
func TestSlackEventsAndActionsRouteByProviderIdentity(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get second app target: %v", err)
	}
	_, err = slackJourneyTarget(t, pool, fixture.Project.Store.Apps(),
		ctx,
		fixture.Project.ProjectUUID,
	...
	first, err := fixture.Project.Store.Apps().GetProjectApp(
		ctx,
		fixture.Project.ProjectUUID,
		fixture.Install.ID,
	)
	...
	second, err := fixture.Project.Store.Apps().GetProjectApp(
		ctx,
		fixture.Project.ProjectUUID,
		secondInstall.ID,
	)
	...
	if first.State != appstore.ProjectAppStateActive ||
		second.State != appstore.ProjectAppStateDisconnected {
...
func TestSlackEventsStableCallbackUsesInstallSigningSecret(t *testing.T) {
	...
	if _, err := slackJourneyTarget(t, pool, project.Store.Apps(),
		ctx,
		project.ProjectUUID,
		installB.ID,
		"C222:111.222",
	); err != nil {
		t.Fatalf("get install B app target: %v", err)
	}
	_, err := slackJourneyTarget(t, pool, project.Store.Apps(),
		ctx,
		project.ProjectUUID,
		installA.ID,
		"C222:111.222",
...
func TestSlackEventsIgnoreRemoteAndBotEvents(t *testing.T) {
	...
	if !storeerr.IsNotFound(err) {
		t.Fatalf(
			"remote event should not create app target, err=%v",
			err,
		)
	}
	...
	if !storeerr.IsNotFound(err) {
		t.Fatalf("bot event should not create app target, err=%v", err)
	}
}
...
func TestSlackEventsDMCreatesInputAndTarget(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get dm app target: %v", err)
	}
	if string(appTarget.ProviderMetadata) != "{}" {
		t.Fatalf("dm app target metadata = %s want {}", appTarget.ProviderMetadata)
	}
	agent, err := fixture.Project.Store.Execution().GetAgentInProject(
		ctx,
		fixture.Project.ProjectUUID,
...
func TestSlackEventsDMFileCreatesArtifactInput(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get dm file app target: %v", err)
	}
	input, found, err := fixture.Project.Store.Execution().GetAppTargetInputByIdempotency(
		ctx,
		executionstore.GetAppTargetInputByIdempotencyInput{
...
func TestSlackEventsSkippedFileCreatesInputWithoutArtifact(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get skipped file app target: %v", err)
	}
	input, found, err := fixture.Project.Store.Execution().GetAppTargetInputByIdempotency(
		ctx,
		executionstore.GetAppTargetInputByIdempotencyInput{
...
func TestSlackEventsDelayedFileShareCreatesNeutralArtifactInput(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get delayed file app target: %v", err)
	}
	...
	if err := pool.QueryRow(ctx, `
		...
		WHERE project_id = $1
		  AND agent_id = $2
		  AND app_target_id = $3
	`, fixture.Project.ProjectUUID, appTarget.AgentID, appTarget.ID).Scan(&inputCount); err != nil {
...
func TestSlackEventsFileShareBeforeAppMentionSuppressesMentionDuplicate(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get file-first app target: %v", err)
	}
...
func TestSlackEventsUnmappedRootFileShareIgnored(t *testing.T) {
	...
	if _, err := slackJourneyTarget(t, pool, fixture.Project.Store.Apps(),
		ctx,
		fixture.Project.ProjectUUID,
		fixture.Install.ID,
		"CUNMAPPED:111.222",
...
func TestSlackEventsLaunchesWithoutExplicitAppSendTool(
	t *testing.T,
) {
	t.Parallel()
	ctx := context.Background()
...
FROM app_inbox ORDER BY created_at DESC LIMIT 1
`).Scan(&state, &attempts, &completed); err != nil {
					t.Errorf("read receipt during failure notice: %v", err)
				} else if state != "failed" || attempts != appstore.AppInboxMaxAttempts || !completed {
	...
	assertLaunchFailure := func(eventID, threadTS string) {
		...
		for attempt := 1; attempt <= appstore.AppInboxMaxAttempts; attempt++ {
			if attempt > 1 {
				_, err := pool.Exec(ctx,
					`UPDATE app_inbox SET available_at=now() WHERE project_id=$1 AND receipt_key=$2`,
...
FROM app_inbox WHERE project_id=$1 AND receipt_key=$2
`, fixture.Project.ProjectUUID, "slack:"+eventID).Scan(
		...
		requestJSONWithHeaders(t, fixture.Handler, http.MethodPost, appEventsPath,
		...
		_, targetErr := slackJourneyTarget(t, pool, fixture.Project.Store.Apps(),
...
func TestSlackEventsLauncherIndependentOfDisabledPostTool(
	...
	target, err := slackJourneyTarget(t, pool, fixture.Project.Store.Apps(),
		ctx,
		fixture.Project.ProjectUUID,
		fixture.Install.ID,
		"C123:111.333",
	...
	subscriptions, err := fixture.Project.Store.Apps().ListAppSubscriptions(
		ctx, appstore.ListAppSubscriptionsInput{
...
func TestSlackEventsNameUpdatesRefreshDisplayNames(t *testing.T) {
	...
	if appTarget.DisplayName != "general-renamed" {
		t.Fatalf(
...
func assertAgentInputText(
	...
	if raw, ok := strings.CutPrefix(texts[last], "Incoming app conversation: "); ok {
		...
		require.NoError(t, pool.QueryRow(ctx, `SELECT app.name, target.provider_ref_kind, target.provider_ref
			FROM app_targets target JOIN project_apps app ON app.id=target.app_id
			WHERE target.project_id=$1 AND target.id=$2`, input.ProjectID, input.AppTargetID).
...
func createSlackHTTPInstall(
	...
	app, err := project.Store.Apps().CreateProjectApp(ctx, appstore.SaveProjectAppInput{
		...
		Settings: appstore.ProjectAppSettings{Launcher: &appstore.AppLauncher{
			...
			Slots: []appstore.AppLaunchSlot{{Key: "default", AgentProfileID: &profileID}},
		}},
	})
	require.NoError(t, err)
	credential, err := project.Store.Secrets().GetSecret(ctx, project.OrgUUID, credentialSecret)
	require.NoError(t, err)
	install, err := project.Store.Apps().ConfigureProjectApp(ctx, appstore.ConfigureProjectAppInput{
		OrgID: project.OrgUUID, ProjectID: project.ProjectUUID, AppID: app.ID, ExpectedSetupRevision: app.SetupRevision,
		InstalledByUserID: project.AdminUserUUID, Provider: appstore.AppProviderSlack,
...
func slackJourneyTarget(
	...
	ref string,
) (appstore.AppTargetRecord, error) {
	t.Helper()
	rows, err := pool.Query(
		ctx,
		`SELECT id FROM app_targets WHERE project_id=$1 AND app_id=$2 AND provider_ref=$3 AND deleted_at IS NULL ORDER BY id`,
		projectID,
		appID,
		ref,
	)
	if err != nil {
		return appstore.AppTargetRecord{}, err
	}
	var ids []uuid.UUID
	for rows.Next() {
		var id uuid.UUID
	...
	if len(ids) == 0 {
		return appstore.AppTargetRecord{}, storeerr.ErrNotFound
	}
	if len(ids) != 1 {
		t.Fatalf("fixture expected one live target for %s, got %d; assert explicit app/slot membership", ref, len(ids))
	}
	return apps.GetAppTarget(ctx, projectID, ids[0])
}

func slackJourneyWorker(project publicHTTPProject, config slack.OAuthConfig) *apps.AppInboxWorker {
	router := apps.NewAppRouter(project.Store.Execution(), project.Store.Apps())
	...
	providers := map[string]apps.AppInboxProvider{"slack": provider}
	chatLauncher := apps.NewChatAppLauncher(project.Store.Apps(), project.Store.Execution(), providers)
	launchers := apps.NewAppLaunchWorkflow(router, map[appdefinition.Type]apps.AppLauncher{
	...
	consumer := apps.NewAppInboxConsumer(
		...
		providers,
		apps.InteractionPresenter{Store: project.Store, HTTPClient: config.HTTPClient},
		launchers,
	)
	return apps.NewAppInboxWorker(project.Store.Apps(), consumer, apps.AppInboxWorkerOptions{})
}
...
"event":{"type":"app_mention","user":"U123","text":"<@U_BOT> help","channel":"C123","ts":"111.222","team":"T123"}}`
			...
			if failure == "receipt" {
				_, err := pool.Exec(ctx, fmt.Sprintf(
					`ALTER TABLE app_inbox ADD CONSTRAINT test_app_receipt_failure CHECK (app_id <> '%s')`, other.ID))
				require.NoError(t, err)
				restore = func() {
					_, err := pool.Exec(ctx, `ALTER TABLE app_inbox DROP CONSTRAINT test_app_receipt_failure`)
					require.NoError(t, err)
				}
			} else {
			...
			requestJSONWithHeaders(t, f.Handler, http.MethodPost, appEventsPath, body, "", http.StatusServiceUnavailable,
				unitSlackSignedHeaders(body, "signing-secret"))
			var count int
			require.NoError(t, pool.QueryRow(ctx,
				`SELECT count(*) FROM app_inbox WHERE app_id=$1`, f.Install.ID).Scan(&count))
			require.Equal(t, 1, count, "a failing sibling must not suppress healthy app admission")
			restore()
			for range 2 {
				requestJSONWithHeaders(t, f.Handler, http.MethodPost, appEventsPath, body, "", http.StatusOK,
					unitSlackSignedHeaders(body, "signing-secret"))
			}
			for _, app := range []appstore.ProjectAppRecord{f.Install, other} {
				var payload []byte
				require.NoError(t, pool.QueryRow(ctx,
					`SELECT count(*) FROM app_inbox WHERE project_id=$1 AND app_id=$2`, app.ProjectID, app.ID).Scan(&count))
				require.Equal(t, 1, count, "redelivery deduplicates independently for each app")
				require.NoError(t, pool.QueryRow(ctx,
					`SELECT payload FROM app_inbox WHERE app_id=$1`, app.ID).Scan(&payload))
				require.Equal(t, body, string(payload))
			}
			require.NoError(t, pool.QueryRow(ctx, `SELECT count(*) FROM app_inbox WHERE app_id=$1`, bad.ID).Scan(&count))
