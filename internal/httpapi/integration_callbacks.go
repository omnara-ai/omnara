...
) (integrationstore.ProjectAppRecord, bool) {
	if s.store == nil {
		apierror.Write(w, openapi.ErrorCodeServiceUnavailable, "store unavailable")
		return integrationstore.ProjectAppRecord{}, false
	}
	if appID == "" || workspaceID == "" {
		apierror.Write(w, openapi.ErrorCodeForbidden, "invalid slack callback identity")
		return integrationstore.ProjectAppRecord{}, false
	}
	install, err := s.store.Integrations().GetProjectAppByID(r.Context(), ownerID)
	if err != nil {
		writeIntegrationProviderError(w, err)
		return integrationstore.ProjectAppRecord{}, false
	}
	if install.State != integrationstore.ProjectAppStateActive ||
