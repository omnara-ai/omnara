		...
		t.Run(endpoint, func(t *testing.T) {
			...
			for _, size := range []int{limit + 1, limit + 16*1024, limit} {
				...
				current, found, err := f.project.Store.Execution().GetAgentInteraction(
					t.Context(), f.integration.ProjectID, f.record.AgentID, f.record.ID)
				require.NoError(t, err)
				...
				if endpoint == "slack-events" && size == limit {
					...
					var captured []byte
					require.NoError(t, f.pool.QueryRow(t.Context(),
						`SELECT payload FROM integration_inbox WHERE integration_id=$1`, f.integration.ID).Scan(&captured))
...
func TestSlackSharedBotUninstallVerifiesEachIntegrationAndFencesSetupRevision(t *testing.T) {
	t.Parallel()
...
func TestSlackChannelOnlySubscriptionReceivesRootMentionAndThreadReply(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = f.Project.Store.Integrations().CreateIntegrationSubscription(
