...
type integrationLaunchHTTPFixture struct {
	...
	configID, profileID, integrationID, integrationName string
	launchToken                                         string
}

func newIntegrationLaunchHTTPFixture(t *testing.T, seed string) integrationLaunchHTTPFixture {
	t.Helper()
	...
	profile := createPublicHTTPAgentProfile(t, handler, project, seed, "Ticket support", configID,
		project.AdminToken, http.StatusCreated)
	return integrationLaunchHTTPFixture{
		handler:         handler,
		project:         project,
		configID:        configID,
...
func (f integrationLaunchHTTPFixture) body() map[string]any {
	...
	body := hostedLaunchHTTPCapabilities(f.integrationName, f.integrationID)
	body["config"], body["profile"], body["initial_input"] = f.configID, f.profileID, initial
	return body
}

func (f integrationLaunchHTTPFixture) counts(t *testing.T) [6]int {
	t.Helper()
	var counts [6]int
...
func TestPublicIntegrationLaunchAtomicRollbackAndReplay(t *testing.T) {
	t.Parallel()
	f := newIntegrationLaunchHTTPFixture(t, "integration-launch-atomic")
	ctx := t.Context()
	...
	require.Equal(t, f.integrationID, testutil.RequireType[map[string]any](t, publicTools[toolName])["integration_id"])
	require.Equal(
		t,
		f.integrationID,
		testutil.RequireType[map[string]any](t, publicHandlers[f.integrationName])["integration_id"],
	)
  ...
  FROM agent_inputs input JOIN actors actor ON actor.id=input.actor_id WHERE input.id=$1`, inputID).
	...
	after = f.counts(t)
	require.Zero(t, after[4], "integration deletion retires its subscriptions")
...
func TestPublicIntegrationLaunchAuthorizationAndIntegrationBoundary(t *testing.T) {
	t.Parallel()
	f := newIntegrationLaunchHTTPFixture(t, "integration-launch-auth")
	before := f.counts(t)
...
func TestPublicIntegrationLaunchWithoutProfile(t *testing.T) {
	t.Parallel()
...
func TestPublicIntegrationLaunchRejectsInvalidAttachmentsAndInput(t *testing.T) {
	t.Parallel()
	f := newIntegrationLaunchHTTPFixture(t, "integration-launch-invalid")
	before := f.counts(t)
	for _, tc := range []struct {
		...
		{"unknown-integration", func(body map[string]any) {
			for key, value := range hostedLaunchHTTPCapabilities("missing-integration", f.integrationID) {
				body[key] = value
			}
		}, "invalid"},
		{"unknown-handler-field", func(body map[string]any) {
			body["interaction_handlers"] = map[string]any{
				f.integrationName: map[string]any{"unexpected": true},
			}
		}, "invalid"},
	} {
		body := f.body()
...
func TestPublicIntegrationLaunchInitialInputReplay(t *testing.T) {
	t.Parallel()
	for _, withIntegration := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			f := newIntegrationLaunchHTTPFixture(t, "launch-replay-"+name)
			body := f.body()
			if !withIntegration {
...
func hostedLaunchHTTPCapabilities(integrationName, integrationID string) map[string]any {
...
func TestPublicSubscriptionOnlyLaunchPreservesConfigAndDetachedReplay(t *testing.T) {
	t.Parallel()
	f := newIntegrationLaunchHTTPFixture(t, "subscription-launch")
	body := f.body()
	delete(body, "tools")
	delete(body, "interaction_handlers")
	before := f.counts(t)
