...
func TestProviderIngressBodyLimits(t *testing.T) {
	...
	for _, endpoint := range []string{"slack-events", "slack-actions", "discord-interactions"} {
		t.Run(endpoint, func(t *testing.T) {
			...
			path, contentType, padding := appEventsPath, "application/json", " "
			...
			switch endpoint {
			case "slack-actions":
				path, contentType, padding = appActionsPath, "application/x-www-form-urlencoded", "x"
				...
				base = slackActionFormBody(t, slackActionPayloadInput{
					Install: f.app, AgentID: f.record.AgentID, AppTargetID: destination.AppTargetID,
			...
			for _, size := range []int{limit + 1, limit + 16*1024, limit} {
				...
				if endpoint == "slack-events" && size == limit {
					...
					var captured []byte
					require.NoError(t, f.pool.QueryRow(t.Context(),
						`SELECT payload FROM app_inbox WHERE app_id=$1`, f.app.ID).Scan(&captured))
...
func TestSlackSharedBotUninstallVerifiesEachAppAndFencesSetupRevision(t *testing.T) {
	...
	for _, concurrentSetup := range []bool{false, true} {
		t.Run(fmt.Sprintf("concurrent_setup=%t", concurrentSetup), func(t *testing.T) {
			...
			request := httptest.NewRequest(http.MethodPost, appEventsPath, strings.NewReader(body))
			for key, value := range unitSlackSignedHeaders(body, "signing-secret") {
				request.Header.Set(key, value)
			}
			...
			require.Equal(t, http.StatusOK, response.Code, response.Body.String())
			for _, before := range []appstore.ProjectAppRecord{f.Install, verified, unverified} {
				after, err := f.Project.Store.Apps().GetProjectApp(ctx, before.ProjectID, before.ID)
				require.NoError(t, err)
				if before.ID == unverified.ID {
					require.Equal(t, before, after, "a sibling cannot borrow another app's callback signature")
					continue
				}
				wantState := appstore.ProjectAppStateDisconnected
				if concurrentSetup && before.ID == f.Install.ID {
					wantState = appstore.ProjectAppStateActive
				}
...
func TestSlackChannelOnlySubscriptionReceivesRootMentionAndThreadReply(t *testing.T) {
	...
	app, err := f.Project.Store.Apps().UpdateProjectApp(ctx, f.Install.ID, appstore.SaveProjectAppInput{
	...
	require.NoError(t, err)
	_, err = f.Project.Store.Apps().CreateAppSubscription(ctx, appstore.CreateAppSubscriptionInput{
