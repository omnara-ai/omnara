...
func TestThreadSchedulesPublishAndValidateTheirInputs(t *testing.T) {
	...
	for _, integration := range []Type{SlackThread, DiscordThread} {
		t.Run(string(integration), func(t *testing.T) {
			...
			raw, err := ValidateScheduleSettings(integration, encode())
			require.NoError(t, err)
			...
			launch, err := PrepareThreadSchedule(integration, raw, occurrence)
			require.NoError(t, err)
			...
			for _, name := range []string{"guild_id", "thread_id", "unexpected"} {
				fields[name] = "123"
				_, err := ValidateScheduleSettings(integration, encode())
				require.Error(t, err)
				delete(fields, name)
			}
