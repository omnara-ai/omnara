...
func TestInboxRetentionDrainsFullBatches(t *testing.T) {
	calls := 0
	count, exhausted, err := drainAppInboxCleanup(t.Context(), func(context.Context) (int64, error) {
		calls++
		if calls <= 3 {
			return appInboxCleanupBatch, nil
		}
		return 2, nil
	})
...
func TestInboxRetentionBoundsSlowBatchAndPreservesCommittedCount(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		calls := 0
		start := time.Now()
		count, exhausted, err := drainAppInboxCleanup(t.Context(), func(ctx context.Context) (int64, error) {
			calls++
			if calls == 1 {
				return appInboxCleanupBatch, nil
			}
		...
		require.EqualValues(t, appInboxCleanupBatch, count)
		require.False(t, exhausted, "hard timeout is an error, not normal budget exhaustion")
		require.Equal(t, 2, calls)
		require.Equal(t, appInboxCleanupTimeout, time.Since(start))
...
func TestInboxRetentionStopsOnErrorOrCancellation(t *testing.T) {
	...
	calls := 0
	_, exhausted, err := drainAppInboxCleanup(t.Context(), func(context.Context) (int64, error) {
		calls++
		return 0, failure
	})
	require.False(t, exhausted)
	...
	cancel()
	_, exhausted, err = drainAppInboxCleanup(ctx, func(context.Context) (int64, error) {
...
func TestInboxRetentionSoftBudgetFinishesCurrentBatch(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		count, exhausted, err := drainAppInboxCleanup(t.Context(), func(ctx context.Context) (int64, error) {
			calls++
			time.Sleep(2 * appInboxCleanupBudget) //nolint:omnaralint // Advance synctest's virtual clock.
