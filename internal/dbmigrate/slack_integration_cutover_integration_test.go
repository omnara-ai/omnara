...
func TestSlackIntegrationCutoverPreflight(t *testing.T) {
	ctx := t.Context()
...
func TestSlackIntegrationCutoverPreservesScopedSendingAndHistory(t *testing.T) {
	for _, scenario := range []string{
		...
		t.Run(scenario, func(t *testing.T) {
			...
			integrationID := uuid.MustParse("00000000-0000-4000-8000-000000000001")
			secondIntegrationID := uuid.MustParse("00000000-0000-4000-8000-000000000002")
			...
			if scenario == "custom_collision" {
				compiledObject["tools"] = map[string]any{
					"int__slack__post_message": map[string]any{"type": "custom", "enabled": true},
				}
			}
...
`
			...
			for i, agentID := range agents {
				...
				if scenario == "multiple_integrations" || scenario == "multiple_targets" {
					otherIntegration := secondIntegrationID
			...
			exec(`INSERT INTO integration_targets(id,project_id,agent_id,integration_install_id,target_ref,
				...
				retiredTargetID, ids.ProjectID, agents[0], integrationID)
			history := func() string {
				t.Helper()
				var value string
			...
			case "unsupported_setup":
				exec(`UPDATE integration_installs SET connection_mode='custom' WHERE id=$1`, integrationID)
			}
			...
			case "preflight_invalid_policy", "preflight_unmapped_policy", "preflight_address", "preflight_quota",
				...
				if scenario == "multiple_targets" {
					want = "has several live targets through one Slack integration"
				}
			...
			if scenario == "invalid_policy" || scenario == "unmapped_policy" || scenario == "invalid_address" ||
				...
				if scenario == "unmapped_policy" {
					want = "disabled legacy send policy has no known Slack integration"
				}
				if scenario == "invalid_address" {
					want = "invalid Slack target"
				}
			...
			for i, agentID := range agents {
				...
				require.Equal(t, integrationID, tool.IntegrationID)
				conversation, found, err := conversations.GetAgentIntegrationConversation(
					ctx,
					ids.ProjectID,
					agentID,
					integrationID,
				...
				if scenario == "multiple_integrations" {
					wantIntegrations = 2
					second, found, err := conversations.GetAgentIntegrationConversation(
						ctx,
						ids.ProjectID,
						agentID,
						secondIntegrationID,
			...
			for _, name := range []string{"int__slack__post_message", "int__slack-2__post_message"} {
			...
			if sendingEnabled {
				...
				require.Equal(t, integrationID, launched.Tools["int__slack__post_message"].IntegrationID)
			} else {
				require.Equal(t, historical.Tools["int__slack__post_message"], launched.Tools["int__slack__post_message"])
			}
			...
			if source != nil {
				...
				if sendingEnabled {
					require.NotContains(t, object["tools"], "int__slack__post_message")
				} else {
					require.Contains(t, object["tools"], "int__slack__post_message")
				}
				require.NotContains(t, migratedSource.String, "integration_id")
				if scenario == "yaml" {
			...
			require.NoError(
				...
				db.QueryRowContext(
					ctx,
					`SELECT count(*) FROM agent_inputs input JOIN integration_targets target ON target.id=input.integration_target_id WHERE target.integration_id=$1`,
					integrationID,
				).Scan(
					&inputTargets,
				),
...
func assertSlackCutoverInputGuards(
	t *testing.T,
	db *sql.DB,
	projectID, agentID, otherAgentID, integrationID uuid.UUID,
) {
	t.Helper()
	ctx := t.Context()
...
func assertSlackCutoverConversationState(
	...
	require.NoError(t, db.QueryRowContext(t.Context(),
		`SELECT data,scope_kind,scope_ref,expires_at FROM integration_states
        WHERE project_id=$1 AND integration_id=$2 AND kind='agent_conversation' AND key=$3`,
		projectID, integrationID, agentID.String()).Scan(&data, &scopeKind, &scopeRef, &expiresAt))
