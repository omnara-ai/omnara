...
func TestMachinePoolDeletionCredentialMigrationUpgradesExistingPools(t *testing.T) {
	t.Parallel()
	...
	if err := dbmigrate.ApplyPostgres(ctx, db, productionMigrationsThrough(t, 20)); err != nil {
...
SELECT EXISTS (
    ...
    WHERE table_schema = current_schema()
      AND table_name = 'machine_pools'
      AND column_name = 'deletion_provider_auth_secret_version_id'
)
`).Scan(&columnExists); err != nil {
		t.Fatal(err)
	}
	if columnExists {
		t.Fatal("machine pool deletion credential column exists before migration 21")
	}

	orgID := uuid.New()
	currentSecretID := uuid.New()
	currentVersionID := uuid.New()
	clearedSecretID := uuid.New()
	clearedVersionOneID := uuid.New()
	clearedVersionTwoID := uuid.New()
	currentPoolID := uuid.New()
	clearedPoolID := uuid.New()
...
VALUES ($1, 'Migration Upgrade Org', statement_timestamp(), statement_timestamp())
`, orgID)
	exec("insert current provider secret", `
INSERT INTO secrets(
    id, org_id, management_kind, owner_kind, name, kind, metadata,
    current_version_id, created_at, updated_at
)
VALUES (
    $1, $2, 'tenant', 'org', 'current-provider-secret', 'generic', '{}'::jsonb,
    $3, statement_timestamp(), statement_timestamp()
)
`, currentSecretID, orgID, currentVersionID)
	exec("insert cleared provider secret", `
INSERT INTO secrets(
    id, org_id, management_kind, owner_kind, name, kind, metadata,
    current_version_id, created_at, updated_at
)
VALUES (
    $1, $2, 'tenant', 'org', 'cleared-provider-secret', 'generic', '{}'::jsonb,
    $3, statement_timestamp(), statement_timestamp()
)
`, clearedSecretID, orgID, clearedVersionTwoID)
	insertVersion := func(label string, versionID, secretID uuid.UUID, versionNumber int) {
		t.Helper()
		exec(label, `
INSERT INTO secret_versions(
    id, org_id, secret_id, version_number, payload_keys, encryption_scheme,
    key_id, dek_wrapped_by, encrypted_dek, encrypted_dek_nonce, nonce,
    ciphertext, created_at
)
VALUES (
    $1, $2, $3, $4, ARRAY['value'], 'aes-256-gcm-envelope-v1',
    'test-key', 'local', decode(repeat('01', 48), 'hex'),
    decode(repeat('02', 12), 'hex'), decode(repeat('03', 12), 'hex'),
    decode(repeat('04', 32), 'hex'), statement_timestamp()
)
`, versionID, orgID, secretID, versionNumber)
	}
	insertVersion("insert current provider secret version", currentVersionID, currentSecretID, 1)
	insertVersion("insert cleared provider secret version one", clearedVersionOneID, clearedSecretID, 1)
	insertVersion("insert cleared provider secret version two", clearedVersionTwoID, clearedSecretID, 2)
	exec("clear deleted provider secret current version", `
UPDATE secrets
...
WHERE id = $1
`, clearedSecretID)
	insertPool := func(label string, poolID, secretID uuid.UUID, name string) {
		t.Helper()
		exec(label, `
INSERT INTO machine_pools(
    id, org_id, name, management_kind, provider, default_machine_memory_mb,
    provider_auth_secret_id, max_total_machines, max_total_memory_mb,
    max_machine_memory_mb, deleted_at, created_at, updated_at
)
VALUES (
    $1, $2, $3, 'tenant', 'test', 1024, $4, 1, 1024, 1024,
    statement_timestamp(), statement_timestamp(), statement_timestamp()
)
`, poolID, orgID, name, secretID)
	}
	insertPool("insert pool with current credential", currentPoolID, currentSecretID, "current-credential-pool")
	insertPool("insert pool with cleared credential", clearedPoolID, clearedSecretID, "cleared-credential-pool")
	if err := tx.Commit(); err != nil {
		t.Fatalf("commit pre-migration state: %v", err)
	}

	if err := dbmigrate.ApplyPostgres(ctx, db, os.DirFS("../../migrations")); err != nil {
		t.Fatalf("upgrade through machine pool deletion credential migration: %v", err)
	}

	for name, expected := range map[string]uuid.UUID{
		"current-credential-pool": currentVersionID,
		"cleared-credential-pool": clearedVersionTwoID,
	} {
		var actual uuid.UUID
		if err := db.QueryRowContext(ctx, `
SELECT deletion_provider_auth_secret_version_id
FROM machine_pools
WHERE name = $1
`, name).Scan(&actual); err != nil {
		...
		if actual != expected {
			t.Fatalf("%s deletion credential version = %s, want %s", name, actual, expected)
		}
	}

	if _, err := db.ExecContext(ctx, `
UPDATE machine_pools
SET deletion_provider_auth_secret_version_id = NULL
WHERE id = $1
`, currentPoolID); err == nil {
		t.Fatal("deleted tenant pool retained its secret without a deletion credential version")
	}
	if _, err := db.ExecContext(ctx, `
UPDATE machine_pools
SET deletion_provider_auth_secret_version_id = $2
WHERE id = $1
`, currentPoolID, clearedVersionTwoID); err == nil {
		t.Fatal("machine pool accepted a deletion credential version from another secret")
	}
}
...
func productionMigrationsThrough(t *testing.T, maxVersion int) fstest.MapFS {
	t.Helper()
	...
	for _, entry := range entries {
		...
		migrations[entry.Name()] = &fstest.MapFile{Data: body}
	}
	if len(migrations) != maxVersion {
