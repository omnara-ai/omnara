...
func TestSlackAppCutoverTombstoneNamesAndCredentials(t *testing.T) {
	for _, scenario := range []struct {
		...
		t.Run(scenario.name, func(t *testing.T) {
			...
			for i, appID := range []uuid.UUID{deletedID, liveID} {
				exec(`INSERT INTO integration_installs(id,org_id,project_id,agent_profile_id,installed_by_user_id,
					...
					appID, ids.OrgID, ids.ProjectID, profileID, ids.ProviderAdminUserID,
			...
			for _, expected := range []struct {
				...
				require.NoError(t, db.QueryRowContext(ctx, `SELECT name,state,credential_secret_id::text,deleted_at IS NOT NULL,
					settings->'launcher'->'slots'->0->>'agent_profile_id' FROM project_apps WHERE id=$1`, expected.id).
			...
			require.Equal(t, liveID, targetApp)
			var successor []byte
			require.NoError(
				t,
				db.QueryRowContext(
