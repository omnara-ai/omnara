...
const outputLimitNotice = "[Automatic Omnara harness notice] The preceding response reached its output limit and may be incomplete. Continue from where it stopped without repeating completed content. This notice was generated by the harness, not the user."
...
func CanonicalHistory(bundle Bundle) ([]HistoryEntry, error) {
	...
	for index := range bundle.Messages {
		...
		entries = append(entries, entry)
		if message.StopReason == modelenvelope.StopReasonMaxTokens && len(orderedResults) == 0 {
			content, err := json.Marshal([]modelenvelope.ResponsePart{{
				Type: modelenvelope.ResponsePartTypeText, Text: outputLimitNotice,
			}})
			if err != nil {
				return nil, err
			}
			entries = append(entries, HistoryEntry{
				...
				Message:  Message{Role: modelprotocol.RoleUser, Sequence: message.Sequence, Content: content},
			})
		}
	}
