...
func TestWebE2E(t *testing.T) {
	...
	providerFixture := webE2EVerifiedIntegrationSetupFixture(t, store, orgID, projectID, adminUserID)
	createWebE2EUser(
		t,
		ctx,
		store,
...
func webE2EVerifiedIntegrationSetupFixture(
	t *testing.T,
	store *storage.Store,
	...
	mux.HandleFunc("POST /integrations/{integration_id}/setup", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil || integration.OrgID != orgID {
			http.Error(
				w,
				"browser integration was not persisted in the fixture project",
				http.StatusNotFound,
			)
			return
		}
		...
		if decoder.Decode(&body) != nil ||
			(integration.Provider != "github" && integration.Provider != "discord") ||
			body.ProviderTenantId != "111" ||
			(integration.Provider == "github" && (body.ProviderAccountRef == nil || *body.ProviderAccountRef != "222")) ||
			(integration.Provider == "discord" && body.ProviderAccountRef != nil) {
			http.Error(w, "invalid provider browser fixture request", http.StatusBadRequest)
			return
		}
		if body.ExpectedSetupRevision != integration.SetupRevision {
			http.Error(w, "integration setup changed", http.StatusConflict)
			return
		}
...
func TestWebE2EVerifiedIntegrationSetupFixture(t *testing.T) {
	ctx, cancel := context.WithTimeout(t.Context(), 2*time.Minute)
	defer cancel()
	env := newDaemonOnlyServiceE2EEnvironment(t, ctx, "web-integration-setup-fixture")
	env.startAPI(t, ctx)
	project := env.bootstrapProjectViaAPI(
		t,
		ctx,
		"web-integration-setup-fixture",
		webE2EProviderConfig,
		webE2EModelName,
	)
	fixtureURL := webE2EVerifiedIntegrationSetupFixture(t, storage.NewStore(env.db),
	...
	for _, test := range []struct{ provider, integrationType string }{
		...
		setup := map[string]any{
			"expected_setup_revision": integration["setup_revision"], "credential_secret_id": secret["id"],
