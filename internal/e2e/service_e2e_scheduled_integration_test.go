...
func TestServiceE2EScheduledSlackIntegrationLaunch(t *testing.T) {
	ctx, cancel := context.WithTimeout(t.Context(), 3*time.Minute)
	defer cancel()
	for _, key := range []string{"OPENAI_API_KEY", "ANTHROPIC_API_KEY", "OPENROUTER_API_KEY"} {
		t.Setenv(key, "service-e2e-test-key")
	}
	env := newDaemonOnlyServiceE2EEnvironment(t, ctx, "scheduled-slack-integration")
	env.startAPI(t, ctx)
	project := env.bootstrapProjectViaAPI(t, ctx, "scheduled-slack-integration", "openai-prod", "service-e2e-local")
...
`
	...
	target := map[string]any{"type": "integration", "integration_id": integrationID, "settings": map[string]any{
	...
	trigger := env.requestJSON(t, ctx, http.MethodPost, project.projectPath+"/cron-triggers", map[string]any{
		...
		"target": target,
	}, "scheduled-integration", project.adminToken, http.StatusCreated)
	...
	model := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 1:
			...
			assert.True(t, requestContainsTool(body, "int__chat__post_message"))
			writeOpenAIFunctionCall(w, fail, fmt.Sprintf("resp_post_%d", i), callID,
				"int__chat__post_message", map[string]any{"text": reports[i]})
		case 2:
	...
	for i, dueAt := range dueTimes {
		...
		require.NoError(t, env.db.QueryRow(
			ctx, `SELECT last_integration_receipt_id FROM cron_triggers WHERE id=$1`, triggerID,
		).Scan(&receiptID))
...
WHERE agent.project_id=$1 AND interaction.interaction_kind='question' AND interaction.state='open'`, projectID).
		...
		require.Equal(t, integration.ID, destination.IntegrationID)
...
WHERE agent_id=$1 AND integration_id=$2 AND subscription_type='thread_messages'`, agentID, integration.ID).Scan(&scope))
  ...
  (SELECT count(*) FROM integration_inbox WHERE integration_id=$1 AND state <> 'completed'),
  (SELECT count(*) FROM agent_runtime_locks WHERE agent_id=$2)`, integration.ID, agentID).Scan(&pending, &locks)
...
WHERE input.agent_id=$1 AND input.input_kind='content' AND block.block_kind='text'
  AND coalesce(block.metadata->>'omnara_hidden', 'false') <> 'true'`, agentID).Scan(&inputTexts))
	...
	waitForServiceE2ECondition(t, ctx, func() (bool, string) {
		err := env.db.QueryRow(ctx, `SELECT agent_id FROM integration_subscriptions
WHERE integration_id=$1 AND scope_ref='C123:333.100'`, integration.ID).Scan(&mentionAgent)
		if !errors.Is(err, pgx.ErrNoRows) {
			require.NoError(t, err)
		}
