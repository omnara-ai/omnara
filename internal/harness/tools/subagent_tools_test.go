...
func TestSubagentToolInputValidation(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "read ok", validate: validateReadAgentInput, input: `{"agent":"a"}`},
		{
			...
			input:    `{"agent":"a","after_sequence":5,"limit":10}`,
		},
		{name: "read missing agent", validate: validateReadAgentInput, input: `{}`, wantErr: "agent is required"},
		{name: "read bad limit", validate: validateReadAgentInput, input: `{"agent":"a","limit":0}`, wantErr: "limit"},
