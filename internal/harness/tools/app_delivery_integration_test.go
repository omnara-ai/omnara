...
func TestPostAppRuntimeMessageUsesSelectedHandler(t *testing.T) {
	...
	if err := executor.PostAppRuntimeMessage(
		ctx,
		fixture.turn(),
	...
	require.NoError(t, executor.PostAppRuntimeMessage(ctx, fixture.turn(), "dashboard only"))
	require.Equal(t, 1, postCount)

}

func TestAppQuestionPromptDisabledTargetFallsBackToOmnara(t *testing.T) {
	ctx := context.Background()
	fixture := newIntegrationToolFixture(t, ctx, "question-disabled-target")
	prepareInteractionPromptFixture(t, ctx, fixture)
	if _, err := fixture.Store.Apps().DisconnectProjectApp(
		ctx,
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		t.Errorf("unexpected app provider post to %s", r.URL.Path)
		http.Error(w, "test handler failed", http.StatusInternalServerError)
	}))
	defer server.Close()
...
func TestAppQuestionPromptRetriesShortRateLimit(t *testing.T) {
	ctx := context.Background()
...
func TestAppQuestionPromptUnknownPostUsesReadback(t *testing.T) {
	ctx := context.Background()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		default:
			t.Errorf("unexpected app provider request to %s", r.URL.Path)
			http.Error(w, "test handler failed", http.StatusInternalServerError)
			return
		}
	}))
...
func TestAppQuestionPromptUnknownOutcomeKeepsDashboardOpen(t *testing.T) {
	ctx := context.Background()
...
func TestAppQuestionPromptDeliveryFailureKeepsDashboardOpen(t *testing.T) {
	ctx := context.Background()
...
func TestAppPermissionPromptDisabledTargetFallsBackToOmnara(t *testing.T) {
	ctx := context.Background()
...
func TestAppPermissionPromptDeliveryDoesNotBlockOmnaraPrompt(t *testing.T) {
	ctx := context.Background()
	...
	case <-time.After(2 * time.Second):
		t.Fatal("app permission prompt copy did not start")
	}
	select {
	...
	case <-postFinished:
	case <-time.After(time.Second):
		t.Fatal("app permission prompt copy did not finish")
	}
}

func TestAppExistingPermissionPromptDoesNotRedeliver(t *testing.T) {
	ctx := context.Background()
...
type toolFixtureOptions struct {
	...
	toolContextAddress appstore.ConversationAddress
...
`,
	...
	var install appstore.ProjectAppRecord
	...
	if fixtureOptions.withDiscordApp {
		address = appstore.ConversationAddress{Kind: "channel", Ref: "444"}
		if fixtureOptions.withToolContext {
			address = appstore.ConversationAddress{Kind: "thread", Ref: "444:555"}
		}
	} else if fixtureOptions.withGitHubApp {
		address = appstore.ConversationAddress{Kind: "pull_request", Ref: "123#7"}
	}
	...
	launch, err := store.Execution().LaunchAgent(
		...
		executionstore.LaunchAgentInput{
			...
			InitialInput: &executionstore.LaunchInitialInput{
				ContentBlocks:    json.RawMessage(`[{"type":"text","text":"send an app reply"}]`),
...
func seedToolContext(
	...
	agent executionstore.AgentRecord, app appstore.ProjectAppRecord,
	...
	target, err := store.Apps().EnsureConversationTargetTx(ctx, tx, appstore.EnsureConversationTargetInput{
		ProjectID: agent.ProjectID, AgentID: agent.ID, AppID: app.ID, Address: address,
	})
	require.NoError(t, err)
	err = store.Apps().AssignAgentAppConversationTx(ctx, tx, agent.ProjectID, agent.ID, app.ID, address)
	require.NoError(t, err)
...
func resolveToolsAppName(ctx context.Context, store *storage.Store, name string) (agentconfig.AppResolution, error) {
	result, err := store.Apps().ListProjectApps(ctx, appstore.ListProjectAppsInput{
	...
	for _, app := range result.Apps {
		if app.Name != name || app.State != appstore.ProjectAppStateActive {
			continue
		}
