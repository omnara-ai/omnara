	...
	for _, tt := range []struct{ provider, operation, input, destination string }{
		...
		t.Run(tt.provider, func(t *testing.T) {
			...
			f := newIntegrationToolFixtureWithOptions(t, ctx, "approval-destination", toolFixtureOptions{
				withSlackIntegration: tt.provider == "slack", withDiscordIntegration: tt.provider == "discord",
				withGitHubIntegration: tt.provider == "github", withToolContext: true,
			...
			call := f.recordPendingToolCall(
				...
				ctx,
				"approval",
				toolcatalog.IntegrationToolName("chat", tt.operation),
				tt.input,
				f.Now,
			)
			var schema struct {
			...
			require.NoError(t, json.Unmarshal(f.IntegrationTools[call.Name].InputSchema, &schema))
			var destination map[string]any
			require.NoError(t, json.Unmarshal([]byte(tt.destination), &destination))
			for field := range destination {
				require.NotContains(t, schema.Properties, field, "destinations are assigned by the integration")
			}
...
func TestIntegrationToolApprovalFailsInvalidScopeBeforeProviderIO(t *testing.T) {
	for _, test := range []struct {
		...
		{"disconnected", "slack", "post_message", `{"text":"hello"}`, "integration is unavailable", true, false},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			f := newIntegrationToolFixtureWithOptions(t, ctx, "approval-scope", toolFixtureOptions{
				withSlackIntegration: test.provider == "slack", withDiscordIntegration: test.provider == "discord",
				withGitHubIntegration: test.provider == "github", withToolContext: test.context,
				slackPermission: toolpermission.ModeAlwaysAsk, githubPermission: toolpermission.ModeAlwaysAsk,
			})
...
func TestIntegrationToolApprovalPreservesInfrastructureErrors(t *testing.T) {
	for _, canceled := range []bool{false, true} {
		t.Run(map[bool]string{false: "lost-lock", true: "canceled"}[canceled], func(t *testing.T) {
			...
			f := newIntegrationToolFixtureWithOptions(t, ctx, "approval-infrastructure", toolFixtureOptions{
				withSlackIntegration: true, withToolContext: true, slackPermission: toolpermission.ModeAlwaysAsk,
			})
			call := f.recordPendingToolCall(
				t, ctx, "post", toolcatalog.IntegrationToolName("chat", "post_message"), `{"text":"hello"}`, f.Now,
			)
			turn := f.turn()
			...
			_, err := integrationPermissionChallenge(operationCtx, executor, turn, call, permissionModeContext{})
...
func TestIntegrationToolApprovalDoesNotBypassCurrentConfig(t *testing.T) {
			...
			t.Run(
				...
				func(t *testing.T) {
					...
					f := newIntegrationToolFixtureWithOptions(
						...
						"scope-approval",
						toolFixtureOptions{
							withSlackIntegration: true,
							withToolContext:      true,
					...
					change := func() {
						...
						case "added-handler":
							source.InteractionHandlers = map[string]agentconfig.AgentConfigIntegrationCapabilitySource{
								"chat": {},
							}
...
func changeIntegrationToolConfig(
	t *testing.T,
	ctx context.Context,
...
func integrationToolConfigChangeInput(
	t *testing.T,
	f integrationToolFixture,
	source agentconfig.AgentConfigSource,
	...
	compiled, err := agentconfig.Compile(agentconfig.SourceFormatJSON, raw, agentconfig.CompileOptions{
		...
		ResolveIntegrationName: func(name string) (agentconfig.IntegrationResolution, error) {
			return resolveToolsIntegrationName(t.Context(), f.Store, name)
		},
	})
	require.NoError(t, err)
...
func TestIntegrationToolsUseSavedConversation(t *testing.T) {
		...
		for _, state := range []string{"active", "retired"} {
			t.Run(provider+"/"+state, func(t *testing.T) {
				...
				f := newIntegrationToolFixtureWithOptions(t, ctx, "saved-context", toolFixtureOptions{
					withSlackIntegration: provider == "slack", withDiscordIntegration: provider == "discord", withToolContext: true,
				})
				input := `{"text":"hello"}`
				if provider == "discord" {
					input = `{"content":"hello"}`
				...
				call := f.recordToolCall(t, ctx, "send", toolcatalog.IntegrationToolName("chat", "post_message"), input, f.Now)
...
func TestIntegrationToolMissingContextBeforeProviderIO(t *testing.T) {
	for _, test := range []struct{ provider, operation, input string }{
		...
		t.Run(test.provider+"/"+test.operation, func(t *testing.T) {
			...
			f := newIntegrationToolFixtureWithOptions(t, ctx, "missing-context", toolFixtureOptions{
				withSlackIntegration: test.provider == "slack", withDiscordIntegration: test.provider == "discord",
				withGitHubIntegration: test.provider == "github",
			})
			call := f.recordToolCall(
				t,
				ctx,
				"missing",
				toolcatalog.IntegrationToolName("chat", test.operation),
				test.input,
				f.Now,
			)
			var requests atomic.Int32
...
func TestIntegrationToolRejectsDestinationArgumentsBeforeProviderIO(t *testing.T) {
	for _, test := range []struct{ provider, input string }{
		...
		t.Run(test.provider, func(t *testing.T) {
			...
			f := newIntegrationToolFixtureWithOptions(t, ctx, "invalid-destination", toolFixtureOptions{
				withSlackIntegration: test.provider == "slack", withDiscordIntegration: test.provider == "discord",
				withGitHubIntegration: test.provider == "github", withToolContext: true,
			})
			call := f.recordToolCall(t, ctx, "invalid", toolcatalog.IntegrationToolName("chat", "read"), test.input, f.Now)
			var requests atomic.Int32
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
				requests.Add(1)
				w.WriteHeader(http.StatusForbidden)
...
func integrationToolSubscriptions(
	t *testing.T,
	f integrationToolFixture,
) []integrationstore.IntegrationSubscriptionRecord {
	t.Helper()
	page, err := f.Store.Integrations().ListIntegrationSubscriptions(
		t.Context(),
		integrationstore.ListIntegrationSubscriptionsInput{
			ProjectID: toolsTestProjectID, IntegrationID: f.Install.ID, Limit: 100,
		},
	)
	require.NoError(t, err)
