...
func TestSleepForAppRateLimitRespectsRetryBounds(t *testing.T) {
	t.Run("sleep budget", func(t *testing.T) {
		slept := appMessageSendMaxRateLimitSleep
		retry, err := sleepForAppRateLimit(
			context.Background(),
			time.Nanosecond,
			&slept,
			1,
		)
		if err != nil || retry || slept != appMessageSendMaxRateLimitSleep {
	...
	t.Run("cancellation", func(t *testing.T) {
		...
		cancel()
		var slept time.Duration
		retry, err := sleepForAppRateLimit(ctx, time.Second, &slept, 1)
...
func TestPostAppMessageAddsMarker(t *testing.T) {
	...
	executor := Executor{AppHTTPClient: appProviderTestClient(server)}
	result, err := slack.PostMessage(
		context.Background(),
		executor.AppHTTPClient,
		target,
		agentPublicID,
		"call_test",
		"hello",
	...
	if err != nil {
		t.Fatalf("post app message: %v", err)
	}
	...
	if !ok || metadata["event_type"] != "omnara_integration_message" {
...
func TestReconcileAppMessageFindsMarker(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("reconcile app message: %v", err)
	}
...
func TestReconcileAppMessageClassifiesProviderErrors(t *testing.T) {
...
func TestAppProviderRateLimitResult(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestPostAppMessageTransportFailureIsStructured(t *testing.T) {
	executor := Executor{
		AppHTTPClient: &http.Client{Transport: errorRoundTripper{err: errors.New("network down")}},
	}
	result, err := slack.PostMessage(
		context.Background(),
		executor.AppHTTPClient,
	...
	if err != nil {
		t.Fatalf("post app message transport failure: %v", err)
	}
...
func TestAppInteractionPromptPayloadUsesPlainTextAndSharedAction(t *testing.T) {
	interactionID := integrationToolTestID("prompt-interaction")
	agentID := integrationToolTestID("prompt-agent")
	payload, err := apps.SlackInteractionPromptPayload(
		executionstore.InteractionDestination{
			AppTargetID: integrationToolTestID("prompt-target"),
...
func TestAppPermissionPromptIncludesInputSummary(t *testing.T) {
	payload, err := apps.SlackInteractionPromptPayload(
		executionstore.InteractionDestination{
			AppTargetID: integrationToolTestID("prompt-target"),
			Address:     appstore.ConversationAddress{Kind: "dm", Ref: "D123"},
		},
...
func TestAppQuestionPromptPayloadIncludesQuestionText(t *testing.T) {
...
func appProviderTestClient(server *httptest.Server) *http.Client {
	...
	return &http.Client{
		Transport:     rewriteAppProviderTransport{target: target, base: transport},
...
type rewriteAppProviderTransport struct {
	target *url.URL
	base   http.RoundTripper
}

func (t rewriteAppProviderTransport) RoundTrip(req *http.Request) (*http.Response, error) {
