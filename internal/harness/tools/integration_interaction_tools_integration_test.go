...
func activateInteractionToolHandlers(
	...
	for _, key := range keys {
		if key != f.Install.Name {
			createSlackToolIntegration(t, ctx, f.Store, f.User.ID, key, key)
		}
		source.InteractionHandlers[key] = agentconfig.AgentConfigIntegrationCapabilitySource{}
	}
	changed, err := f.Store.Execution().ChangeAgentConfig(ctx, integrationToolConfigChangeInput(t, f, source))
	require.NoError(t, err)
