...
const listGitHubWebhookCredentialConnections = `-- name: ListGitHubWebhookCredentialConnections :many
...
WHERE connection.provider = 'github'
  AND connection.provider_tenant_id = $1::text
...
type ListGitHubWebhookCredentialConnectionsParams struct {
	AppID    string
	RowLimit int32
}
...
func (q *Queries) ListGitHubWebhookCredentialConnections(ctx context.Context, arg ListGitHubWebhookCredentialConnectionsParams) ([]IntegrationConnection, error) {
	rows, err := q.db.Query(ctx, listGitHubWebhookCredentialConnections, arg.AppID, arg.RowLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
