...
func TestManagedChannelRegistrationRejectsExternalConnection(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	f := newPublicChannelFixture(t, ctx, "managed-rejects-external")
	...
	_, err = f.store.Integrations().GetIntegrationTargetByProviderRef(ctx, testProjectID, f.install.ID, input.ProviderRef)
	require.ErrorIs(t, err, storeerr.ErrNotFound)
