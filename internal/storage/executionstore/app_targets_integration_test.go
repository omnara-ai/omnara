...
func TestProjectAppIdentityRotationAndOAuthReplay(t *testing.T) {
	...
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "install")
	input := slackProjectAppSetupInput(admin.ID, credentialID, "A_PROFILE", "T_SHARED")
	input.OAuthFlowID = appOAuthFlowID(1)
	input = prepareProjectAppSetup(t, ctx, store, input)
	pending, err := store.Apps().GetProjectApp(ctx, input.ProjectID, input.AppID)
	if err != nil || pending.State != appstore.ProjectAppStateDisconnected ||
	...
	if app.ID != pending.ID || app.State != appstore.ProjectAppStateActive ||
	...
	consumed, err := store.Apps().AppOAuthFlowConsumed(ctx, input.OAuthFlowID)
	if err != nil || !consumed {
	...
	fixedInput := slackProjectAppSetupInput(admin.ID, credentialID, "A_FIXED", "T_SHARED")
	...
	for _, tc := range []struct {
		name   string
		mutate func(*appstore.ConfigureProjectAppInput)
	}{
		{"missing tenant", func(v *appstore.ConfigureProjectAppInput) { v.ProviderTenantID = "" }},
		{"missing credential", func(v *appstore.ConfigureProjectAppInput) { v.CredentialSecretID = uuid.Nil }},
		{"non-object identity", func(v *appstore.ConfigureProjectAppInput) { v.ProviderIdentity = json.RawMessage(`[]`) }},
		{"missing OAuth flow", func(v *appstore.ConfigureProjectAppInput) { v.OAuthFlowID = uuid.Nil }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			invalid := prepareProjectAppSetup(t, ctx, store, fixedInput)
			tc.mutate(&invalid)
			if _, err := store.Apps().ConfigureProjectApp(ctx, invalid); !errors.Is(err, storeerr.ErrInvalidRequest) {
	...
	rotated.CredentialSecretID = createAppCredential(t, ctx, store, testProjectID, admin.ID, "rotated")
	...
	rotated.OAuthFlowID = appOAuthFlowID(2)
	updated, err := store.Apps().ConfigureProjectApp(ctx, rotated)
	if err != nil {
		t.Fatalf("rotate app credentials: %v", err)
	}
	if updated.ID != app.ID || updated.CredentialSecretID != rotated.CredentialSecretID ||
		updated.State != appstore.ProjectAppStateActive || updated.ProviderAccountRef != app.ProviderAccountRef ||
	...
	for _, replay := range []appstore.ConfigureProjectAppInput{rotated, input} {
		if _, err := store.Apps().ConfigureProjectApp(ctx, replay); !errors.Is(err, storeerr.ErrConflict) {
	...
	if applied, err := store.Apps().DisconnectProjectApp(ctx, appstore.DisconnectProjectAppInput{
	...
	disconnected, err := store.Apps().GetProjectApp(ctx, updated.ProjectID, updated.ID)
	if err != nil || disconnected.State != appstore.ProjectAppStateDisconnected ||
	...
	if _, err := store.Apps().
		...
		err,
		storeerr.ErrAppOAuthFlowConsumed,
	) {
	...
	if _, err := store.Apps().ConfigureProjectApp(ctx, badCredential); !errors.Is(err, storeerr.ErrNotFound) {
...
func TestProjectAppAuthorizationAndIndependentIdentityAcrossProjects(t *testing.T) {
	...
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "install-scope")
	...
	if _, err := store.Apps().ConfigureProjectApp(
		ctx, unauthorized,
	...
	otherIdentity := slackProjectAppSetupInput(
		admin.ID,
		createAppCredential(t, ctx, store, otherProject.ID, admin.ID, "other-project"),
	...
	if _, err := store.Apps().DisconnectProjectApp(ctx, appstore.DisconnectProjectAppInput{
		ProjectID: first.ProjectID, AppID: first.ID,
	}); err != nil {
		t.Fatal(err)
	}
	unaffected, err := store.Apps().GetProjectApp(ctx, other.ProjectID, other.ID)
	if err != nil || unaffected.State != appstore.ProjectAppStateActive ||
...
func TestAppTargetRechecksAgentAfterArchiveWait(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	pool := openIntegrationDB(t, ctx)
	seedMigratedDB(t, ctx, pool)
	...
	address := appstore.ConversationAddress{Kind: "thread", Ref: "C_ARCHIVE:target"}
	...
	if _, err := dbsqlc.New(blockingTx).LockAgentInProject(
		...
		t.Fatalf("lock app target agent: %v", err)
	}
	...
	targetDone := integrationdb.RunAsyncError(func() error {
		...
		if err := appstore.LockAppsTx(ctx, tx, testProjectID, nil, install.ID); err != nil {
			return err
		}
		if err := appstore.LockConversationTx(ctx, tx, testProjectID, install.ID, address); err != nil {
			return err
		}
		...
		_, err = store.Apps().EnsureConversationTargetTx(ctx, tx, appstore.EnsureConversationTargetInput{
	...
	if err := blockingTx.Commit(ctx); err != nil {
		t.Fatalf("release app target agent blocker: %v", err)
	}
	if err := integrationdb.Await(t, archiveDone, "agent archival"); err != nil {
		t.Fatalf("archive app target agent: %v", err)
	}
	if err := integrationdb.Await(
		t, targetDone, "app target creation",
	...
	if err := pool.QueryRow(
		ctx,
		`SELECT count(*) FROM app_targets
		 WHERE project_id = $1 AND app_id = $2 AND provider_ref = $3`,
		testProjectID,
		install.ID,
		address.Ref,
	).Scan(&targetCount); err != nil {
		t.Fatalf("count app targets after agent archive: %v", err)
	}
	if targetCount != 0 {
		t.Fatalf("app targets after agent archive = %d, want 0", targetCount)
	}
}
...
func TestProjectAppDeletionWaitsForTargetCreation(t *testing.T) {
	...
	install := mustCreateProjectApp(t, ctx, store, installInput)
	lease := prepareAppOrigin(t, ctx, store, agent.ID, install.ID,
		appstore.ConversationAddress{Kind: "thread", Ref: "C_DELETE:" + label})
	...
	deleteDone := integrationdb.RunAsyncError(func() error {
		return store.Apps().DeleteProjectAppOnceForIntegration(ctx, testProjectID, install.ID)
	})
	...
	targetOutcome := integrationdb.Await(t, targetDone, "app target creation")
	if err := integrationdb.Await(t, deleteDone, "project app deletion"); err != nil {
		t.Fatalf("delete project app: %v", err)
	}
	if targetOutcome.Err != nil || !targetOutcome.Value.Created || !targetOutcome.Value.AppTarget.Created {
		t.Fatalf("target creation before deletion = %+v err=%v", targetOutcome.Value, targetOutcome.Err)
	}
	if targetOutcome.Value.AgentInput.AppTargetID != targetOutcome.Value.AppTarget.ID {
...
func TestProjectAppDeletionFreezesTargetAgents(t *testing.T) {
	...
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "install-growth")
	install := mustCreateProjectApp(t, ctx, store, slackProjectAppSetupInput(
		admin.ID, credentialID, "A_GROWTH", "T_GROWTH",
	))
	firstAgent := createAppBoundAgent(t, ctx, store, profile, admin.ID, "first-target-agent")
	firstOrigin, err := admitAppOrigin(
		t,
		ctx,
		store,
		firstAgent.ID,
		install.ID,
		appstore.ConversationAddress{Kind: "thread", Ref: "C_FIRST"},
	)
	first := firstOrigin.AppTarget
	if err != nil {
		t.Fatalf("create first target: %v", err)
	}
	mustCreateAppInput(t, ctx, store, install, first, "U_GROWTH", "Ev-first", "select first target")
	secondAgent := createAppBoundAgent(t, ctx, store, profile, admin.ID, "install-growth-second")

	lateLease := prepareAppOrigin(t, ctx, store, secondAgent.ID, install.ID,
		appstore.ConversationAddress{Kind: "thread", Ref: "C_SECOND"})
	...
	defer cancel()
	otherTargetOrigin, err := admitAppOrigin(
		t,
		otherCtx,
		store,
		secondAgent.ID,
		otherInstall.ID,
		appstore.ConversationAddress{Kind: "thread", Ref: "C_OTHER"},
	)
	otherTarget := otherTargetOrigin.AppTarget
	if err != nil {
		t.Fatalf("create unrelated install target during deletion: %v", err)
	}
	mustCreateAppInput(t, otherCtx, store, otherInstall, otherTarget, "U_GROWTH", "Ev-other", "other install")
	...
	if err := pool.QueryRow(ctx, `SELECT
		(SELECT count(*) FROM app_targets WHERE app_id = $1 AND deleted_at IS NULL),
		(SELECT count(*) FROM agents WHERE app_target_id = $2)`,
		install.ID, first.ID,
	...
	if _, err := store.Apps().GetAppTarget(ctx, testProjectID, otherTarget.ID); err != nil {
...
func TestProjectAppDeletionSerializesWithScopeDeletion(t *testing.T) {
	...
	for _, scope := range []string{"project", "organization"} {
		t.Run(scope, func(t *testing.T) {
			...
			install := mustCreateProjectApp(t, ctx, store, installInput)
			targetOrigin, err := admitAppOrigin(
				t,
				ctx,
				store,
				agent.ID,
				install.ID,
				appstore.ConversationAddress{Kind: "thread", Ref: "C_SCOPE_DELETE:" + scope},
			)
			target := targetOrigin.AppTarget
			if err != nil {
				t.Fatalf("create unbound app target: %v", err)
			}
			var boundTargetID *uuid.UUID
			if err := pool.QueryRow(
				ctx,
				`SELECT app_target_id FROM agents WHERE project_id = $1 AND id = $2`,
				testProjectID,
				agent.ID,
			...
			if boundTargetID != nil {
				t.Fatalf("app target fixture is bound: %s", *boundTargetID)
			}
			...
			installDeleteDone := integrationdb.RunAsyncError(func() error {
				return store.Apps().DeleteProjectAppOnceForIntegration(
					context.Background(),
					testProjectID,
					install.ID,
				)
			...
			if err := pool.QueryRow(
				...
				`SELECT
				   ...
				   (SELECT count(*)::integer FROM app_targets
...
func TestDisconnectProjectAppRequiresCurrentSetupRevision(t *testing.T) {
	...
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "disconnect-revision")
	...
	current, err := store.Apps().GetProjectApp(ctx, app.ProjectID, app.ID)
	if err != nil || current.State != appstore.ProjectAppStateActive ||
	...
	current, err = store.Apps().GetProjectApp(ctx, app.ProjectID, app.ID)
	if err != nil || current.State != appstore.ProjectAppStateDisconnected ||
...
func TestProjectAppSetupRechecksRevisionAfterLifecycleLock(t *testing.T) {
	...
	for _, changeSetup := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "app-update-lock")
			input := prepareProjectAppSetup(t, ctx, store, slackProjectAppSetupInput(
			...
			done := integrationdb.RunAsync(func() (appstore.ProjectAppRecord, error) {
				return store.Apps().ConfigureProjectApp(ctx, input)
			})
			integrationdb.WaitForNamedLockWaiters(t, ctx, pool, "LockProjectAppLifecycleExclusive", 1)
			...
			if result.Value.SetupRevision != app.SetupRevision+1 || result.Value.LastOAuthFlowID != input.OAuthFlowID ||
				result.Value.State != appstore.ProjectAppStateActive || result.Value.UpdatedAt.Before(releaseFloor) {
...
func TestAppTargetProviderRefReusableAfterTargetDeletion(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	pool := openIntegrationDB(t, ctx)
	seedMigratedDB(t, ctx, pool)
	...
	fixedAgent := createAppBoundAgent(t, ctx, store, profile, admin.ID, "target-reuse-agent")
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "target-reuse")
	...
	firstOrigin, err := admitAppOrigin(
		...
		fixedAgent.ID,
		install.ID,
		appstore.ConversationAddress{Kind: "channel", Ref: "C900:reuse"},
	)
	first := firstOrigin.AppTarget
	if err != nil {
	...
	recreatedOrigin, err := admitAppOrigin(
		t,
		ctx,
		store,
		fixedAgent.ID,
	...
	recreated := recreatedOrigin.AppTarget
	if err != nil {
...
func TestAppActorIdentityAcrossAppsAndConcurrency(t *testing.T) {
	...
	agent := createAppBoundAgent(t, ctx, store, profile, admin.ID, "identity-agent")
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "identity")
	...
	for index, testCase := range testCases {
		...
		targetOrigin, err := admitAppOrigin(
			...
			agent.ID,
			install.ID,
			appstore.ConversationAddress{Kind: "dm", Ref: testCase.targetRef},
		)
		target := targetOrigin.AppTarget
		if err != nil {
			t.Fatalf("create identity target %d: %v", index, err)
		}
		input := mustCreateAppInput(
			t,
			ctx,
			store,
			install,
	...
	require.NoError(t, store.Apps().DeleteProjectApp(ctx, testOrgID, testProjectID, appIDs[0]))
...
func TestAppInputDedupeTargetProgressionAndDisconnect(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	pool := openIntegrationDB(t, ctx)
	seedMigratedDB(t, ctx, pool)
	...
	agent := createAppBoundAgent(t, ctx, store, profile, admin.ID, "input-fixed-agent")
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "input")
	installInput := slackProjectAppSetupInput(
		admin.ID,
		credentialID,
		"A_INPUT",
	...
	slot.Input.Origin.Address = appstore.ConversationAddress{Kind: "dm", Ref: "D_FIRST"}
	...
	firstTarget, firstInput := first.AppTarget, first.AgentInput
	...
	if admittedInput.ID != firstInput.ID || admittedInput.AppTargetID != firstTarget.ID {
		t.Fatalf(
			"admitted input = %+v, want id %s with app target %s",
			admittedInput,
			firstInput.ID,
			firstTarget.ID,
		)
	...
	secondTarget, secondInput := second.AppTarget, second.AgentInput
	...
	if secondAgent.AppTargetID != uuid.Nil {
		t.Fatalf("origin without an authorized handler selected target %s", secondAgent.AppTargetID)
	}
	...
	if afterReplay.AppTargetID != secondAgent.AppTargetID {
		t.Fatalf("replay changed interaction destination to %s", afterReplay.AppTargetID)
	}
	...
	if _, err := store.Apps().DisconnectProjectApp(
		ctx,
...
func TestAppInputAdmissionSerializesWithAppDisconnectAndDeletion(t *testing.T) {
	t.Parallel()
	for _, tc := range []struct {
		name          string
		deleteInstall bool
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			agent := createAppBoundAgent(t, ctx, store, profile, admin.ID, "input-disable-race-agent")
			credentialID := createAppCredential(
				t,
				ctx,
				store,
				testProjectID,
			...
			targetOrigin, err := admitAppOrigin(
				...
				agent.ID,
				install.ID,
				appstore.ConversationAddress{Kind: "dm", Ref: "D_INPUT_DISABLE_RACE"},
			)
			target := targetOrigin.AppTarget
			if err != nil {
			...
			slot.Input.Origin.Address = appstore.ConversationAddress{
			...
			changeInstall := func() error {
				...
				applied, err := store.Apps().DisconnectProjectApp(
					ctx,
					appstore.DisconnectProjectAppInput{
						ProjectID:             install.ProjectID,
						AppID:                 install.ID,
...
SELECT (SELECT count(*) FROM agent_inputs WHERE agent_id = $1 AND input_idempotency_key = $2),
       agent.app_target_id IS NULL, install.deleted_at IS NOT NULL, target.deleted_at IS NOT NULL
FROM agents agent
JOIN app_targets target ON target.agent_id = agent.id AND target.id = $3
...
func TestAppTargetHostedOriginRequiresInbox(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	pool := openIntegrationDB(t, ctx)
	seedMigratedDB(t, ctx, pool)
	...
	agent := createAppBoundAgent(t, ctx, store, profile, admin.ID, "producer-agent")
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, "producer")
	installInput := slackProjectAppSetupInput(
		admin.ID,
		credentialID,
		"A_PRODUCER",
	...
	targetOrigin, err := admitAppOrigin(
		...
		agent.ID,
		install.ID,
		appstore.ConversationAddress{Kind: "dm", Ref: "D_PRODUCER"},
	)
	target := targetOrigin.AppTarget
	if err != nil {
	...
	if err != nil || created.AgentInput.AppTargetID != target.ID {
...
func prepareAppOrigin(
	...
	t.Helper()
	app, err := store.Apps().GetProjectApp(ctx, testProjectID, appID)
	if err != nil {
...
func admitAppOrigin(
	...
	t.Helper()
	lease := prepareAppOrigin(t, ctx, store, agentID, appID, address)
	return store.Execution().AdmitInboxInputSlot(ctx, lease, "recipient")
}
...
func createAppBoundAgent(
	...
	if err != nil {
		t.Fatalf("launch app-bound agent: %v", err)
	}
	return launch.Agent
}
...
func createFixedIntegrationFixture(
	...
	agent := createAppBoundAgent(t, ctx, store, profile, admin.ID, label+"-agent")
	credentialID := createAppCredential(t, ctx, store, testProjectID, admin.ID, label)
	return admin, agent, credentialID
}

func createAppCredential(
	t *testing.T,
	ctx context.Context,
	store *Store,
	...
	if err != nil {
		t.Fatalf("build app credential payload: %v", err)
	}
...
) appstore.ConfigureProjectAppInput {
	return appstore.ConfigureProjectAppInput{
		OrgID:                    testOrgID,
		ProjectID:                testProjectID,
...
func appOAuthFlowID(sequence int) uuid.UUID {
...
func mustCreateProjectApp(
	t *testing.T, ctx context.Context, store *Store, input appstore.ConfigureProjectAppInput,
...
func mustCreateAppInput(
	...
	result, err := store.Execution().AdmitInboxInputSlot(ctx, receipt.Lease(), "recipient")
	if err != nil {
		t.Fatalf("admit app input: %v", err)
	}
