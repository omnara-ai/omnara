...
func (AppAccess) ValidateAppDestination(
	ctx context.Context,
	tx pgx.Tx,
	binding appstore.AppDestination,
) error {
	qtx := dbsqlc.New(tx)
...
func (AppAccess) ClearAppTargetsFromAgents(
	...
	err := dbsqlc.New(tx).ClearDeletedAppTargetsFromAgents(
		ctx,
		dbsqlc.ClearDeletedAppTargetsFromAgentsParams{
			ProjectID: projectID,
			AppID:     appID,
		},
	)
	if err != nil {
		return fmt.Errorf("clear app targets from agents: %w", err)
	}
	return nil
}
