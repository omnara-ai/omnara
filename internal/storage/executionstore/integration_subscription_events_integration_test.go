...
func TestIntegrationSubscriptionEventChangesRequireExplicitReattachmentAndFenceFrozenInput(t *testing.T) {
	t.Parallel()
	...
	_, err := f.store.Integrations().UpdateProjectIntegration(
		...
		integrationstore.SaveProjectIntegrationInput{
			OrgID: testOrgID, ProjectID: testProjectID, Name: f.integration.Name, IntegrationType: f.integration.IntegrationType,
			Settings: integrationstore.ProjectIntegrationSettings{Launcher: &integrationstore.IntegrationLauncher{
				Trigger: "mention", ScopeKind: "installation", ScopeRef: f.integration.ProviderAccountRef,
				Slots: []integrationstore.IntegrationLaunchSlot{{Key: "review", AgentProfileID: &f.profile.ID}},
			}},
		},
	)
	...
	launch.Subscriptions = []integrationstore.IntegrationSubscriptionAttachment{
		{IntegrationID: f.integration.ID, Type: "pull_request", Conversation: json.RawMessage(`{"repository_id":123,"pull_request":41}`), Events: []string{"discussion_comment"}},
		{IntegrationID: f.integration.ID, Type: "pull_request", Conversation: json.RawMessage(`{"repository_id":123,"pull_request":42}`), Events: []string{"discussion_comment"}},
	}
	launch.InitialInput = &executionstore.LaunchInitialInput{
		...
		Actor:            mustIntegrationActorParams(t, f.integration.ID, "789"),
		Origin:           &executionstore.LaunchInputOrigin{IntegrationID: f.integration.ID, Address: address},
	...
	_, _, err = f.store.Integrations().AcceptIntegrationReceipt(f.ctx, integrationstore.VerifiedIntegrationReceipt{
		ProjectID: testProjectID, IntegrationID: f.integration.ID, ReceiptKey: "launch", Payload: []byte(`{}`),
	})
	require.NoError(t, err)
	receipt, found, err := f.store.Integrations().ClaimIntegrationInbox(f.ctx, integrationstore.ClaimIntegrationInboxInput{
		ProjectID: testProjectID, IntegrationID: f.integration.ID, LeaseDuration: time.Minute,
	})
	require.NoError(t, err)
	...
	freeze := func(event string) integrationstore.IntegrationInboxRecord {
		input := inboxInputPlan(t, launched.Agent.ID, f.integration, event)
	...
	_, err = f.store.Integrations().CreateIntegrationSubscription(
		...
		integrationstore.CreateIntegrationSubscriptionInput{
			OrgID: testOrgID, ProjectID: testProjectID, IntegrationID: f.integration.ID, AgentID: launched.Agent.ID,
	...
	require.NoError(
		t,
		f.store.Integrations().DeleteIntegrationSubscription(f.ctx, testOrgID, testProjectID, f.integration.ID, selectedID),
	)
...
func TestIntegrationSubscriptionLaunchBatchRejectsInvalidOrConflictingAttachment(t *testing.T) {
	t.Parallel()
