...
func TestFireDueTriggersAppHandoffRollbackAndRecovery(t *testing.T) {
	t.Parallel()
	f, trigger := cronAppFixture(t)
 ...
 RETURN NEW; END $$;
 CREATE TRIGGER fail_app_handoff BEFORE UPDATE ON cron_triggers FOR EACH ROW EXECUTE FUNCTION fail_app_handoff()`)
	require.NoError(t, err)
	service := crontrigger.NewService(f.store.Execution(), nil, slog.Default())
	stats, err := service.FireDueTriggers(f.ctx)
	require.NoError(t, err)
 ...
 last_app_receipt_id IS NOT NULL FROM cron_triggers WHERE id=$1`, trigger.ID).Scan(&held, &pointed))
	require.True(t, held)
