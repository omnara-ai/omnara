...
type integrationActivationFixture struct {
...
func newIntegrationActivationFixture(t *testing.T) integrationActivationFixture {
	...
	user := createIntegrationProjectAdmin(t, ctx, store, "integration-activation@example.com")
	profile := createIntegrationTestProfile(t, ctx, store, "integration-activation")
	integration := (integrationInteractionFixture{ctx: ctx, store: store, user: user}).createIntegration(t, "chat")

	return integrationActivationFixture{ctx: ctx, store: store, user: user, profile: profile, integration: integration}
}
...
func (f integrationActivationFixture) definition(
	t *testing.T,
	instruction string,
...
func (f integrationActivationFixture) encodedDefinition(
	t *testing.T,
...
func (f integrationActivationFixture) withSendingTools(
	t *testing.T,
	...
	for _, operation := range []string{"post_message", "read"} {
		compiled.Tools[toolcatalog.IntegrationToolName(f.integration.Name, operation)] = agentconfig.ToolCompiled{
			Enabled: true, IntegrationID: f.integration.ID,
...
func (f integrationActivationFixture) launchInput(configID uuid.UUID, key string) executionstore.LaunchAgentInput {
...
func (f integrationActivationFixture) changeInput(
	t *testing.T,
	agentID uuid.UUID,
	instruction string,
	key string,
	...
	return executionstore.ChangeAgentConfigInput{
		...
		ActorID:                f.user.ID,
		Reason:                 "integration-capabilities-test",
		IdempotencyKey:         key,
	}
}
...
func (f integrationActivationFixture) attach(
	t *testing.T, agentID uuid.UUID, attachment integrationstore.IntegrationSubscriptionAttachment,
) integrationstore.IntegrationSubscriptionRecord {
	t.Helper()
	subscription, err := f.store.Integrations().CreateIntegrationSubscription(
		f.ctx,
...
func (f integrationActivationFixture) detach(t *testing.T, agentID uuid.UUID) {
	t.Helper()
...
func TestIntegrationSubscriptionsDetachAndReattachIndependentOfConfig(t *testing.T) {
	...
	_, err = f.store.Execution().ChangeAgentConfig(
		f.ctx,
		f.changeInput(t, launch.Agent.ID, "Config without integration tools", "remove-tools"),
	)
	require.NoError(t, err)
...
func TestIntegrationSubscriptionsProfileLaunchActivationAndReplay(t *testing.T) {
	t.Parallel()
	f := newIntegrationActivationFixture(t)
	config, err := f.store.Execution().CreateAgentConfig(f.ctx, f.definition(t, "Saved integration config"))
	require.NoError(t, err)
	profile, err := f.store.Execution().CreateAgentProfile(f.ctx, executionstore.CreateAgentProfileInput{
		ProjectID: testProjectID, Name: "Integration profile", CurrentConfigID: config.ID,
	})
	require.NoError(t, err)
	var count int
	require.NoError(
	...
	input := f.launchInput(config.ID, "integration-launch")
	...
	removed, err := f.store.Execution().ChangeAgentConfig(
		f.ctx,
		f.changeInput(t, launch.Agent.ID, "Another config", "integration-remove"),
	)
	require.NoError(t, err)
...
func TestIntegrationSubscriptionsRejectCrossProjectIntegrations(t *testing.T) {
	...
	for _, state := range []string{"active", "disconnected", "deleted", "missing"} {
		t.Run(state, func(t *testing.T) {
			...
			if state == "missing" {
				attachment.IntegrationID = uuid.New()
			}
			input := f.launchInput(f.profile.CurrentConfigID, state)
			input.Subscriptions = []integrationstore.IntegrationSubscriptionAttachment{f.attachment(), attachment}
			...
			for _, name := range []string{"int__chat__post_message", "int__chat__read"} {
				tool := compiled.Tools[name]
				tool.IntegrationID = attachment.IntegrationID
...
func TestIntegrationConfigChangesRemainAvailableAtSubscriptionQuota(t *testing.T) {
	...
	_, err = f.store.pool.Exec(
		f.ctx,
		`INSERT INTO org_resource_limit_overrides(org_id,max_active_integration_subscriptions_per_agent) VALUES($1,0)`,
		testOrgID,
	)
	require.NoError(t, err)
	_, err = f.store.Integrations().CreateIntegrationSubscription(
...
func TestIntegrationSubscriptionQuotaPreservesExistingRoutesAndReleasesOnDetach(t *testing.T) {
	t.Parallel()
	f := newIntegrationActivationFixture(t)
	...
	_, err = f.store.pool.Exec(
		f.ctx,
		`INSERT INTO org_resource_limit_overrides(org_id,max_active_integration_subscriptions_per_agent) VALUES($1,1)`,
		testOrgID,
	)
	require.NoError(t, err)
...
func TestIntegrationFailedConfigActivationPreservesSubscriptions(t *testing.T) {
	t.Parallel()
	f := newIntegrationActivationFixture(t)
...
func TestIntegrationCapabilitiesLockIntegrationsBeforeLaunchKeyAndProfile(t *testing.T) {
	t.Parallel()
	f := newIntegrationActivationFixture(t)
	...
	require.NoError(t, err, "launch must not hold its profile while waiting on an integration")
	require.NoError(t, control.Commit(f.ctx))
	launch := integrationdb.AwaitSuccess(t, done, "launch after integration gate")
	require.Len(t, f.subscriptions(t, launch.Agent.ID), 1)
}

func TestIntegrationCapabilitiesLockIntegrationsBeforeAgentSourcesAndAgent(t *testing.T) {
	t.Parallel()
	f := newIntegrationActivationFixture(t)
	...
	require.NoError(t, err, "activation must not hold agent locks while waiting on an integration")
	require.NoError(t, control.Commit(f.ctx))
	changed := integrationdb.AwaitSuccess(t, done, "change after integration gate")
...
func TestIntegrationCapabilitiesValidateCurrentConfigAfterIntegrationWait(t *testing.T) {
	t.Parallel()
...
func TestIntegrationCapabilitiesUnavailableSecondaryDoesNotBlockLaunchOrConfigChange(t *testing.T) {
	t.Parallel()
	for _, state := range []string{"disconnected", "deleted"} {
		t.Run(state, func(t *testing.T) {
			t.Parallel()
			f := newIntegrationActivationFixture(t)
			secondary := (integrationInteractionFixture{ctx: f.ctx, store: f.store, user: f.user}).createIntegration(
				t,
				"secondary",
			)
			definition := f.definition(t, "Saved before integration revocation")
			var compiled agentconfig.Compiled
			require.NoError(t, json.Unmarshal(definition.CompiledDefinition, &compiled))
			compiled.Tools = map[string]agentconfig.ToolCompiled{
				"int__secondary__post_message": {
					Enabled: false, IntegrationID: secondary.ID,
					Permission: toolpermission.DefaultSelection(toolpermission.ModeAlwaysAllow),
				},
			}
			...
			require.NoError(t, err)
			attachment := f.attachment()
			attachment.IntegrationID = secondary.ID
			baseInput := f.launchInput(f.profile.CurrentConfigID, "base")
			baseInput.Subscriptions = []integrationstore.IntegrationSubscriptionAttachment{f.attachment(), attachment}
			...
			launchInput.Subscriptions = []integrationstore.IntegrationSubscriptionAttachment{f.attachment()}
			...
			matching := func(integrationID uuid.UUID) []dbsqlc.IntegrationSubscription {
				t.Helper()
			...
			require.Len(t, matching(f.integration.ID), 2)
			require.Empty(t, matching(secondary.ID), "unavailable integration cannot deliver")
...
func TestIntegrationCapabilitiesInboxLaunchToleratesUnavailableSecondary(t *testing.T) {
	t.Parallel()
	for _, state := range []string{"disconnected", "deleted"} {
		t.Run(state, func(t *testing.T) {
			t.Parallel()
			...
			definition := f.definition(t, "Secondary integration is optional")
			var compiled agentconfig.Compiled
			require.NoError(t, json.Unmarshal(definition.CompiledDefinition, &compiled))
			compiled.Tools = map[string]agentconfig.ToolCompiled{"int__secondary__read": {
				Enabled: false, IntegrationID: secondary.ID,
				Permission: toolpermission.DefaultSelection(toolpermission.ModeAlwaysAllow),
			}}
			...
			_, _, err = f.store.Integrations().AcceptIntegrationReceipt(f.ctx, integrationstore.VerifiedIntegrationReceipt{
				...
				ReceiptKey:    "unavailable-secondary",
				Payload:       []byte(`{}`),
			})
			require.NoError(t, err)
...
func TestIntegrationSubscriptionLaunchBatchQuotaRollbackAndDeduplication(t *testing.T) {
	t.Parallel()
	f := newIntegrationActivationFixture(t)
	_, err := f.store.pool.Exec(
		f.ctx,
	...
	second := f.attachment()
	second.Conversation = json.RawMessage(`{"channel_id":"C456"}`)
	input.Subscriptions = []integrationstore.IntegrationSubscriptionAttachment{f.attachment(), second}
	_, err = f.store.Execution().LaunchAgent(f.ctx, input)
	require.ErrorIs(t, err, storeerr.ErrConflict)
...
func TestIntegrationSubscriptionConcurrentAttachmentsSerializeQuota(t *testing.T) {
	t.Parallel()
	f := newIntegrationActivationFixture(t)
	...
	require.NoError(t, err)
	var pending []<-chan integrationdb.AsyncResult[integrationstore.IntegrationSubscriptionRecord]
	for _, conversation := range []string{`{"channel_id":"C123"}`, `{"channel_id":"C456"}`} {
		pending = append(pending, integrationdb.RunAsync(func() (integrationstore.IntegrationSubscriptionRecord, error) {
			return f.store.Integrations().CreateIntegrationSubscription(
				f.ctx,
				integrationstore.CreateIntegrationSubscriptionInput{
