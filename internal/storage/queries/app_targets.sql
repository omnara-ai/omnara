...
SELECT pg_advisory_xact_lock(hashtextextended(
    'app_conversation:' || jsonb_build_array(sqlc.arg(project_id)::uuid, sqlc.arg(connection_id)::uuid,
...
WHERE target.project_id = sqlc.arg(project_id) AND target.integration_connection_id = sqlc.arg(connection_id)
...
WHERE project_id = sqlc.arg(project_id) AND integration_connection_id = sqlc.arg(connection_id)
  AND provider_ref_kind = sqlc.arg(kind) AND provider_ref = sqlc.arg(ref)
  AND routing_role = 'selected' AND app_id = sqlc.arg(app_id) AND selection_slot = sqlc.arg(slot);
...
WHERE project_id = sqlc.arg(project_id) AND agent_id = sqlc.arg(agent_id)
  AND integration_connection_id = sqlc.arg(connection_id)
  AND provider_ref_kind = sqlc.arg(kind) AND provider_ref = sqlc.arg(ref)
...
VALUES (sqlc.arg(project_id), sqlc.arg(agent_id), sqlc.arg(connection_id), sqlc.arg(target_ref),
    sqlc.arg(kind), sqlc.arg(ref), sqlc.arg(display_name), sqlc.arg(routing_role), sqlc.narg(app_id), sqlc.narg(slot),
