...
VALUES (sqlc.arg(project_id), sqlc.arg(agent_id), sqlc.arg(integration_id), sqlc.arg(subscription_type),
...
SELECT EXISTS (
    ...
    WHERE subscription.project_id = sqlc.arg(project_id) AND subscription.agent_id = sqlc.arg(agent_id)
      AND subscription.integration_id = sqlc.arg(integration_id) AND subscription.subscription_type = sqlc.arg(subscription_type)
