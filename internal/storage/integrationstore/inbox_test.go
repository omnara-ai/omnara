...
func TestInboxReceiptBounds(t *testing.T) {
	valid := VerifiedIntegrationReceipt{
		ProjectID: uuid.New(), IntegrationID: uuid.New(), ReceiptKey: "event:1", Payload: []byte{0, 255, 1},
	}
	...
	for _, mutate := range []func(*VerifiedIntegrationReceipt){
		...
		func(v *VerifiedIntegrationReceipt) { v.IntegrationID = uuid.Nil },
...
func TestInboxSelectionRequiresReceiptIntegration(t *testing.T) {
	...
	_, _, err = ensureIntegrationProfileChoiceTx(t.Context(), nil,
		...
		EnsureIntegrationProfileChoiceInput{IntegrationID: uuid.New()})
	if !errors.Is(err, storeerr.ErrUnauthorized) {
		t.Fatalf("another integration's chooser accepted: %v", err)
	}
}
