...
func TestConversationDisplayNameReusesLatestLiveLabelAcrossAgents(t *testing.T) {
	...
	read := func(project, integrationID uuid.UUID, address integrationstore.ConversationAddress) string {
		t.Helper()
		name, err := f.store.GetConversationDisplayName(f.ctx, project, integrationID, address)
		require.NoError(t, err)
		return name
	}
	require.Empty(t, read(f.project, f.integrationID, address))
 ...
 VALUES($1,$2,$3,$4,$5,$6,$7,$8,$8,$9)`,
	...
	require.Equal(t, "latest", read(f.project, f.integrationID, address))
	require.Empty(t, read(uuid.New(), f.integrationID, address))
	require.Empty(t, read(f.project, uuid.New(), address))
	require.Empty(t, read(f.project, f.integrationID, integrationstore.ConversationAddress{Kind: "dm", Ref: address.Ref}))
	require.Empty(
		t,
		read(f.project, f.integrationID, integrationstore.ConversationAddress{Kind: "thread", Ref: "C456:1.2"}),
	)
	f.exec(t, `UPDATE integration_targets SET deleted_at=now() WHERE id=$1`, targets[1])
	require.Equal(t, "old", read(f.project, f.integrationID, address))
