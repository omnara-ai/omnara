...
func ProjectIntegrationCredentialKind(provider string) (secrets.Kind, error) {
	switch provider {
...
func normalizeConfigureProjectIntegrationInput(input ConfigureProjectIntegrationInput) (
	ConfigureProjectIntegrationInput,
	error,
) {
	...
	_, err := ProjectIntegrationCredentialKind(input.Provider)
	if err != nil {
		return input, err
	}
	if input.CredentialSecretID == uuid.Nil {
		return input, fmt.Errorf("credential secret is required for %s integrations", input.Provider)
	}
	if input.IntegrationID == uuid.Nil || input.ExpectedSetupRevision < 1 || input.CredentialVersionID == uuid.Nil {
		return input, errors.New("integration, expected setup revision, and verified credential version are required")
	}
...
func validateLauncherProviderScope(launcher *IntegrationLauncher, provider, tenantID, accountRef string) error {
	if launcher == nil {
		return nil
	}
	if provider == IntegrationProviderSlack && launcher.ScopeKind == "workspace" && launcher.ScopeRef != tenantID {
		return errors.New("launcher workspace must match the integration's verified Slack workspace")
	}
	if provider == IntegrationProviderGitHub && launcher.ScopeKind == "installation" && launcher.ScopeRef != accountRef {
		return errors.New("launcher installation must match the integration's verified GitHub installation")
	}
	return nil
}
