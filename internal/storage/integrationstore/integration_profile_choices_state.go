...
const integrationProfileChoiceKind = "profile_choice"

// integration_profile_choices.sql also reads owner_receipt_id, message_id and selected_key;
// changes to these JSON keys must update those queries.
type integrationProfileChoiceData struct {
	OwnerReceiptID   uuid.UUID                        `json:"owner_receipt_id"`
	Event            json.RawMessage                  `json:"event"`
	Payload          []byte                           `json:"payload"`
	Options          []IntegrationProfileChoiceOption `json:"options"`
...
func encodeIntegrationProfileChoice(record IntegrationProfileChoiceRecord) (json.RawMessage, error) {
	if err := validateIntegrationProfileChoiceState(record); err != nil {
		return nil, err
	}
	return encodeIntegrationState(integrationProfileChoiceData{
...
func integrationProfileChoiceRecord(row dbsqlc.IntegrationState) (IntegrationProfileChoiceRecord, error) {
	var result IntegrationProfileChoiceRecord
	if row.Kind != integrationProfileChoiceKind || row.ScopeKind == nil || row.ScopeRef == nil || row.ExpiresAt == nil {
		return result, inboxInvalid("profile choice requires its kind, conversation and deadline")
	}
	var data integrationProfileChoiceData
	decoder := json.NewDecoder(bytes.NewReader(row.Data))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&data); err != nil {
		return result, fmt.Errorf("decode integration profile choice: %w", err)
	}
	...
	if err := validateIntegrationProfileChoiceState(result); err != nil {
		return IntegrationProfileChoiceRecord{}, err
	}
	return result, nil
}

func validateIntegrationProfileChoiceState(record IntegrationProfileChoiceRecord) error {
	if err := validateIntegrationProfileChoice(EnsureIntegrationProfileChoiceInput{
		IntegrationID: record.IntegrationID, Address: record.Address, SourceKey: record.SourceKey,
