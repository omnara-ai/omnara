...
func TestConversationDisplayNameReusesLatestLiveLabelAcrossAgents(t *testing.T) {
	...
	read := func(project, connection uuid.UUID, address integrationstore.ConversationAddress) string {
		t.Helper()
		name, err := f.store.GetConversationDisplayName(f.ctx, project, connection, address)
		require.NoError(t, err)
		return name
	}
	require.Empty(t, read(f.project, f.connection, address))
 ...
 (id,project_id,agent_id,integration_connection_id,target_ref,provider_ref_kind,provider_ref,
  display_name,created_at,updated_at,deleted_at)
 VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,$9,$10)`,
			targetID, f.project, launch.Agent.ID, f.connection, targetID.String(), address.Kind, address.Ref,
			name, updated, deleted)
	}
	// Empty and retired targets are newer but cannot hide the last useful label.
	require.Equal(t, "latest", read(f.project, f.connection, address))
	require.Empty(t, read(uuid.New(), f.connection, address))
	require.Empty(t, read(f.project, uuid.New(), address))
	require.Empty(t, read(f.project, f.connection, integrationstore.ConversationAddress{Kind: "dm", Ref: address.Ref}))
	require.Empty(
		t,
		read(f.project, f.connection, integrationstore.ConversationAddress{Kind: "thread", Ref: "C456:1.2"}),
	)
	f.exec(t, `UPDATE integration_targets SET deleted_at=now() WHERE id=$1`, targets[1])
	require.Equal(t, "old", read(f.project, f.connection, address))
