...
func TestIntegrationProfileChoiceUnplannedHandoffReservesConversation(t *testing.T) {
	...
	for _, state := range []string{"pending", "processing"} {
		t.Run(state, func(t *testing.T) {
			...
			f.exec(t, `UPDATE integration_states SET expires_at=now()-interval '1 second' WHERE id=$1`, choice.ID)
			nextInput := f.input
			nextInput.SourceKey = "new-mention"
			reused, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, late.Lease(), nextInput)
			require.NoError(t, err)
			...
			otherIntegration := f.addIntegration(t, "another-setup", setup.Settings)
			other := f
			...
			otherMenu := nextInput
			otherMenu.IntegrationID = otherIntegration.ID
			otherMenu.Options = otherMenu.Options[1:]
			_, _, err = f.store.EnsureIntegrationProfileChoice(f.ctx, late.Lease(), otherMenu)
			require.ErrorIs(t, err, storeerr.ErrUnauthorized, "a receipt cannot create another integration's menu")
			independent, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, otherReceipt.Lease(), otherMenu)
			require.NoError(t, err)
...
func TestIntegrationProfileChoiceFrozenPlanTakesOverReservation(t *testing.T) {
	t.Parallel()
	f := newProfileChoiceFixture(t)
	choice := f.menu(t)
	late := f.receipt(t, "raw-follow-up", f.input.Payload)
	...
	retained, made, err := f.store.EnsureIntegrationProfileChoice(f.ctx, late.Lease(), newMention)
	require.NoError(t, err)
	require.False(t, made, "a frozen processing plan still owns this integration's menu")
	...
	reused, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, late.Lease(), newInput)
	require.NoError(t, err)
...
func TestIntegrationProfileChoiceFailedUnplannedAllowsNewRequest(t *testing.T) {
	t.Parallel()
	f := newProfileChoiceFixture(t)
	choice := f.menu(t)
	...
	fresh, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, late.Lease(), newInput)
	require.NoError(t, err)
	...
	require.NoError(
		t,
		f.store.RecordIntegrationProfileChoiceMessage(f.ctx, f.project, f.integrationID, fresh.ID, "C123", "new-menu"),
	)
...
func TestIntegrationProfileChoiceRechecksSettledSelectionAfterRoutingSnapshot(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			f := newProfileChoiceFixture(t)
			var original integrationstore.IntegrationProfileChoiceRecord
			if tc.unpublished {
				var err error
				original, _, err = f.store.EnsureIntegrationProfileChoice(f.ctx, f.source.Lease(), f.input)
				require.NoError(t, err)
			} else if tc.pending {
				original = f.menu(t)
			}
			var snapshot integrationstore.IntegrationRoutingCandidates
...
func TestIntegrationProfileChoiceStaleOfferedProfileExpiresMenu(t *testing.T) {
	t.Parallel()
	f := newProfileChoiceFixture(t)
	choice := f.menu(t)
	...
	_, err = f.store.ChooseIntegrationProfile(f.ctx, wrongMenu)
	require.ErrorIs(t, err, storeerr.ErrUnauthorized)
	unknown := click
	unknown.Key = "never-offered"
	unchanged, err := f.store.ChooseIntegrationProfile(f.ctx, unknown)
	...
	_, err = f.store.ChooseIntegrationProfile(f.ctx, staleRevision)
	require.ErrorIs(t, err, storeerr.ErrConflict)
	require.Equal(t, choice, f.readChoice(t, choice.ID))
	retired, err := f.store.ChooseIntegrationProfile(f.ctx, click)
	require.ErrorIs(t, err, storeerr.ErrStateTransitionConflict)
...
func TestIntegrationProfileChoiceUnpublishedMenuFollowsOwnerRecovery(t *testing.T) {
	...
	for _, state := range []string{"pending", "processing", "failed", "completed", "deleted"} {
		t.Run(state, func(t *testing.T) {
			t.Parallel()
			f := newProfileChoiceFixture(t)
			choice, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, f.source.Lease(), f.input)
			require.NoError(t, err)
			require.True(t, created)
			...
			replayed, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, late.Lease(), f.input)
			require.NoError(t, err)
			require.False(t, created)
...
func TestIntegrationProfileChoicePublishedMenuSurvivesOwnerCompletion(t *testing.T) {
	t.Parallel()
	f := newProfileChoiceFixture(t)
	choice := f.menu(t)
	f.mutate(t, f.source, func(work *integrationstore.IntegrationInboxLeaseTx) error {
	...
	input := f.input
	input.SourceKey = "later-message"
	retained, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, late.Lease(), input)
	require.NoError(t, err)
	require.False(t, created, "a published menu remains available after its owner completes")
	require.Equal(t, choice, retained)
}

func TestIntegrationProfileChoiceExplicitExpiryPreservesAcceptedWork(t *testing.T) {
	t.Parallel()
	f := newProfileChoiceFixture(t)
	choice := f.menu(t)
	require.ErrorIs(
		t,
		f.store.ExpireIntegrationProfileChoice(f.ctx, uuid.New(), f.integrationID, choice.ID),
		storeerr.ErrNotFound,
	)
	require.ErrorIs(
		t,
		f.store.ExpireIntegrationProfileChoice(f.ctx, f.project, uuid.New(), choice.ID),
		storeerr.ErrNotFound,
	)
	require.NoError(t, f.store.ExpireIntegrationProfileChoice(f.ctx, f.project, f.integrationID, choice.ID))
	expired := f.readChoice(t, choice.ID)
	require.True(t, expired.ExpiresAt.Before(choice.ExpiresAt))
	require.NoError(t, f.store.ExpireIntegrationProfileChoice(f.ctx, f.project, f.integrationID, choice.ID))
	...
	fresh, created, err := f.store.EnsureIntegrationProfileChoice(f.ctx, f.source.Lease(), input)
	require.NoError(t, err)
	require.True(t, created)
	require.NoError(t, f.store.RecordIntegrationProfileChoiceMessage(
		f.ctx, f.project, f.integrationID, fresh.ID, "C123", "fresh-menu"))
	fresh = f.readChoice(t, fresh.ID)
	selected, err := f.store.ChooseIntegrationProfile(f.ctx, f.chooseInput(t, fresh, "review"))
	require.NoError(t, err)
	receipt := f.decidedReceipt(t, selected.ID)
	require.NoError(t, f.store.ExpireIntegrationProfileChoice(f.ctx, f.project, f.integrationID, selected.ID))
...
func TestIntegrationProfileChoiceInboxRetentionProtectsOldSource(t *testing.T) {
	...
	for _, state := range []string{"pending", "processing", "completed", "failed"} {
		t.Run(state, func(t *testing.T) {
			...
			f := newProfileChoiceFixture(t)
			choice := f.menu(t)
			_, found, err := f.store.GetIntegrationProfileChoiceInbox(f.ctx, f.project, f.integrationID, choice.ID)
			require.NoError(t, err)
			...
			got, found, err := f.store.GetIntegrationProfileChoiceInbox(f.ctx, f.project, f.integrationID, choice.ID)
			require.NoError(t, err)
			require.True(t, found)
			require.Equal(t, receipt, got)
			for _, scope := range [][2]uuid.UUID{{uuid.New(), f.integrationID}, {f.project, uuid.New()}} {
				_, found, err = f.store.GetIntegrationProfileChoiceInbox(f.ctx, scope[0], scope[1], choice.ID)
				require.NoError(t, err)
				require.False(t, found)
			}
			f.exec(t, `UPDATE integration_states SET expires_at=now()-interval '30 days' WHERE id=$1`, choice.ID)
			for range 2 {
