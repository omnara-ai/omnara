...
func (f agentConversationFixture) assign(appID uuid.UUID, address integrationstore.ConversationAddress) error {
	...
	if err := integrationstore.LockAppsTx(f.ctx, tx, f.project, nil, appID); err != nil {
		return err
	}
...
func TestAgentAppConversationStateIsolationAndLifecycle(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{uuid.New(), f.agentID, f.appID, integrationstore.ConversationAddress{}},
		{f.project, uuid.New(), f.appID, integrationstore.ConversationAddress{}},
...
func TestAgentAppConversationRejectsMalformedState(t *testing.T) {
	t.Parallel()
	f := newAgentConversationFixture(t)
	require.NoError(t, f.assign(f.appID, integrationstore.ConversationAddress{Kind: "thread", Ref: "C123:1.2"}))
