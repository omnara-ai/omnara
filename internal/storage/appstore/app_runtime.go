...
func (s *Store) ListPersistentApps(
	...
	result := make([]AppInboxApp, 0, len(rows))
	for _, row := range rows {
		result = append(result, AppInboxApp{ProjectID: row.ProjectID, AppID: row.ID})
	}
	return result, nil
}
...
func (s *Store) ClaimAppRuntime(
	...
	if err != nil {
		return AppRuntimeClaim{}, false, fmt.Errorf("claim app runtime: %w", err)
	}
...
func (s *Store) lockRuntimeAuthority(ctx context.Context, tx pgx.Tx, revision AppRuntimeRevision) error {
	...
	if app.Provider != AppProviderDiscord || app.SetupRevision != revision.SetupRevision {
...
func (s *Store) CommitAppRuntime(
	...
	if receipt != nil {
		if err := validateAppReceipt(*receipt); err != nil {
			return err
		}
	...
	return s.withAppRuntime(ctx, lease, func(q *dbsqlc.Queries) error {
		if receipt != nil {
			_, err := q.InsertAppInboxReceipt(ctx, dbsqlc.InsertAppInboxReceiptParams{
