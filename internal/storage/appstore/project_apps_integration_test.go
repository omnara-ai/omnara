...
func TestProjectAppReferencesAndLifecycle(t *testing.T) {
	t.Parallel()
	f := newInboxFixture(t)
	s := appstore.New(f.pool, executionstore.AppAccess{})
	...
	require.Equal(t, "integration:slack:"+app.ID.String(), appstore.IdempotencyScope(app),
...
func TestProjectAppIndependentCapabilitiesAndPagination(t *testing.T) {
	...
	require.NoError(t, err)
	page, err := s.ListProjectApps(f.ctx, appstore.ListProjectAppsInput{ProjectID: f.project, Limit: 1})
	require.NoError(t, err)
	require.True(t, page.HasMore)
	require.Equal(t, second.ID, page.Apps[0].ID)
	page, err = s.ListProjectApps(
		f.ctx,
		appstore.ListProjectAppsInput{ProjectID: f.project, Limit: 1, After: page.Next},
	)
	require.NoError(t, err)
	require.True(t, page.HasMore)
	...
	page, err = s.ListProjectApps(f.ctx, appstore.ListProjectAppsInput{ProjectID: f.project, Limit: 100})
	require.NoError(t, err)
