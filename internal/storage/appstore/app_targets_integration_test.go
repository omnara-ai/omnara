...
func TestAppConversationSelectionsRemainIndependentOfSubscriptions(t *testing.T) {
	...
	createApp := func(name string) uuid.UUID {
		t.Helper()
		return f.addApp(t, name, appstore.ProjectAppSettings{}).ID
	}
	input := appstore.EnsureConversationTargetInput{
	...
	require.NoError(t, err)
	require.Empty(t, attribution.SelectionSlot)
	_, err = store.CreateAppSubscription(f.ctx, appstore.CreateAppSubscriptionInput{
	...
	f.exec(t, `UPDATE app_targets SET deleted_at=now() WHERE id=$1`, second.ID)
	...
	require.NoError(t, appstore.LockAppsTx(f.ctx, tx, f.project, nil, first.AppID, second.AppID))
	require.NoError(t, appstore.LockConversationTx(f.ctx, tx, f.project, input.AppID, input.Address))
	candidates, err := store.AppRoutingCandidatesTx(
		...
		input.AppID,
		input.Address,
		[]appstore.ConversationAddress{input.Address},
		"message",
	)
	require.NoError(t, err)
	...
	require.NoError(t, appstore.LockConversationTx(f.ctx, tx, f.project, first.AppID, input.Address))
	other, err := store.AppRoutingCandidatesTx(f.ctx, tx, f.project, first.AppID, input.Address,
		[]appstore.ConversationAddress{input.Address}, "message")
	require.NoError(t, err)
...
func (f inboxFixture) ensureConversationTarget(
	...
	if err := appstore.LockAppsTx(f.ctx, tx, f.project, nil, input.AppID); err != nil {
		return appstore.AppTargetRecord{}, err
	}
	if err := appstore.LockConversationTx(f.ctx, tx, f.project, input.AppID, input.Address); err != nil {
		return appstore.AppTargetRecord{}, err
	}
