...
func slackSetupFixture(
	t *testing.T,
) (inboxFixture, appstore.ConfigureProjectAppInput, appstore.SaveProjectAppInput) {
	t.Helper()
	...
	return f, input, appstore.SaveProjectAppInput{
...
func TestSlackAppSetupFailureRetainsMetadataAndReconnectPreservesSettings(t *testing.T) {
	...
	consumed, err := f.store.AppOAuthFlowConsumed(f.ctx, input.OAuthFlowID)
	require.NoError(t, err)
	...
	applied, err := f.store.DisconnectProjectApp(f.ctx, appstore.DisconnectProjectAppInput{
	...
	require.Equal(t, appstore.ProjectAppStateActive, reconnected.State)
	require.Equal(t, disconnected.SetupRevision+1, reconnected.SetupRevision)
	require.Nil(t, reconnected.Settings.Launcher)
	page, err := f.store.ListProjectApps(f.ctx, appstore.ListProjectAppsInput{ProjectID: f.project, Limit: 100})
	require.NoError(t, err)
...
func TestSlackAppInvalidLauncherEditLeavesSetupAndOAuthUnchanged(t *testing.T) {
	...
	consumed, err := f.store.AppOAuthFlowConsumed(f.ctx, pendingFlow)
	require.NoError(t, err)
	require.False(t, consumed)
	current, err := f.store.GetProjectApp(f.ctx, f.project, app.ID)
	require.NoError(t, err)
...
func TestSlackAppSetupRechecksConcurrentDisconnect(t *testing.T) {
	...
	require.ErrorIs(t, result.Err, appstore.ErrProjectAppSetupChanged)
...
func TestProjectAppLauncherMatchesVerifiedProviderAccount(t *testing.T) {
	...
	for _, provider := range []string{
		appstore.AppProviderSlack, appstore.AppProviderGitHub,
	} {
		t.Run(provider, func(t *testing.T) {
			...
			require.NoError(t, err)
			if provider == appstore.AppProviderGitHub {
			...
			metadata.Settings.Launcher = &appstore.AppLauncher{
			...
			if setup.OAuthFlowID != uuid.Nil {
				consumed, err := f.store.AppOAuthFlowConsumed(f.ctx, setup.OAuthFlowID)
				require.NoError(t, err)
				require.False(t, consumed)
			}
