...
func explainInboxQuery(t *testing.T, f inboxFixture, name string, parameters map[string]any) inboxQueryPlan {
	t.Helper()
	return explainInboxQueryFile(t, f, "app_inbox.sql", name, parameters)
}
 ...
 FROM generate_series(1,32000) n`, f.project, f.appID, app.String())
	...
	assertLookup := func(t *testing.T, wantID uuid.UUID, wantState string, maxInspected float64) {
		...
		walk = func(node inboxQueryPlan) {
			if node.Index == "app_inbox_selection_idx" && node.Loops > 0 {
				ginUsed = true
				require.LessOrEqual(
					t,
...
func assertInboxRowsInspected(t *testing.T, plan inboxQueryPlan, maxRows float64) {
	...
	var walk func(inboxQueryPlan)
	walk = func(node inboxQueryPlan) {
		if node.Relation == "app_inbox" && node.NodeType != "ModifyTable" {
...
func TestInboxPollAccessPathsIgnoreHealthyPendingAndHistory(t *testing.T) {
	t.Parallel()
	f := newInboxFixture(t)
	disabled := f.addApp(t, "disconnected", appstore.ProjectAppSettings{}).ID
	f.exec(t, `UPDATE project_apps SET state='disconnected' WHERE id=$1`, disabled)
	other := f.addApp(t, "other-ready", appstore.ProjectAppSettings{}).ID
	f.exec(
		t,
		`INSERT INTO app_inbox(project_id,app_id,receipt_key,payload,state,available_at,completed_at)
 ...
 FROM generate_series(1,32000) n`,
	...
	for _, name := range []string{
		"RecoverExpiredAppInboxReceipts", "FailInactiveAppInboxReceipts",
		"CleanupDeletedAppInboxReceipts",
	} {
		t.Run(name, func(t *testing.T) {
			assertInboxRowsInspected(t, explainInboxQuery(t, f, name, map[string]any{"row_limit": 100}), 0)
		})
	}
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "OldestReadyAppInboxLag", nil), 1)
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "ListReadyAppInboxApps", map[string]any{
		"row_limit": 100, "after_project_id": uuid.Nil, "after_app_id": uuid.Nil,
	}), 100)
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "CleanupTerminalAppInboxReceipts", map[string]any{
		"row_limit": 100, "retention_milliseconds": time.Hour.Milliseconds(),
	}), 0)
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "ClaimAppInboxReceipt", map[string]any{
	...
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "RecoverExpiredAppInboxReceipts", map[string]any{
		"row_limit": 1,
	}), 2)
	f.exec(t, `INSERT INTO app_inbox(project_id,app_id,receipt_key,payload)
 SELECT $1,$2,'inactive:'||n,'x'::bytea FROM generate_series(1,8000) n`, f.project, disabled)
	f.exec(t, "ANALYZE app_inbox")
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "ListReadyAppInboxApps", map[string]any{
		"row_limit": 100, "after_project_id": uuid.Nil, "after_app_id": uuid.Nil,
	}), 100)
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "FailInactiveAppInboxReceipts", map[string]any{
		"row_limit": 1,
	}), 2)
	f.exec(t, `UPDATE project_apps SET deleted_at=now() WHERE id=$1`, disabled)
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "CleanupDeletedAppInboxReceipts", map[string]any{
		"row_limit": 1,
	}), 2)
	f.exec(t, `UPDATE app_inbox SET available_at=now()+interval '1 day' WHERE state='pending'`)
	f.exec(t, "VACUUM ANALYZE app_inbox")
	assertInboxRowsInspected(t, explainInboxQuery(t, f, "OldestReadyAppInboxLag", nil), 0)
}
...
func TestInboxInactiveRecoveryBatchesAppsAcrossProjects(t *testing.T) {
 ...
 (SELECT credential_secret_id FROM project_apps WHERE id=$6),now(),now())`,
			...
			f.exec(t, `INSERT INTO app_inbox(project_id,app_id,receipt_key,payload,state,completed_at)
 ...
 CASE WHEN $3::text='failed' THEN now() ELSE NULL END FROM generate_series(1,4000) n`, project, id, inboxState)
	...
	f.exec(t, "ANALYZE project_apps")
	f.exec(t, "ANALYZE app_inbox")
	name := "FailInactiveAppInboxReceipts"
 ...
 SELECT project_id,id,'batch-pending','x'::bytea FROM project_apps WHERE state='disconnected'`)
	...
	query, args := bindInboxQueryFile(t, "app_inbox.sql", name, map[string]any{"row_limit": 3})
	...
	require.NoError(t, f.pool.QueryRow(f.ctx,
		`SELECT count(*) FROM app_inbox WHERE state='pending'`).Scan(&remaining))
