...
func (s *Store) AcceptScheduledAppEventTx(
	...
	if err := validateAppReceipt(VerifiedAppReceipt{
	...
	if app.State != ProjectAppStateActive {
		return AppInboxRecord{}, false, storeerr.ErrUnauthorized
	}
	if _, err := appdefinition.ValidateScheduleSettings(app.AppType, input.Event.Settings); err != nil {
		return AppInboxRecord{}, false, storeerr.InvalidRequest(err)
	}
	...
	if !created {
		...
		if err == nil && (row.Source != string(AppInboxSourceScheduled) ||
			!jsoncanonical.Equal(row.Payload, payload)) {
			return AppInboxRecord{}, false, storeerr.ErrIdempotencyConflict
		}
	}
	if err != nil {
		return AppInboxRecord{}, false, err
...
func (r AppInboxRecord) ScheduledEvent() (ScheduledAppEvent, error) {
	var event ScheduledAppEvent
	if r.Source != AppInboxSourceScheduled {
...
func (r AppInboxRecord) ValidateScheduledPlan(app ProjectAppRecord, plan json.RawMessage) error {
