...
func (s *Store) ConfigureProjectApp(ctx context.Context, input ConfigureProjectAppInput) (ProjectAppRecord, error) {
	...
	if input.Provider == AppProviderSlack && input.OAuthFlowID == uuid.Nil {
	...
	if storeutil.IsUniqueViolationOnConstraint(err, "project_apps_last_oauth_flow_id_idx") {
		return ProjectAppRecord{}, storeerr.ErrAppOAuthFlowConsumed
	}
	if err != nil {
...
func (s *Store) AppOAuthFlowConsumed(ctx context.Context, flowID uuid.UUID) (bool, error) {
	if flowID == uuid.Nil {
		return false, errors.New("flow id is required")
	}
	return s.q.AppOAuthFlowConsumed(ctx, dbsqlc.AppOAuthFlowConsumedParams{FlowID: &flowID})
}
