...
func newInboxFixture(t *testing.T) inboxFixture {
 ...
 VALUES($1,$2,$3,$4,'active','T123','inbox-app','inbox-app','slack_thread',$5,now(),now())`,
	...
	require.NoError(t, err)
	return inboxFixture{
		ctx: ctx, pool: pool, store: appstore.New(pool, executionstore.AppAccess{}), org: ids.OrgID,
		project: ids.ProjectID, appID: appID, user: ids.ProviderAdminUserID,
	}
}

func (f inboxFixture) accept(t *testing.T, key string) appstore.AppInboxRecord {
	t.Helper()
	r, created, err := f.store.AcceptAppReceipt(f.ctx, appstore.VerifiedAppReceipt{
...
func (f inboxFixture) claim(t *testing.T) appstore.AppInboxRecord {
	t.Helper()
	r, ok, err := f.store.ClaimAppInbox(f.ctx, appstore.ClaimAppInboxInput{
...
func (f inboxFixture) mutate(
	t *testing.T, r appstore.AppInboxRecord, apply func(*appstore.AppInboxLeaseTx) error,
) {
	t.Helper()
	require.NoError(t, f.store.WithAppInboxLease(f.ctx, r.Lease(), apply))
}
...
func (f inboxFixture) read(t *testing.T, id uuid.UUID) appstore.AppInboxRecord {
	t.Helper()
	r, err := f.store.GetAppInbox(f.ctx, f.project, id)
	require.NoError(t, err)
	return r
}
...
func TestInboxVerifiedReceiptDeduplicationAndIsolation(t *testing.T) {
	...
	payload := bytes.Repeat([]byte{0, 255}, appstore.AppInboxMaxPayloadBytes/2)
	binary, created, err := f.store.AcceptAppReceipt(f.ctx, appstore.VerifiedAppReceipt{
	...
	_, err = f.store.GetAppInbox(f.ctx, uuid.New(), first.ID)
	require.ErrorIs(t, err, storeerr.ErrNotFound)
	_, _, err = f.store.AcceptAppReceipt(f.ctx, appstore.VerifiedAppReceipt{
		ProjectID: uuid.New(), AppID: f.appID, ReceiptKey: "cross-project", Payload: []byte{1},
	})
	require.ErrorIs(t, err, storeerr.ErrNotFound)
	for _, sql := range []string{
		`UPDATE app_inbox SET payload=decode(repeat('00',1048577),'hex') WHERE id=$1`,
		`UPDATE app_inbox SET payload=''::bytea WHERE id=$1`,
		`UPDATE app_inbox SET receipt_key=repeat('x',513) WHERE id=$1`,
		`UPDATE app_inbox SET plan=jsonb_build_object('x',repeat('x',262145)) WHERE id=$1`,
		`UPDATE app_inbox SET progress=jsonb_build_object('x',repeat('x',262145)) WHERE id=$1`,
		`UPDATE app_inbox SET last_error=repeat('x',4097) WHERE id=$1`,
		`UPDATE app_inbox SET attempt_count=9 WHERE id=$1`,
		`UPDATE app_inbox SET state='failed' WHERE id=$1`,
		`UPDATE app_inbox SET completed_at=now() WHERE id=$1`,
	} {
...
func TestInboxClaimSkipsLockedAndHasSingleOwner(t *testing.T) {
	...
	_, err = tx.Exec(f.ctx, `SELECT id FROM app_inbox WHERE id=$1 FOR UPDATE`, first.ID)
	require.NoError(t, err)
	claimed := f.claim(t)
	...
	_, ok, err := f.store.ClaimAppInbox(f.ctx, appstore.ClaimAppInboxInput{
		ProjectID: f.project, AppID: f.appID, LeaseDuration: time.Minute,
	})
	require.NoError(t, err)
	require.False(t, ok)
...
func TestInboxCrashRecoveryExhaustsBudgetAndPreservesPlan(t *testing.T) {
	...
	f.accept(t, "crashing")
	r := f.claim(t)
	f.mutate(t, r, func(w *appstore.AppInboxLeaseTx) error {
	...
	for attempt := 1; attempt <= appstore.AppInboxMaxAttempts; attempt++ {
		require.Equal(t, attempt, r.AttemptCount)
		f.exec(t, `UPDATE app_inbox SET claim_expires_at=now()-interval '1 second' WHERE id=$1`, r.ID)
		n, err := f.store.RecoverAppInbox(f.ctx, 1)
		require.NoError(t, err)
		require.EqualValues(t, 1, n)
		err = f.store.WithAppInboxLease(f.ctx, r.Lease(), func(w *appstore.AppInboxLeaseTx) error {
			return w.Fail(f.ctx, "stale")
		})
		require.ErrorIs(t, err, appstore.ErrAppInboxLeaseLost)
		if attempt < appstore.AppInboxMaxAttempts {
			r = f.claim(t)
		}
	}
	final := f.read(t, r.ID)
	require.Equal(t, appstore.AppInboxFailed, final.State)
...
func TestInboxFrozenSlotsPreparationAndAtomicProgress(t *testing.T) {
	...
	for _, apply := range []func(*appstore.AppInboxLeaseTx) error{
		func(w *appstore.AppInboxLeaseTx) error {
		...
		func(w *appstore.AppInboxLeaseTx) error { return w.Complete(f.ctx) },
	} {
		require.Error(t, f.store.WithAppInboxLease(f.ctx, r.Lease(), apply))
	}
	tx, err := f.pool.Begin(f.ctx)
	require.NoError(t, err)
	w, err := f.store.LockAppInboxLeaseTx(f.ctx, tx, r.Lease())
	require.NoError(t, err)
	...
	f.exec(t, `UPDATE app_inbox SET available_at=now() WHERE id=$1`, r.ID)
	r = f.claim(t)
	err = f.store.WithAppInboxLease(f.ctx, r.Lease(), func(w *appstore.AppInboxLeaseTx) error {
	...
	original := f.read(t, r.ID)
	f.mutate(t, r, func(w *appstore.AppInboxLeaseTx) error { return w.Fail(f.ctx, "second slot failed") })
...
func TestInboxLeaseRevalidatedAfterLockWaitAndInsideTransaction(t *testing.T) {
	...
	require.NoError(t, err)
	defer func() { _ = tx.Rollback(f.ctx) }()
	_, err = tx.Exec(f.ctx, `SELECT id FROM app_inbox WHERE id=$1 FOR UPDATE`, r.ID)
	require.NoError(t, err)
	done := make(chan error, 1)
	go func() {
		done <- f.store.WithAppInboxLease(f.ctx, r.Lease(), func(w *appstore.AppInboxLeaseTx) error {
			return w.FreezePlan(f.ctx, json.RawMessage(`{}`))
		})
	}()
	integrationdb.WaitForNamedLockWaiters(t, f.ctx, f.pool, "LockAppInboxReceipt", 1)
	_, err = tx.Exec(f.ctx, `UPDATE app_inbox SET claim_expires_at=now()-interval '1 second' WHERE id=$1`, r.ID)
	require.NoError(t, err)
	require.NoError(t, tx.Commit(f.ctx))
	require.ErrorIs(t, <-done, appstore.ErrAppInboxLeaseLost)
	_, err = f.store.RecoverAppInbox(f.ctx, 1)
	require.NoError(t, err)
	r = f.claim(t)
	tx, err = f.pool.Begin(f.ctx)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = tx.Exec(f.ctx,
		`UPDATE app_inbox SET claim_expires_at=clock_timestamp()-interval '1 second' WHERE id=$1`, r.ID)
	require.NoError(t, err)
	require.ErrorIs(t, w.FreezePlan(f.ctx, json.RawMessage(`{}`)), appstore.ErrAppInboxLeaseLost)
	require.NoError(t, tx.Rollback(f.ctx))
}
...
func TestInboxRetrySchedulingFailureAndBoundedCleanup(t *testing.T) {
	...
	read := f.read(t, r.ID)
	require.Equal(t, appstore.AppInboxPending, read.State)
	require.Nil(t, read.CompletedAt)
	require.LessOrEqual(t, len(read.LastError), 4096)
	ready, err := f.store.ListReadyAppInboxApps(f.ctx, appstore.AppInboxApp{}, 100)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.False(t, ok)
	f.exec(t, `UPDATE app_inbox SET available_at=now(),attempt_count=7 WHERE id=$1`, r.ID)
	r = f.claim(t)
	f.mutate(t, r, func(w *appstore.AppInboxLeaseTx) error {
		return w.Retry(f.ctx, time.Second, "last failure")
	})
	require.Equal(t, appstore.AppInboxFailed, f.read(t, r.ID).State)
	require.NotNil(t, f.read(t, r.ID).CompletedAt)
	for i := range 3 {
		f.accept(t, fmt.Sprintf("done-%d", i))
		claimed := f.claim(t)
		f.mutate(t, claimed, func(w *appstore.AppInboxLeaseTx) error {
	...
	f.exec(t, `UPDATE app_inbox SET completed_at=now()-interval '1 day' WHERE state='completed'`)
	n, err := f.store.CleanupTerminalAppInbox(f.ctx, time.Hour, 2)
	require.NoError(t, err)
...
func TestInboxScopeLifecycleFencesAdmissionAndPurgesDeletedPayloads(t *testing.T) {
	...
	require.NoError(t, err)
	require.EqualValues(t, 1, n)
	require.Equal(t, appstore.AppInboxFailed, f.read(t, pending.ID).State)
	require.NotNil(t, f.read(t, pending.ID).CompletedAt)
	require.Equal(t, appstore.AppInboxProcessing, f.read(t, r.ID).State)
	n, err = f.store.RecoverAppInbox(f.ctx, 1)
	require.NoError(t, err)
	require.Zero(t, n)
	f.exec(t, `UPDATE app_inbox SET claim_expires_at=now()-interval '1 second' WHERE id=$1`, r.ID)
	...
	require.Equal(t, appstore.AppInboxFailed, f.read(t, r.ID).State)
	require.NotNil(t, f.read(t, r.ID).CompletedAt)
	n, err = f.store.CleanupDeletedAppInbox(f.ctx, 1)
	require.NoError(t, err)
	require.Zero(t, n)
...
func TestInboxRejectsDeletedProjectAndOrganization(t *testing.T) {
	...
	for _, scope := range []string{"project", "organization"} {
		t.Run(scope, func(t *testing.T) {
			...
			require.ErrorIs(t, err, storeerr.ErrNotFound)
			_, _, err = f.store.ClaimAppInbox(f.ctx, appstore.ClaimAppInboxInput{
				ProjectID: f.project, AppID: f.appID, LeaseDuration: time.Minute,
			})
			require.ErrorIs(t, err, storeerr.ErrNotFound)
			n, err := f.store.CleanupDeletedAppInbox(f.ctx, 1)
			require.NoError(t, err)
			require.EqualValues(t, 1, n)
			_, err = f.store.GetAppInbox(f.ctx, f.project, r.ID)
...
func TestInboxMultipleHandlesCannotOverwritePreparedStages(t *testing.T) {
	...
	require.NoError(t, err)
	defer func() { _ = tx.Rollback(f.ctx) }()
	first, err := f.store.LockAppInboxLeaseTx(f.ctx, tx, r.Lease())
	require.NoError(t, err)
	second, err := f.store.LockAppInboxLeaseTx(f.ctx, tx, r.Lease())
	require.NoError(t, err)
...
func TestInboxDisableWaitsForAtomicAdmission(t *testing.T) {
	...
	require.Equal(t, appstore.AppInboxCompleted, f.read(t, r.ID).State)
}
...
func TestInboxJSONBNormalizedSizeBoundaryIsExplicitAndAtomic(t *testing.T) {
	...
	remaining := appstore.AppInboxMaxPlanBytes - len(prefix) - len(suffix)
	oversized := json.RawMessage(prefix + strings.Repeat("x", remaining) + suffix)
	require.Len(t, oversized, appstore.AppInboxMaxPlanBytes)
	err := f.store.WithAppInboxLease(f.ctx, r.Lease(), func(w *appstore.AppInboxLeaseTx) error {
	...
	f.mutate(t, r, func(w *appstore.AppInboxLeaseTx) error { return w.FreezePlan(f.ctx, fits) })
	overhead := len(`{"slot":{"prepared":{"data":""}}}`)
	preparationBytes := strings.Repeat("x", appstore.AppInboxMaxPlanBytes-overhead)
...
func TestInboxSetupUpdateWaitsForAtomicAdmission(t *testing.T) {
	t.Parallel()
	f := newInboxFixture(t)
	f.store = appstore.New(f.pool, executionstore.AppAccess{})
	...
	require.NoError(t, err)
	defer func() { _ = tx.Rollback(f.ctx) }()
	work, err := f.store.LockAppInboxLeaseTx(f.ctx, tx, receipt.Lease())
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, appstore.ProjectAppStateActive, during.State)
	...
	require.Equal(t, appstore.ProjectAppStateActive, after.State)
	require.Equal(t, app.SetupRevision+1, after.SetupRevision)
	require.Equal(t, credential.ID, after.CredentialSecretID)
	require.Equal(t, appstore.AppInboxCompleted, f.read(t, receipt.ID).State)
}

func TestInboxDiscoveryPagesAppsIndependentlyOfBacklog(t *testing.T) {
	t.Parallel()
	f, _, setup := projectAppSetupFixture(t)
	var apps []appstore.AppInboxApp
	for i := range 4 {
		app, err := f.store.CreateProjectApp(f.ctx, appstore.SaveProjectAppInput{
		...
		require.NoError(t, err)
		apps = append(apps, appstore.AppInboxApp{ProjectID: f.project, AppID: app.ID})
		fixture := f
	...
	f.exec(t, `UPDATE app_inbox SET available_at=now()+interval '1 day' WHERE app_id=$1`, apps[0].AppID)
	_, err := f.store.DisconnectProjectApp(f.ctx, appstore.DisconnectProjectAppInput{
		ProjectID: f.project, AppID: apps[1].AppID,
	})
	require.NoError(t, err)
	var after appstore.AppInboxApp
	var ready []appstore.AppInboxApp
	for i := range apps {
		page, err := f.store.ListReadyAppInboxApps(f.ctx, after, 1)
		require.NoError(t, err)
		if i < len(apps)-1 {
	...
	page, err := f.store.ListReadyAppInboxApps(f.ctx, after, 100)
	require.NoError(t, err)
...
func TestInboxRecoveryMakesProgressThroughMixedBacklogs(t *testing.T) {
 ...
 count(*) FILTER (WHERE app_id=$1 AND state='pending') FROM app_inbox`, disabled).
	...
	ready, err = f.store.ListReadyAppInboxApps(f.ctx, appstore.AppInboxApp{}, 100)
	require.NoError(t, err)
	require.Equal(t, []appstore.AppInboxApp{{ProjectID: f.project, AppID: f.appID}}, ready.Apps)
	claimed := f.claim(t)
...
func TestInboxOldestReadyLagUsesAvailabilityAndDistinguishesFailure(t *testing.T) {
 ...
 VALUES ($1,$2,'future','x'::bytea,now()-interval '3 days',now()+interval '1 hour','pending',NULL),
		...
		f.project, f.appID)
	lag, err := f.store.OldestReadyAppInboxLag(f.ctx)
	require.NoError(t, err)
	require.Zero(t, lag, "future retries and terminal history are not ready work")
	f.exec(t, `INSERT INTO app_inbox(project_id,app_id,receipt_key,payload,created_at,available_at)
 VALUES ($1,$2,'ready','x'::bytea,now()-interval '2 days',now()-interval '90 seconds')`, f.project, f.appID)
	...
	cancel()
	_, err = f.store.OldestReadyAppInboxLag(ctx)
