...
func projectAppSetupFixture(
	t *testing.T,
) (inboxFixture, *secretstore.Store, appstore.ConfigureProjectAppInput) {
	t.Helper()
	f := newInboxFixture(t)
	...
	require.NoError(t, err)
	return f, secretStore, appstore.ConfigureProjectAppInput{
		OrgID: f.org, ProjectID: f.project, AppID: app.ID, InstalledByUserID: f.user,
		Provider: appstore.AppProviderSlack, ProviderTenantID: "T123", ProviderAccountRef: "router",
...
func projectAppMetadata(app appstore.ProjectAppRecord) appstore.SaveProjectAppInput {
	return appstore.SaveProjectAppInput{
...
func TestProjectAppSetupLifecycleAndOwnership(t *testing.T) {
	...
	require.ErrorIs(t, err, storeerr.ErrNotFound)
	other, err := f.store.CreateProjectApp(f.ctx, appstore.SaveProjectAppInput{
	...
	require.Equal(t, appstore.ProjectAppStateDisconnected, updated.State)
...
func TestProjectAppTypeConstraints(t *testing.T) {
	...
	require.Equal(t, appstore.AppProviderSlack, app.Provider)
...
func TestProjectAppCredentialAuthorizationAndRotation(t *testing.T) {
	...
	input.Provider = appstore.AppProviderGitHub
...
func TestProjectAppOAuthReplayCannotRecreateDeletedApp(t *testing.T) {
	...
	require.ErrorIs(t, err, storeerr.ErrAppOAuthFlowConsumed)
	require.NoError(t, f.store.DeleteProjectApp(f.ctx, f.org, f.project, first.ID))
	_, err = f.store.ConfigureProjectApp(f.ctx, input)
	require.ErrorIs(t, err, storeerr.ErrNotFound)
	_, err = f.store.ConfigureProjectApp(f.ctx, replay)
	require.ErrorIs(t, err, storeerr.ErrAppOAuthFlowConsumed, "deleted apps still prevent cross-app OAuth replay")
}
