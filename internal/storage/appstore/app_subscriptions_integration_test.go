...
) appstore.CreateAppSubscriptionInput {
	return appstore.CreateAppSubscriptionInput{
...
func TestAppSubscriptionsIndependentIdentityPaginationAndDetach(t *testing.T) {
	...
	require.NoError(t, err)
	page, err := f.store.ListAppSubscriptions(f.ctx, appstore.ListAppSubscriptionsInput{
		ProjectID: f.project, AppID: f.appID, Limit: 1,
	})
	require.NoError(t, err)
	require.True(t, page.HasMore)
	require.Equal(t, second.ID, page.Subscriptions[0].ID)
	next, err := f.store.ListAppSubscriptions(f.ctx, appstore.ListAppSubscriptionsInput{
	...
	require.NoError(t, f.pool.QueryRow(f.ctx, `SELECT current_config_id,
	 (SELECT count(*) FROM app_targets WHERE agent_id=$1) FROM agents WHERE id=$1`,
	...
	_, err = f.store.ListAppSubscriptions(f.ctx, appstore.ListAppSubscriptionsInput{
...
func TestAppSubscriptionValidationDisconnectAndLifecycle(t *testing.T) {
	...
	require.NoError(t, err)
	changed, err := f.store.DisconnectProjectApp(f.ctx, appstore.DisconnectProjectAppInput{
	...
	other := f.addApp(t, "other", appstore.ProjectAppSettings{})
	...
	third := f.addApp(t, "third", appstore.ProjectAppSettings{})
...
func TestAppSubscriptionDeletionLocksReceiveOnlyAgentAndFencesAttach(t *testing.T) {
	...
	attached := integrationdb.RunAsync(func() (appstore.AppSubscriptionRecord, error) {
