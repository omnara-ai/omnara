...
func scheduledInboxSnapshot(t *testing.T, f inboxFixture) appstore.ScheduledAppEvent {
	t.Helper()
...
func acceptScheduledInbox(t *testing.T, f inboxFixture, launch appstore.ScheduledAppEvent) {
	t.Helper()
	tx := integrationdb.BeginTx(t, f.ctx, f.pool)
	require.NoError(t, lifecyclelock.EnterActiveProject(f.ctx, tx, f.org, f.project))
	require.NoError(t, appstore.LockAppsTx(f.ctx, tx, f.project, nil, f.appID))
	_, created, err := f.store.AcceptScheduledAppEventTx(f.ctx, tx, appstore.AcceptScheduledAppEventInput{
...
func TestScheduledSelectionRequiresTrustedReceiptSource(t *testing.T) {
	...
	require.NoError(t, err)
	provider := f.claim(t)
	require.Equal(t, appstore.AppInboxSourceProvider, provider.Source)
	...
	scheduled := f.claim(t)
	f.mutate(t, scheduled, func(w *appstore.AppInboxLeaseTx) error {
		return w.FreezePlan(f.ctx, plan)
	})
...
func TestScheduledReceiptReplayRequiresIdenticalSnapshot(t *testing.T) {
	...
	for _, changed := range []bool{false, true} {
		...
		receipt, created, err := f.store.AcceptScheduledAppEventTx(f.ctx, tx, appstore.AcceptScheduledAppEventInput{
...
func scheduledPlanLaunch(
	t *testing.T, f inboxFixture, event appstore.ScheduledAppEvent, root appdefinition.Scope,
) map[string]any {
	t.Helper()
