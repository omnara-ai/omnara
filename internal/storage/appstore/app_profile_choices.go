...
func (s *Store) EnsureAppProfileChoice(
	ctx context.Context, lease AppInboxLease, input EnsureAppProfileChoiceInput,
...
func ensureAppProfileChoiceTx(
	ctx context.Context, q *dbsqlc.Queries, receipt AppInboxRecord, input EnsureAppProfileChoiceInput,
) (AppProfileChoiceRecord, bool, error) {
...
func (s *Store) GetAppProfileChoiceBySource(
	...
	if projectID == uuid.Nil || appID == uuid.Nil ||
		!choiceText(sourceKey, AppInboxMaxReceiptKeyBytes) {
...
func (s *Store) GetAppProfileChoiceInbox(
	...
	if projectID == uuid.Nil || appID == uuid.Nil || choiceID == uuid.Nil {
		return AppInboxRecord{}, false, inboxInvalid("project, app and choice are required")
	}
	row, err := s.q.GetAppInboxReceiptByKey(ctx, dbsqlc.GetAppInboxReceiptByKeyParams{
	...
	if err != nil {
		return AppInboxRecord{}, false, fmt.Errorf("get app profile choice inbox: %w", err)
	}
...
func (s *Store) ChooseAppProfile(ctx context.Context, input ChooseAppProfileInput) (AppProfileChoiceRecord, error) {
	...
	if err := validateChoiceJSON(input.Events, '[', AppInboxMaxEventsBytes); err != nil {
		return result, err
	}
	tx, err := s.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
...
func validateAppProfileChoice(input EnsureAppProfileChoiceInput) error {
	if input.AppID == uuid.Nil || !choiceText(input.SourceKey, AppInboxMaxReceiptKeyBytes) {
	...
	if err := validateChoiceJSON(input.Event, '{', AppInboxMaxEventsBytes); err != nil {
		return err
	}
