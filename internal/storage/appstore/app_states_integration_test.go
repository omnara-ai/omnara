...
func TestAppProfileChoiceMaximumSourceRoundTrip(t *testing.T) {
	...
	f := newProfileChoiceFixture(t)
	input := f.input
	input.Payload = bytes.Repeat([]byte{0, 255, '<', '\n'}, appstore.AppInboxMaxPayloadBytes/4)
	input.Event = json.RawMessage(`{"text":"` + strings.Repeat("a", appstore.AppInboxMaxEventsBytes-512) + `"}`)
	...
	require.NoError(t, f.pool.QueryRow(f.ctx,
		`SELECT payload FROM app_inbox WHERE project_id=$1 AND app_id=$2 AND receipt_key=$3`,
