...
func newAppRuntimeFixture(
	t *testing.T,
) (inboxFixture, *secretstore.Store, appstore.ProjectAppRecord, uuid.UUID) {
	t.Helper()
	f := newInboxFixture(t)
 ...
 VALUES($1,$2,'owner',now()) ON CONFLICT DO NOTHING`, f.org, f.user)
	...
	require.NoError(t, err)
	app, err = f.store.ConfigureProjectApp(f.ctx, appstore.ConfigureProjectAppInput{
		OrgID: f.org, ProjectID: f.project, AppID: app.ID, InstalledByUserID: f.user,
		Provider: appstore.AppProviderDiscord, ProviderTenantID: "123", ProviderAccountRef: "456",
...
func TestAppRuntimeFailureUsesCurrentSetupAndCredential(t *testing.T) {
	for _, scenario := range []string{
		...
		t.Run(scenario, func(t *testing.T) {
			...
			read := func() *appstore.AppRuntimeFailure {
				t.Helper()
      ...
      FOR EACH ROW EXECUTE FUNCTION reject_runtime_checkpoint()`)
			...
			if scenario == "commit" {
				...
				require.NoError(
					...
					f.pool.QueryRow(
						f.ctx,
						`SELECT count(*) FROM app_inbox WHERE app_id=$1`,
						app.ID,
					).
						Scan(
							&count,
			...
			require.NoError(
				t,
				f.pool.QueryRow(f.ctx, `SELECT count(*) FROM app_inbox WHERE app_id=$1`, app.ID).
					Scan(&count),
			)
...
func TestAppRuntimeSettingsPreserveLeaseCheckpointAndBackoff(t *testing.T) {
	...
	updated, err := f.store.UpdateProjectApp(f.ctx, app.ID, appstore.SaveProjectAppInput{
