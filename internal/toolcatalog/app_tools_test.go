...
func TestEveryAppOperationWithContextAndExplicitDestination(t *testing.T) {
	for _, definition := range appdefinition.All() {
		id := definition.AppType
		for _, operation := range definition.Tools {
			t.Run(string(id)+"/"+operation, func(t *testing.T) {
				...
				case AppOperationPostMessage:
					...
					if id == appdefinition.DiscordThread {
...
func TestAppToolChannelContextPreservesLegacyChildThreads(t *testing.T) {
	for _, test := range []struct {
		appType                          appdefinition.Type
		kind, ref, args, wantRef, escape string
	}{
		{appdefinition.SlackThread, "channel", "C123", `{"text":"hello","thread_ts":"1.2"}`, "C123:1.2",
			`{"text":"hello","thread_ts":"3.4"}`},
		{appdefinition.SlackThread, "dm", "D123", `{"text":"hello","thread_ts":"1.2"}`, "D123:1.2",
			`{"text":"hello","thread_ts":"3.4"}`},
		{appdefinition.DiscordThread, "channel", "456", `{"content":"hello","thread_id":"789"}`, "456:789",
			`{"content":"hello","thread_id":"790"}`},
	} {
		t.Run(string(test.appType)+"/"+test.kind, func(t *testing.T) {
			tool, _ := LookupAppTool(test.appType, AppOperationPostMessage)
			context, err := appdefinition.ParseConversation(tool.Provider, test.kind, test.ref)
			require.NoError(t, err)
...
func TestDiscordContextGuildMetadata(t *testing.T) {
	tool, _ := LookupAppTool(appdefinition.DiscordThread, AppOperationPostMessage)
