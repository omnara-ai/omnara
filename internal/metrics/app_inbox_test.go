...
func TestAppInboxLagDistinguishesEmptyFailedAndStaleSamples(t *testing.T) {
	set := New()
	m := NewAppInboxRecorder(set)
	value := func(name string) float64 {
		t.Helper()
		families, err := set.registry.Gather()
		require.NoError(t, err)
		for _, family := range families {
			if family.GetName() == "omnara_app_inbox_"+name {
	...
	require.Contains(t, body, "omnara_app_inbox_oldest_ready_lag_seconds 0")
	require.Contains(t, body, "omnara_app_inbox_lag_sample_last_success_timestamp_seconds ")
	require.NotContains(t, body, "omnara_app_inbox_oldest_ready_lag_seconds{")
}
