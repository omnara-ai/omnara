...
export type UpdateOmnaraCommentParams = {
  owner: string;
  repo: string;
  commentId: number;
  body: string;
...
export type UpdateOmnaraCommentResult = {
  id: number;
  html_url: string;
  updated_at: string;
};
...
/**
 * Updates an Omnara comment on GitHub (either an issue/PR comment or a PR review comment)
 *
 ...
 */
export async function updateOmnaraComment(
  octokit: Octokit,
  params: UpdateOmnaraCommentParams,
): Promise<UpdateOmnaraCommentResult> {
