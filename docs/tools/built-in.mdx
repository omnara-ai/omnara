...
Omnara includes built-in tools for common agent tasks. Add ordinary tools by name under `tools:` in the agent's config. App tools use names such as `app__engineering__post_message` and the credentials of that project [app](/integrations/apps). Built-in tools default to `always_allow`, but you can [change the permission](/tools/permissions) for each one.
...
| Apps | Slack, GitHub and Discord read/write tools; `list_interaction_handlers`, `set_interaction_handler` |
| Skills | `skill` |
...
On success, the artifact appears in the conversation and the durable tool result contains its `path` and a `media_ref` that clients can use to display or download it. The returned `path` can be passed directly to `download_file`; `source` and `destination` refer to paths on the selected machine. The artifact's contents are not added to later model context. To deliver one or more through an external integration, take the artifact ID from the final component of each `/artifacts/<artifact_id>` path and pass those IDs in `artifact_ids` to the Slack or Discord app’s qualified `post_message` tool.
...
Select [app tools](/integrations/apps) by qualified name, such as
`app__engineering__post_message`. An app owns its credentials. Hosted launches save
an immutable sending conversation for each agent/app. Tools may omit destination
arguments to use that context; supplied values must stay within it. A channel
context permits its child threads, while a thread context stays in that thread.
Without context, supply a complete address. Tool declarations have no hidden
`config`. Credential access, live app state and ordinary permission govern execution.

Slack and Discord offer `read` and `post_message`. GitHub offers `read`,
`discussion_comment`, `inline_comment` and `reply`. See the provider guides for
arguments and pagination. Ordinary model output remains in Omnara.

Slack and Discord posts may explicitly request `follow_replies: true`. Successful
publication and local registration attach an app-owned subscription to the
confirmed conversation using the posting tool's existing authorization; no config
receive grant is required. This does not select an approval destination. Later
config or sender changes leave the subscription intact. Manage it from the app's
[Conversations list or subscriptions API](/integrations/apps#stop-forwarding).

### `list_interaction_handlers` / `set_interaction_handler`

These are compiled defaults for tool-capable configs, unless explicitly disabled.
`list_interaction_handlers` accepts `cursor` and `limit`, returning handler keys,
descriptions and effective argument schemas. Its current selection is included
independently of pagination. No configured handlers means an empty list.

`set_interaction_handler` takes `{"handler":"engineering","args":{"channel_id":"C123"}}`
for a Slack channel, or `{"handler":null,"args":{}}` for dashboard only. Always
supply the complete destination described by the handler's schema in `args`;
this selection is independent of the app's sending context. The choice
persists for future questions and approvals; existing prompts keep their captured
destination. A new provider input can select its matching handler automatically.
