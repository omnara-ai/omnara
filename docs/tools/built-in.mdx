...
Omnara includes built-in tools for common agent tasks. Add ordinary tools by name under `tools:` in the agent's config. Integration tools use names such as `int__engineering__post_message` and the credentials of that project [integration](/integrations/overview). Built-in tools default to `always_allow`, but you can [change the permission](/tools/permissions) for each one.
...
| Integrations | Integration-specific tools; `list_interaction_handlers`, `set_interaction_handler` |
| Skills | `skill` |
...
Select [integration tools](/integrations/overview) by qualified name, such as
`int__engineering__post_message`. An integration owns its credentials and defines its
...
hidden `config`. Credential access, live integration state and ordinary permission govern
...
them from the integration's [Conversations list or subscriptions API](/integrations/overview#stop-forwarding).
...
dashboard only. Handler selection is independent of the integration's sending context. The choice
