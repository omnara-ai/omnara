...
Use `download_file` to copy a file from Omnara's virtual filesystem to an attached machine. Currently, downloads support only `/artifacts/<artifact_id>`; provide `machine_ref` when the agent has more than one machine.
