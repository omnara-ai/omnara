...
Slack and Discord posts may request `follow_replies: true` when the agent config
contains that app's `thread_messages` listener. Confirmed publication registers
the returned conversation under the listener; it does not choose an approval
destination. The listener remains independent of later sender changes.
