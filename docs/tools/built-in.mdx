...
| Apps | App-specific tools; `list_interaction_handlers`, `set_interaction_handler` |
| Skills | `skill` |
...
`app__engineering__post_message`. An app owns its credentials and defines its
tools' argument schemas and conversation requirements. See its guide and catalog
for available operations, arguments and pagination. Tool declarations have no
hidden `config`. Credential access, live app state and ordinary permission govern
...
them from the app's [Conversations list or subscriptions API](/integrations/apps#stop-forwarding).

### `list_interaction_handlers` / `set_interaction_handler`

Select these tools explicitly under `tools`, or use an app launcher that supplies
...
`set_interaction_handler` takes a `handler` key and the complete destination
described by that handler's schema in `args`. Use `{"handler":null,"args":{}}` for
dashboard only. Handler selection is independent of the app's sending context. The choice
