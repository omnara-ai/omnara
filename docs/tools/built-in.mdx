...
- With several, copy the lowercase public `machine_id` (`mch_...`) exactly as returned by `list_machines`.
...
Use `upload_artifact` to make a regular file on an attached machine available for the user to view or download, such as a generated PDF, image, or report. Pass a relative, absolute, or `~`-relative `path`; provide `machine_id` when the agent has more than one machine.
...
Use `download_artifact` to copy a conversation artifact to an attached machine. Pass the artifact's public `artifact_id` and the exact destination `path`; provide `machine_id` when the agent has more than one machine. Relative and `~`-relative paths are supported, the parent directory must already exist, and an existing destination is replaced atomically.
