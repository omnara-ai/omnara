...
Starts a subagent from a configured handle with a clean context. Requires `agent` (the handle) and `task`; `name` optionally labels the subagent for later messages. Returns immediately with the subagent's id. The subagent's final answer arrives later as a message from it.
...
Lists the agent's subagents with their handle, state, and last activity.
