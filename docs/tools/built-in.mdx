...
Starts a subagent from a configured `subagents` key with a clean context. Requires `agent` (the key) and `task`; `name` optionally labels the subagent for later messages. Returns immediately with the subagent's id. The subagent's final answer arrives later as a message from it.

### `wait_agents`

Parks the tool call until subagents finish. `agents` lists names or ids; omit it to wait for every running subagent. `mode` is `all` (default) or `any`; `timeout_seconds` bounds the wait. The result carries each subagent's state and final answer, or the question it is waiting on.
...
Lists the agent's subagents with their key, state, and last activity.
