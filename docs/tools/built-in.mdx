...
Starts a subagent from a configured `subagents` key with a clean context. Requires `agent` (the key) and `task`; `name` is an optional display label. Returns immediately with the subagent's `agent_id` and `agent_ref`, the short form the other subagent tools take. The subagent's final answer arrives later as a message from it.

### `read_agent`

Reads a page of a subagent's timeline by `agent_ref`: inputs, model outputs, and tool results with their text. `after_sequence` pages forward from the start, oldest first; `before_sequence` pages backward, newest first, with 0 meaning the latest events. `limit` defaults to 20.

### `send_agent_message`

Delivers `message` to the subagent identified by `agent_ref` as a steering input: it interrupts the subagent's current work and cancels any open question or permission request. Parents cannot answer a subagent's questions; those wait for a human. The reply arrives as a message from the subagent.

### `stop_agent`

Cancels and archives the subagent identified by `agent_ref`.

### `list_agents`

Lists the agent's subagents with their `agent_ref`, name, key, state, and last activity.
