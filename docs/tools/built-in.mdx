...
Starts a subagent from a configured `subagents` key with a clean context. Requires `agent` (the key) and `task`; `name` optionally labels the subagent for later messages. Returns immediately with the subagent's id. The subagent's final answer arrives later as a message from it. `timeout_seconds` optionally caps the subagent's lifetime; when it elapses the subagent is stopped and the parent is notified. There is no default timeout.

### `read_agent`

Reads a page of a subagent's timeline: inputs, model outputs, and tool results with their text. `after_sequence` pages forward from the start; `before_sequence` pages backward, with 0 meaning the latest events. `limit` defaults to 20.

### `send_agent_message`

Delivers `message` to a subagent. Pass `interaction_id` to answer a question the subagent asked. The reply arrives as a message from the subagent.

### `stop_agent`

Cancels and archives a subagent.

### `list_agents`

Lists the agent's subagents with their name, key, state, and last activity.
