...
Omnara includes built-in tools for common agent tasks. Add ordinary tools by name under `tools:` in the agent's config. Provider tools also require a scoped [app resource](/integrations/apps). Built-in tools default to `always_allow`, but you can [change the permission](/tools/permissions) for each one.
...
| Apps | Slack, GitHub and Discord read/write tools; `list_interaction_destinations`, `set_interaction_destination` |
| Skills | `skill` |
...
On success, the artifact appears in the conversation and the durable tool result contains its `path` and a `media_ref` that clients can use to display or download it. The returned `path` can be passed directly to `download_file`; `source` and `destination` refer to paths on the selected machine. The artifact's contents are not added to later model context. To deliver one or more through an external integration, take the artifact ID from the final component of each `/artifacts/<artifact_id>` path and pass those IDs in `artifact_ids` to `slack_post_message` or `discord_post_message`, with an app resource authorizing the destination.
...
Select these tools through [app resources](/integrations/apps), with the connection
and scope the agent may use. Tool calls accept a `resource` key; it is required
when several attachments expose the action. Sending and receiving are separate
capabilities, and ordinary model output remains in Omnara.

- [Slack](/integrations/slack#configure-an-existing-agent): `slack_read` reads a
  channel or thread; `slack_post_message` sends `text` and optional `artifact_ids`.
- [GitHub](/integrations/github#select-tools-for-one-pr): `github_read` reads a PR;
  `github_discussion_comment`, `github_inline_comment` and `github_reply` post
  individual comments inside that PR.
- [Discord](/integrations/discord#give-an-agent-scoped-tools): `discord_read` reads
  messages; `discord_post_message` sends `content` and optional `artifact_ids`.

Slack and Discord posts can request `follow_replies: true` when the resource also
has `follow: {replies: true}`. A successful post registers receiving for that
conversation; it does not select an approval destination. Normal tool permission
modes apply to provider actions.

### `list_interaction_destinations` / `set_interaction_destination`

Declaring an enabled interaction handler adds these tools unless explicit policy
disables them. `list_interaction_destinations` lists eligible `resource` and
`target_id` pairs. `set_interaction_destination` takes
`{"destination":{"resource":"approvals","target_id":"itgt_…"}}` from that list,
or `{"destination":null}` for dashboard-only. It changes future questions and
approvals, without moving existing prompts or changing send-tool scopes.
