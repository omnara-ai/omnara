...
an immutable sending conversation for each agent/app. Tools use that conversation
and accept only action and pagination arguments. They fail before provider I/O
when the agent has no assigned conversation. Tool declarations have no hidden
`config`. Credential access, live app state and ordinary permission govern execution.

Slack and Discord offer `read` and `post_message` within their assigned conversation.
GitHub offers `read`, `discussion_comment`, `inline_comment` and `reply` within its
assigned PR. See the provider guides for arguments and pagination. Ordinary model
output remains in Omnara.

Posting never changes where the agent listens. Launchers attach subscriptions;
manage them independently from the app's
...
Slack and Discord launchers add these tools with their interaction handler,
preserving explicit tool settings. GitHub does not add them. Other configs can
select them explicitly under `tools`; they are not global defaults.
