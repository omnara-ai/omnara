...
Reads a page of a subagent's timeline by `agent_ref`: inputs, model outputs, and tool results with their text. `after_sequence` pages forward from the start; `before_sequence` pages backward, with 0 meaning the latest events. `limit` defaults to 20.

### `send_agent_message`

Delivers `message` to the subagent identified by `agent_ref`. Pass `interaction_id` to answer a question the subagent asked. The reply arrives as a message from the subagent.
