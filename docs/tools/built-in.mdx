...
| Integrations | `list_channels`, `send_channel_message`, `send_integration_message`, `set_integration_target` |
| Skills | `skill` |
...
Sends a user-visible text message through an attached channel. Requires `text` and supports only `always_allow`.

When the current model call came from exactly one sendable channel, omit `channel_id` to reply there. Otherwise, call `list_channels` and pass the exact `channel_id`. Omnara still checks the agent's active send binding at execution time, so a stale or unrelated ID cannot expand the agent's access.

The result reports whether the delivery was completed, rejected, still pending, or could not be confirmed. A pending result means Omnara durably accepted the message; the connector can finish sending it after the model call ends.

The following tools remain available for the native Slack compatibility path:
