...
Omnara includes built-in tools for common agent tasks. Add ordinary tools by name under `tools:` in the agent's config. App tools use names such as `app__engineering__post_message` and the credentials of that project [app](/integrations/apps). Their optional fixed config narrows the destination; otherwise the model can choose destinations accessible to those credentials. Built-in tools default to `always_allow`, but you can [change the permission](/tools/permissions) for each one.
...
`app__engineering__post_message`. An app owns its credentials; each tool's optional
`config` fixes settings separately from its model arguments. The credential's
provider access, live app state and ordinary tool permission govern execution.
...
when that handler permits a dynamic channel, or `{"handler":null,"args":{}}` for
dashboard only. Fixed destination fields are omitted from `args`. The choice
