...
| Artifacts | `upload_artifact`, `show_artifact` |
...
Creates an artifact from a regular file on an attached machine. Requires a relative, absolute, or `~`-relative `path`; provide `machine_ref` when the agent has more than one machine. The result includes a process ID; on successful completion, process output contains the new `artifact_id`. Files must be non-empty and may be up to 10 MiB.

### `show_artifact`

Requests that clients consuming the conversation display an [artifact](/events/artifacts#show-an-existing-artifact) available to the agent and make it available to download. Requires an `artifact_id`. It does not add the artifact's contents to model context or deliver the file through an external integration.
