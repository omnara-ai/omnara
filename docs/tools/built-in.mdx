...
Creates a machine from an available pool. Specify `machine_pool_name` only when the config provides more than one pool. Optional `cpu` and `memory_mb` override the defaults where supported, within pool and project limits.
...
`list_machines` lists the agent's BYO and pool-backed machines. `inspect_machine` shows whether a specific machine is ready to run commands; provide `machine_id` when the agent has more than one. Results include `cpu` and `memory_mb` when known and identify the machine's `source_kind` and `binding_kind`; only pool bindings can be passed to `delete_machine`.
...
Artifact patterns such as `/artifacts/*.pdf` match filenames and return `/artifacts/<artifact_id>` paths. A literal path matches only that entry. Results include hidden entries; if `truncated` is true, narrow the pattern.
...
Reads a UTF-8 text file at `/artifacts/<artifact_id>` or `/memory/<store-name>/<file-path>`. Files must contain no NUL bytes. Artifacts may be up to 48 MiB; memory files may be up to 10 MiB. Readability is determined from the file's bytes, regardless of its declared content type.
...
| Input | Type | Notes |
| --- | --- | --- |
| `path`<span style={{ color: "#c97a7a", fontWeight: 700 }}>*</span> | string | Exact `/memory/<store-name>/<file-path>` destination |
