...
| `limit_lines` | integer | 1–200; default 200 |
| `offset_byte` | integer | ≥ 0; default 0 in byte mode; must be a UTF-8 character boundary |
| `limit_bytes` | integer | 1–4096; default 4096 in byte mode; use at least 4 to accommodate any character |

Reading defaults to line mode. Supplying either byte input selects byte mode; do not combine line and byte inputs. Each response contains at most 4 KiB of file text, together with `size_bytes`, `content_type`, and `has_more`.

When `has_more` is true, continue using the returned `next_offset_line` or `next_offset_byte`. A line longer than one page returns its first chunk and a byte offset; switch to byte mode to continue. Reading past the end returns empty content with `has_more: false`.
