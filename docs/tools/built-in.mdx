...
On success, the artifact appears in the conversation and the durable tool result contains a `media_ref` that clients can use to display or download it. The artifact's contents are not added to later model context or delivered through external integrations.
