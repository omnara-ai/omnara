...
`list_machines` lists the agent's BYO and pool-backed machines, plus pools from its config that have an active project grant. Pools are included even when they have no machines yet. Each pool includes its `machine_pool_id`, `machine_pool_name`, and configured description.
...
When `list_machines` returns `next_cursor`, pass it as `cursor` in the next call to continue listing. Pools are listed first, followed by machines. If `list_machines` is disabled and multiple pools are configured, provide their IDs in the agent's instructions so it can select a pool when calling `create_machine`.
