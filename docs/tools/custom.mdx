...
<Tip>
To avoid continuous polling, connect to the agent's [event stream](/events/streaming), then query `GET /tool-calls?state=ready&type=custom` for already-ready calls and watch for future `tool_call_update` frames whose `state` is `ready`. Repeat the query after reconnecting because lifecycle updates are not replayed. Subagents can call custom tools too: the parent's stream carries their custom-tool `tool_call_update` frames with the subagent's `agent_id`, and `include_subagents=true` on the query returns their calls. Question and permission requests use separate `agent_change` frames with `interactions` in `changes`; refresh the interactions endpoint for those requests.
</Tip>
