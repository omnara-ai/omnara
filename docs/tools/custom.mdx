...
<Tip>
To avoid polling, watch the agent's [event stream](/events/streaming) for new tool calls and query for calls in `ready` state. That way your streaming worker can react the moment a call appears.
</Tip>
