...
For a complete runnable example — PostHog's hosted MCP server behind bearer auth, driven from a notebook with Slack delivery and a daily cron trigger — see [`examples/posthog-pulse-agent`](https://github.com/omnara-ai/omnara/tree/main/examples/posthog-pulse-agent).
