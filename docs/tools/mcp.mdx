...
| `auth.type` | `bearer` (a token in a [generic secret](/organization/secrets)) or `oauth` (an `oauth_token_set` secret — see below). Omit for unauthenticated servers. |
...
| `default_enabled` | `false` flips the server to allowlist mode: only tools listed under `tools:` are exposed. |
...
These names appear in `model_output` tool calls and in [`GET /tool-calls`](/tools/custom#find-calls-waiting-for-you) with `type: "mcp"`. Server and tool names cannot contain `__`, and the combined name must fit within 64 characters.
