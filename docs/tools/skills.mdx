...
Each skills page lists the skills owned at that scope. Fetching a skill through the API returns `skill_md`, the full instruction text of its current revision. To delete an owned skill in the console, open its row menu and click **Delete**. Deletion removes its revisions and grants, but is blocked with `409` while an active agent config references it.
