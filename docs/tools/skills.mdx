...
<Tab title="API">
  ...
  `POST /skills` takes multipart form data: an `owner` and the `archive` itself. Send `owner` as a JSON part (`Content-Type: application/json`); a part without a content type is rejected. The CLI's `--archive` takes a `.zip` or `.tar.gz` file, or a skill directory that it packs for you:
