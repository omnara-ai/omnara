...
<Note>
Provider tools such as `int__engineering__post_message` use the ordinary permission modes.
They use the named project [integration](/integrations/overview) and its provider credentials.
The integration's guide describes any required conversation context. Approval prompts
show the destination alongside the action. Interaction
handlers are independent: an agent can request approval through a handler without
having that integration's sending tools.
</Note>
