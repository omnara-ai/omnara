...
<Note>
Provider tools such as `app__engineering__post_message` use the ordinary permission modes.
They use the named project [app](/integrations/apps) and its provider credentials.
Hosted launches save a sending conversation independently of config. Calls stay
within that context; without it, the model supplies any destination the credentials
can access. A channel context permits its child threads; a thread context remains
confined to that thread. Approval prompts show the resolved destination even when
the call omits it. Interaction handlers are independent: an agent can request
approval in Slack without having a Slack sending tool.
</Note>
