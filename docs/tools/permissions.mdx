...
<Note>
Provider tools such as `slack_post_message` use the ordinary permission modes.
They also need a scoped [app resource](/integrations/apps); permission alone does
not grant a destination. The interaction handler can present an approval without
granted access to the send tool being approved.
</Note>
