...
The five channel tools are supplied by live bindings: `list_channels`, `get_channel`, `set_current_channel`, `read_channel`, and `send_channel_message`. They are not configurable profile tools. Read and send availability follows the corresponding grants and provider capabilities; revoked access takes effect without creating a new config. Slack uses this same toolkit.

Omnara has exactly three kinds of tools, distinguished by _who executes them_:
...
<Note>
  The channel toolkit (`list_channels`, `get_channel`, `set_current_channel`,
  `read_channel`, and `send_channel_message`) is supplied from current channel
  bindings and supports only `always_allow`. Each operation still checks its
  required live grant. Send and receive grants do not imply permission to read
  history; selecting the current channel grants no additional access.
</Note>
