...
Fixed tool config can restrict the destination; with empty config the model may
choose any destination those credentials can access. Interaction handlers are
independent: an agent can request approval in Slack without having a Slack sending tool.
</Note>
