...
Hosted launches save a sending conversation independently of config. The shipped
app tools always use that conversation and fail before provider I/O if none is
assigned. Approval prompts show the destination alongside the action. Interaction
handlers are independent: an agent can request approval in Slack without having a
Slack sending tool.
</Note>
