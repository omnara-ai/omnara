...
For an interaction routed to an integration handler, list and resolve responses include
its captured `destination`. Its `integration_id` and `integration_target_id`
...
message metadata. These fields describe integration delivery; your application can
present interactions without them. See [Integrations](/integrations/overview) for handler
configuration and integration-specific behavior, or
[Custom integrations](/integrations/custom-integrations#present-and-resolve-interactions)
for presenting interactions through your own service.
