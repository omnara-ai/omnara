...
Similarly, you can cancel a queued input, change its position, or switch it between queued and steering delivery. Promoting is idempotent: it also returns `200` if the input is already steering or the agent has already taken it. Other invalid state transitions return `409`.
...
Hosted integrations record senders with `provider: "integration"`, the configured integration’s public
`itg_` ID as `provider_tenant_id`, and the sender’s provider ID as
`provider_user_id`. The same sender is reused within that integration; different integrations
have separate actor identities. These are attribution records, not permissions,
and remain readable after integration deletion. Historical Slack actors retain
`provider: "slack"` and their original workspace-based identities.
...
Your service chooses which agent receives each provider event and submits an
ordinary input. Pass an `actor` to attribute the message, and use a stable
`Idempotency-Key` for retries. Your service owns the conversation-to-agent mapping;
Omnara does not need an integration registration. See [Custom integrations](/integrations/custom-integrations)
for the complete input, custom-tool and interaction flow.
