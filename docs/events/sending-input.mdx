...
Hosted apps record senders with `provider: "app"`, the configured app’s public
`app_` ID as `provider_tenant_id`, and the sender’s provider ID as
`provider_user_id`. The same sender is reused within that app; different apps
have separate actor identities. These are attribution records, not permissions,
and remain readable after app deletion. Historical Slack actors retain
...
Omnara does not need an app registration. See [Custom integrations](/integrations/custom-integrations)
