...
A successful [`upload_file`](/tools/built-in#upload_file) result contains the public `artifact_id` as structured data and a `media_ref` for presentation. Use the ID with the metadata and content endpoints below.
...
| `upload_file` | The durable tool result stores the artifact ID and a `media_ref`. |
