...
Model context identifies conversation attachments by their public `artifact_id`, allowing an agent with [`download_file`](/tools/built-in#download_file) to copy one to an attached machine using `path: "/artifacts/<artifact_id>"` and a machine `destination`.

## Artifacts uploaded by an agent

A successful [`upload_file`](/tools/built-in#upload_file) result contains the public `artifact_id` and canonical VFS `path` as structured data and a `media_ref` for presentation. Use the ID with the metadata and content endpoints below.
  ...
  "content_blocks": [
    {
      ...
      "value": {
        ...
        "path": "/artifacts/art_z3jehcyd5n6a2bfgik7mv4qtrw"
      }
    },
    {
      "type": "media_ref",
...
| `upload_file` | The durable tool result stores the artifact ID, canonical VFS path, and a `media_ref`. |
