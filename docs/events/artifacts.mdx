...
A successful [`upload_artifact`](/tools/built-in#upload_artifact) result carries artifact metadata in a `structured_data` block rather than a `media_ref`, so presenting it does not make its bytes eligible for later model context. Match the result's `tool_call_id` to the corresponding `upload_artifact` call, then use its `artifact_id` with the endpoints below.
...
| `upload_artifact` | The durable tool result carries the artifact metadata needed to display or download it. |
