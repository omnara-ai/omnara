...
A successful [`upload_file`](/tools/built-in#upload_file) result contains the `path` as structured data and a `media_ref` for presentation. Use the artifact ID from the final component of the path with the metadata and content endpoints below.
...
| `upload_file` | The durable tool result stores the path and a `media_ref`. |
