...
A successful [`upload_artifact`](/tools/built-in#upload_artifact) result contains a `media_ref`. Read its `artifact_id` from the tool result, then use the metadata and content endpoints below to display or download the artifact.
...
| `upload_artifact` | The durable tool result stores a `media_ref`. |
