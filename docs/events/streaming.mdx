...
Saved events use their `sequence` as the SSE `id`. Heartbeat comments (`:` lines) only keep the connection open and can be ignored. Live delivery notifications are best-effort hints; a later notification or a reconnect with `Last-Event-ID` catches up every saved event after the cursor. A simplified exchange looks like:
