...
- **`model_output`** — one completed model response, with a `stop_reason` and any tool calls the model requested. `continue_after_truncation: true` requests automatic continuation after an output limit; treat that output as ongoing work. Later output or cancellation determines when the turn ends.
