...
- **`model_output`** — one completed model response, with a `stop_reason` and any tool calls the model requested. `stop_reason: max_tokens` schedules continuation after an output limit; treat that output as ongoing work. Tool calls settle individually before the next model request. Later output or cancellation determines when the turn ends.
...
| `tool_call_update` | A tool call ID, its agent ID, and a reported lifecycle state | No |
| `agent_change` | The changed agent and parent IDs, and which current-state queries to refresh | No |
| `model_output_delta` | A live preview while the model responds | No |
| `error` | A stream-closing error; retry only `service_unavailable` | No |

Saved events use their `sequence` as the SSE `id`. Heartbeat comments (`:` lines) arrive every 10 seconds while the stream is idle; they only keep the connection open and can be ignored. While a connection remains open, the server keeps it caught up from its last delivered saved sequence. After a disconnect, reconnecting with `Last-Event-ID` catches up every saved event after that cursor. A simplified exchange looks like:
...
`tool_call_update` frames contain only `tool_call_id`, `agent_id`, and `state`. The stream includes every tool type on this agent and custom tools on its descendants. After the stream connects, including after a reconnect, query `GET /tool-calls?include_subagents=true` for current state.

`agent_change` frames contain `agent_id`, a nullable `parent_agent_id`, and a `changes` array. An `agent` change covers this agent or a direct child: refresh the changed agent's details and the child list filtered by its `parent_agent_id`. An `interactions` change covers this agent or a descendant at any depth: refresh `GET /interactions?include_subagents=true` when a question or permission request is created, answered, or canceled. The IDs identify the changed agent and its direct parent, even when a more distant ancestor receives the frame.

These notifications may arrive out of order and are not replayed. Refresh the agent's details, current tool calls, interactions, and child lists after connecting or reconnecting; a manual refresh also recovers missed notifications. A parent's stream does not carry its descendants' timelines, model previews, or ordinary built-in tool updates.

After an `error` frame, the server closes that HTTP response. A raw client should reconnect when the stable `code` is `service_unavailable`; other currently documented error codes are terminal and should be surfaced to the caller. Do not decide from the human-readable `error` text.
...
Raw HTTP clients should reconnect after a network failure or an unexpected end of the response. Set `Last-Event-ID` to the last saved sequence you processed. You can also pass the same value as `after_sequence`:
...
The TypeScript SDK owns this loop for you. `openAgentEventStream` follows the durable stream until you stop it, automatically reconnects after transient HTTP, transport, and `service_unavailable` failures, and resumes after the last delivered saved sequence. Other API and contract errors are thrown.
...
for await (const frame of openAgentEventStream({
  ...
  onConnectionStateChange(state) {
    if (state.state === 'reconnecting') clearDeltaPreviews()
    if (state.state === 'connected' && state.reconnected) refreshOpenToolCalls()
  },
})) {
  applyFrame(frame)
}
...
Use the abort signal when you need to stop the stream promptly; breaking out of the loop also closes it. A reconnect can miss best-effort delta and tool lifecycle frames, so discard connection-scoped previews and refresh any authoritative state you project outside the saved event timeline.
