...
Saved events use their `sequence` as the SSE `id`. Heartbeat comments (`:` lines) only keep the connection open and can be ignored. Live delivery notifications provide the immediate path but are best-effort. The service periodically reconciles active streams against durable history, so a missed notification is recovered without reconnecting. Reconnecting with `Last-Event-ID` also catches up every saved event after the cursor. A simplified exchange looks like:
...
The TypeScript SDK owns this loop for you. `openAgentEventStream` follows the durable stream until you abort it and resumes after the last delivered durable sequence. It retries HTTP 408, 429, and 5xx responses; fetch or reader disconnects; unexpected end-of-response; 35 seconds without SSE activity while actively waiting; and in-band `service_unavailable` errors. Retries use bounded jittered backoff. Other HTTP, API, request, and contract errors are terminal and thrown.
...
Aborting the signal or breaking out of the loop closes the physical response normally. If a `next()` read is already pending, use the abort signal for prompt cancellation; an iterator `return()` waits behind that read. A reconnect can miss best-effort delta and tool lifecycle frames, so discard connection-scoped previews and refresh any authoritative state you project outside the durable event timeline.
