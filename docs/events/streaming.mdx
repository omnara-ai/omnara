...
| `subagent_interaction` | A subagent's question or permission request, on creation and each state change; only with `include_subagent_interactions=true` | No |
| `subagent_tool_call` | A subagent's custom tool call, on each lifecycle state change; only with `include_subagent_interactions=true` | No |
...
A parent's stream does not carry its subagents' timelines. Pass `include_subagent_interactions=true` to also receive `subagent_interaction` frames, each a full [interaction](/events/interactions) with the subagent's `agent_id`, `agent_name`, and `subagent_key`, and `subagent_tool_call` frames, each a full custom tool call with its `agent_id`, from every subagent below the agent at any depth. Like `tool_call_update`, they are best-effort and not replayed, so after connecting query `GET /interactions?include_subagents=true` and `GET /tool-calls?include_subagents=true` for current state.
