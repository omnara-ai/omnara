...
Most importantly, **the machine does not own the agent**. Agents live in the control plane as [event logs](/concepts); machines are execution environments they can attach and release. Disconnecting a machine does not kill the agent.

## Two ways to get one

| | BYO (bring your own) | Pool |
| --- | --- | --- |
| What it is | A computer you already have, registered by installing the daemon | Capacity provisioned on demand from Freestyle, Blaxel, Daytona, Modal, or Unikraft Cloud |
| Created by | You, via the [install flow](/machines/connect) | Omnara, when an agent needs one |
| Lifetime | Yours to manage | Created on demand — at launch or mid-conversation — and deleted when released or after a configured idle period |
...
The [mounted storage example](/examples/mounted-storage) is the canonical BYO story.
...
A binding stores the working directory and environment for that agent. Tools select the machine using its public `machine_id` (`mch_...`), the same ID used by the API. Agents sharing a machine use the same machine ID:

```json
"machine_bindings": [
  {
    "machine_id": "mch_aaaaaaaaaaaaaaaaaaaaaaaaae",
    "binding_kind": "pool",
    "state": "attached",
    "cwd": "/workspace",
    "...": "..."
...
<CardGroup cols={2}>
  ...
  <Card title="Machine pools" icon="layer-group" href="/machines/pools">
    On-demand capacity from Freestyle, Blaxel, Daytona, Modal, or Unikraft
  </Card>
