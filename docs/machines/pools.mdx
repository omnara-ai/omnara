...
- **boxd:** CPU and memory maximums are required. Defaults are optional and must name a boxd size class (1 vCPU with 4 GB, 2 with 8 GB, or 4 with 16 GB); an omitted size uses the snapshot's captured size or the boxd org default.
