...
<Tab title="CLI">
  ...
  `omnara grant pools list` shows the project's pool grants, `omnara grant pools edit <pool-grant-id>` changes a grant's overrides, and `omnara grant pools delete <pool-grant-id>` revokes one.
</Tab>
<Tab title="API">
  ```bash
