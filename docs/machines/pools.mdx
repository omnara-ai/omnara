---
title: Machine pools
description: On-demand machine capacity from Freestyle, Blaxel, Daytona, Modal, or Unikraft Cloud
icon: "layer-group"
---
...
- **Bring your own provider** — connect your Freestyle, Blaxel, Daytona, Modal, or Unikraft Cloud account and create a custom pool. You provide the API credential and choose the image or snapshot containing your tools and dependencies.
...
Freestyle, Blaxel, Daytona, Modal, and Unikraft pools support runtime protection. It does not apply to directly connected machines, and provider errors delay enforcement. The setting belongs to the pool and cannot be overridden by a project grant or agent config.
...
<Tab title="Dashboard">
  1. On the **Machines** page, click **New pool** and choose Freestyle, Blaxel, Daytona, Modal, or Unikraft.
...
- **Freestyle:** CPU and memory defaults and maximums are required. Omnara grows the snapshot VM to the configured size; it rejects snapshots whose CPU or memory already exceed that size.
...
Freestyle pool options require `snapshot`. Set `idle_timeout_seconds` to use Freestyle's native pause and resume lifecycle. The snapshot must run systemd and include a POSIX shell, `curl`, `base64`, and `sha256sum` or `shasum` so Omnara can install its daemon as a service.
