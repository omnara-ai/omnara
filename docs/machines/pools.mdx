---
title: Machine pools
description: On-demand machine capacity from Blaxel, Unikraft Cloud, Daytona, Modal, or boxd
icon: "layer-group"
---
...
- **Bring your own provider** — connect your Blaxel, Unikraft Cloud, Daytona, Modal, or boxd account and create a custom pool. You provide the API credential and choose the image or snapshot containing your tools and dependencies.
...
Blaxel, boxd, Daytona, Modal, and Unikraft pools support runtime protection. It does not apply to directly connected machines, and provider errors delay enforcement. The setting belongs to the pool and cannot be overridden by a project grant or agent config.
...
<Tab title="Dashboard">
  1. On the **Machines** page, click **New pool** and choose Blaxel, Unikraft, Daytona, Modal, or boxd.
...
- **boxd:** CPU and memory maximums are required. Defaults are optional and must name a boxd size class (1 vCPU with 4 GB, 2 with 8 GB, or 4 with 16 GB); an omitted size uses the snapshot's captured size or the boxd org default. The dashboard offers the size classes as a picker, with **Snapshot or org default** leaving the size to boxd behind the per-machine limits you set.
