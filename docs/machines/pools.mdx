...
`machine_cpu` and `machine_memory_mb` set the default size; where supported, the agent can override `cpu` and `memory_mb` per `create_machine` call within pool and project limits.
