...
This page covers common agent lifecycle operations in the console, with the [CLI](/quickstart), and over the API. CLI examples assume you have run `omnara login` and `omnara config select`; API examples use a [bearer token](/api/authentication):
...
<Tab title="CLI">
  ```bash
  omnara agents launch \
    --config acfg_mv4qtrwz3jehcyd5n6a2bfgik7 \
    --name "Weekly report agent" \
    --message "Draft release notes for the changes merged this week."
  ```

  Instead of `--config`, pass `--file agent.yaml` (or `--source` with inline YAML) and the CLI creates the config and launches from it in one step. To launch from a profile, pass `--profile aprf_…` together with the config; `--name` defaults to the profile's name.

  The command prints the new agent's record, including its `agt_…` ID. Add `--json` for the full response with the config, machine bindings, and opening input.
</Tab>
<Tab title="API">
  ```bash
  curl "$BASE/agents" \
  ...
  To launch from a profile, pass both `profile` and `config`. The profile identifies the named template; the config pins the exact current or historical version to launch. `name` is optional — it sets the agent's display name, and defaults to the profile's name when launching from a profile.

  The response includes the agent, its config, attached machines, and opening input:

  ```json
  {
    "agent": {
      "id": "agt_v4qtrwz3jehcyd5n6a2bfgik7m",
      "state": "active",
      "current_config_id": "acfg_mv4qtrwz3jehcyd5n6a2bfgik7",
      "...": "..."
    },
    "agent_config": {
      "id": "acfg_mv4qtrwz3jehcyd5n6a2bfgik7",
      "...": "..."
    },
    "machine_bindings": [],
    "agent_input": {
      "id": "ain_4qtrwz3jehcyd5n6a2bfgik7mv",
      "state": "received",
  ...
  `agent_config` is the exact immutable config used for this agent. `machine_bindings` lists machines attached at launch and is empty when the config declares none. `agent_input` is the opening message and is absent when you omit `message`.

  Retrying the same request with the same `Idempotency-Key` returns the original agent instead of launching another.
...
<Tab title="CLI">
  ```bash
  omnara agents cancel agt_v4qtrwz3jehcyd5n6a2bfgik7m
  ```

  The response includes `affected: true` when work was canceled. `affected: false` means the agent was already idle, so the command is safe to repeat.
</Tab>
<Tab title="API">
  ```bash
...
<Tab title="CLI">
  ```bash
  omnara agents archive agt_v4qtrwz3jehcyd5n6a2bfgik7m
  ```

  The response returns the agent with `state: "archived"`. Repeating the command is safe.
</Tab>
<Tab title="API">
  ```bash
