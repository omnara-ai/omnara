...
### Attach integrations and an initial input atomically
...
The separate `subscriptions` array attaches integration-owned conversations to the new
agent, using `integration_id`, `type`, `conversation` and optional `events`. Subscriptions
alone leave the pinned config unchanged. Ordinary API attachments do not establish
sending context. The shipped integration tools require context assigned by a provider or
scheduled launch. Handler selection independently takes a complete destination.
...
actor. This example subscribes a new agent to events from one PR without changing
its config:
...
```json
{
  ...
  "subscriptions": [{
    "integration_id": "YOUR_GITHUB_INTEGRATION_ID",
    "type": "pull_request",
    "conversation": {"repository_id": 123456, "pull_request": 42},
    "events": ["discussion_comment", "review_comment", "commit"]
  }],
  "initial_input": {
    "content_blocks": [{"type": "text", "text": "Track updates to pull request 42."}],
...
The agent, subscriptions, initial input and any derived config commit together.
...
the current agent. The retry body is not applied, even if it has changed or an integration
...
Archiving retires an agent. It deletes its integration subscriptions, stops current work, drops queued inputs, releases machine bindings, and deletes pooled machines created for the agent. The agent leaves active listings, but its event history remains available by ID.
