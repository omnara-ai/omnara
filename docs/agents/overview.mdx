...
The create-agent API accepts `app_resources` alongside the pinned `config`.
These use the same inline definitions or `app_instance` references as config
authoring. The compiler resolves only the new attachments, preserves the base's
pinned model and other identities, and rejects replacing an existing resource key.
When `profile` is supplied, the base `config` must be its current config or a
historical version of that profile.
Adding attachments requires project management permission; operating an existing
...
actor. This example attaches read access to one GitHub PR without changing the
profile. The saved app must export `github_read`:
...
```json
{
  ...
  "app_resources": {
    "pull_request": {
      ...
      "scope": {"github": {"repository_id": 123456, "pull_request": 42}},
      "tools": {"github_read": {}}
    }
  },
  "initial_input": {
...
The derived config, agent, selected listeners and initial input commit together.
