...
agent, using `integration_id` and `conversation`. The integration determines which
activity is forwarded. Subscriptions alone leave the pinned config unchanged. Ordinary API attachments do not establish
sending context. The shipped integration tools require context assigned by a provider or
scheduled launch. Their interaction handlers use that same assigned conversation.
