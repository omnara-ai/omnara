...
This page covers common agent lifecycle operations in the console and over the API. API examples use a [bearer token](/api/authentication):
...
<Tab title="Console">
  ...
  - **Builder** — a form for name, instruction, model, machine sources, tools, skills, and MCP servers, with a live YAML preview.
  ...
  Add a **First message (optional)** to kick the agent off, then click **Create agent**. You land in the conversation view.
</Tab>
<Tab title="API">
  ```bash
  curl "$BASE/agents" \
  ...
  To launch from a profile, pass both `profile` and `config`. The profile identifies the named template; the config pins the exact current or historical version to launch.
...
The project's **Agents** page lists active agents; click one to open its conversation. Over the API, listing returns active agents newest first. Archived agents are omitted from the list but remain retrievable by ID.
