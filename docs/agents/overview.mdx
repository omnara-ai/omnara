...
alongside the pinned `config`. They use the same names and fixed configs as
...
alone leave the pinned config unchanged.
...
actor. This example attaches read access and incoming events for one GitHub PR without changing the
