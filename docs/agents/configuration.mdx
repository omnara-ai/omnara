...
| `cache_retention` | Prompt-cache preference: `short` (default) applies the route's default caching and, where the route accepts one, a stable conversation key for cache-aware routing; `long` also requests the longest retention the route supports (Anthropic's one-hour cache on models that offer it, OpenAI extended retention); `none` sends no Omnara-managed cache controls, though providers may still cache on their own. |
