...
Use `app_resources` to select provider tools, listeners, reply-following and
interaction handlers independently. For example, a Slack send tool needs a
connection and a channel/thread scope; an incoming Slack message alone does not
grant it. A top-level `tools.slack_post_message` entry can set policy but supplies
no destination by itself.

See [Composable apps](/integrations/apps) for a complete config that combines
GitHub tools with a Slack approval destination, plus reusable project apps and
provider-triggered launchers. App settings are captured when the config is
created; editing a reusable app does not change an existing agent's config.
