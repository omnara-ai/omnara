...
Each attachment requires `access: read_only` or `access: read_write`. Writes require both `read_write` access and a writable store. New stores default to allowing writes.

Enable the [file tools](/tools/built-in#files) under `tools:` to let the agent discover, download, and upload files at `/memory/<store-name>/<file-path>`. File paths can include nested directories. Reading or editing file contents requires an attached machine.
