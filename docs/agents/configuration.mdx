...
settings independently of model arguments. Named `listeners` and
`interaction_handlers` are separate config blocks. Removing a tool does not
remove subscriptions; removing a listener removes its initial and runtime subscriptions.
...
Tools, listeners and interaction handlers are not inherited by spawned subagents.
