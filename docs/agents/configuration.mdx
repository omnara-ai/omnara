...
When `machine_sources` is nonempty, Omnara adds missing entries for `run_command`, `write_process`, `read_process`, `stop_process`, `list_processes`, `list_machines`, `inspect_machine`, `upload_file`, and `download_file` to the saved config. If any source is a machine pool, it also adds `create_machine` and `delete_machine`, regardless of pool counts or machine availability. Attached skills similarly add `skill`. These defaults are applied in the builder and when compiling YAML or JSON submitted through the API. Explicit disables and permissions are preserved.

In the builder, use **Tools → Other tools** to change permissions or select **Disabled**. Removing the last relevant machine source, skill, or subagent in the builder also removes its associated tool entries. Compilation itself never removes explicit tools. While the relevant source remains, omitting a tool adds it back; set `enabled: false` to disable it.
