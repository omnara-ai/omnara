...
`tools.app__<app-name>__<operation>`. Optional tool `config` fixes implementation
settings independently of model arguments. `interaction_handlers` is a separate
config block. Incoming subscriptions belong to the app and are attached through
launch requests or the app subscriptions API, not config. Removing a tool or
changing config does not remove subscriptions.
