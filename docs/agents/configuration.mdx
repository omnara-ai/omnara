...
Slack's `send_integration_message` remains contextual: it is available in Slack conversations without being added to the saved config, unless explicitly disabled. The builder only shows this tool when it is explicitly listed in the config.
...
Each POST contains `event` and `data` matching the [event stream](/events/streaming), excluding token previews, heartbeats, and stream errors. See [Webhook payloads](/events/webhooks) for examples and SDK validation. Each agent sends its own events. Self-subagents inherit the webhook configuration and send events with their own agent IDs; other subagents use their profile’s configuration.
...
Signing is optional. Set `event_webhook.signing_secret_id` to a project-available [generic secret](/organization/secrets), generated with `openssl rand -base64 32`. Configure the same value on your receiver and verify requests using [Standard Webhooks](https://github.com/standard-webhooks/standard-webhooks/blob/main/spec/standard-webhooks.md).

## Machines

For new agents only, the builder preselects a cluster-managed pool granted to the project if one exists; otherwise, it starts without a machine source.
...
| `model` | Set both `provider_config` and `name` to select a model at compilation. Omit both to inherit the parent's model for `self` or the profile's current model for `profile`. Reasoning and token-limit overrides can be set independently. |
...
`memory_stores` attaches project-owned stores where agents can keep files between conversations or share them with other agents. Create a store first with `POST /orgs/{orgID}/projects/{projectID}/memory-stores`, passing a body such as `{"name": "engineering"}`, then reference its name in the config:
...
Store names are unique within a project and use 1–64 lowercase letters, digits, and single hyphens between groups of letters or digits. Config creation resolves the name to the store's ID.

Each attachment requires `access: read_only` or `access: read_write`. Agent writes require both `read_write` access and a writable store. Project managers can edit files in read-only stores. New stores default to allowing writes.

Attached stores are available through the [file tools](/tools/built-in#files) at `/memory/<store-name>/<file-path>`. Paths can include nested directories. Retrieval tools are added automatically; `read_write` attachments also add `write_file`. Explicit tool disables still apply.
