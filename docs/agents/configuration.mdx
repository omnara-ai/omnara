...
Channel tools are contextual: live channel bindings make `list_channels`, `get_channel`, `set_current_channel`, `read_channel`, and `send_channel_message` available as permitted by those bindings. These tools cannot be declared in saved config and are excluded from the builder's configurable tools. Manage channel access through bindings instead.
