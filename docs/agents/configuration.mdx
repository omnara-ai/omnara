...
Create a project app first, then reference its tools as
`tools.app__<app-name>__<operation>`. Tool entries contain enabled state, deferred
loading and permissions; handler entries use `interaction_handlers: {app-name: {}}`.
The shipped tools use immutable app-agent context established by a provider or
scheduled launch. They fail before provider I/O if no conversation is assigned. Handler selection
always takes complete destination arguments and is independent of sending context.
Incoming subscriptions belong to the app and are attached through launch requests
or the app subscriptions API, not config. Removing a tool or changing config does
not remove subscriptions.

App names resolve to immutable IDs when compiled, while credentials remain live
on the app. See [Apps](/integrations/apps) for examples and provider launchers.
App tools, subscriptions and interaction handlers are not inherited by spawned subagents.
...
Deleting a profile removes the profile and its version history, but agents launched from it keep running. In the dashboard, open the profile's row menu and click **Delete**, or run `omnara profiles delete {agent-profile-id}`. An active app that uses the profile must be removed first. See [Delete agent profile](/api-reference/endpoints/configs-and-profiles/delete-agent-profile).
