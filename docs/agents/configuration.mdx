...
Set `event_webhook.events` to an event-type allowlist, such as `[tool_call_update, model_output]`. Supported event types are `agent_input`, `model_output`, `tool_result`, `context_checkpoint`, and `tool_call_update`. Omit the list to send all supported event types; an explicit list must contain at least one type. Changes apply to newly queued events.

Each POST contains `event` and `data` matching the [event stream](/events/streaming), excluding token previews, heartbeats, and stream errors. Each agent sends its own events. Self-subagents inherit the webhook configuration and send events with their own agent IDs; other subagents use their profile’s configuration.

Deliveries are queued for up to ten minutes, with a five-second request timeout. Failed `tool_call_update` deliveries are retried within that window for custom tools entering `ready`, any tool entering `awaiting_permission`, and the built-in `ask_question` entering `running`. All other events are best effort, with no scheduled retries after failure. Return `2xx` to acknowledge receipt. Events may arrive more than once or out of order; deduplicate custom tool execution by tool-call ID.
