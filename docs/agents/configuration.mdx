...
<Tab title="CLI">
  The CLI also has no separate config step: pass the config to the command that uses it. `--file` takes a `.yaml`, `.yml`, or `.json` file, `--source` takes inline YAML or JSON, and `--config` references an existing `acfg_…` ID:
  ...
  The CLI uploads the config first — reusing the existing ID when the same definition was already created — and validation errors identify the field that needs to be fixed.
</Tab>
<Tab title="API">
...
<Tab title="CLI">
  Point a name at an existing config, or pass `--file` to create the config and the profile in one step:
...
<Tab title="CLI">
  One command creates the new config and points the profile at it:

  ```bash
  omnara profiles update aprf_3jehcyd5n6a2bfgik7mv4qtrwz --file agent.yaml
  ```

  The CLI reads the profile's current config before updating, so the request fails with `409` if someone changed the profile in between. Pass `--expected-current-config-id acfg_…` to pin the expected version yourself.
</Tab>
<Tab title="API">
