...
Create a project integration first, then reference its tools as
`tools.int__<integration-name>__<operation>`. Tool entries contain enabled state, deferred
...
on the integration. See [Integrations](/integrations/overview) for examples and provider launchers.
