...
Subagents start with a clean context: only the `task` passed to `spawn_agent` reaches them. `self` subagents share the parent's attached machines; `profile` subagents provision their own from the profile's `machine_sources`. A subagent's final answer, question, or failure arrives in the parent's timeline as a message from the subagent; `read_agent` reads its timeline on demand. Subagents appear in the console and API as agents with a `parent_agent_id`; listing agents returns top-level agents unless `include_subagents` or `parent_agent_id` is set. Archiving a parent archives its subagents.
