...
| `cache_retention` | Prompt-cache preference: `none`, `short`, or `long`. Anthropic models default to `short` when unset. |
...
`subagents` declares the helpers an agent may spawn, keyed by a handle the agent passes to the `spawn_agent` tool. Declaring at least one handle implicitly adds the five subagent tools (`spawn_agent`, `wait_agents`, `send_agent_message`, `stop_agent`, `list_agents`); list one under `tools` only to disable it or change its permission mode.
...
| Field | What it does |
|---|---|
| `type` | `profile` launches the named profile's current config. `self` launches a copy of this agent's own config with `subagents` and the subagent tools removed, so forks cannot spawn further. |
| `profile` | Profile name; required for `type: profile`, rejected for `type: self`. |
| `description` | Shown to the model when it chooses a handle. |
...
| `max_concurrent` | Cap on active subagents spawned from this handle. |
| `archive_after_idle_minutes` | Archive subagents from this handle once they have been idle this long. |
| `max_subagents` | Top-level cap on active subagents across every handle. |

Subagents start with a clean context: only the `task` passed to `spawn_agent` reaches them. `self` subagents share the parent's attached machines; `profile` subagents provision their own from the profile's `machine_sources`. A subagent's final answer, question, or failure arrives in the parent's timeline as a message from the subagent, or completes a pending `wait_agents` call. Subagents appear in the console and API as agents with a `parent_agent_id`; listing agents returns top-level agents unless `include_subagents` or `parent_agent_id` is set. Archiving a parent archives its subagents.
