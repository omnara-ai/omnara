...
Environment names beginning with `OMNARA_` are reserved case-insensitively, so user configuration containing one is rejected. As defense in depth, the daemon also removes reserved values before launching an agent process. The sole process-level exception is the daemon-provided `OMNARA_HOME`, a non-secret path used to locate installed skills; user configuration cannot override it.
