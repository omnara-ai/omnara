...
Add `https://app.omnara.com/mcp` to your MCP client and sign in through the browser — no tokens to paste. The Omnara API is exposed as MCP tools, so your agent can launch Omnara agents, send them messages, follow their progress, and manage profiles, secrets, and machines, with the same permissions you have in the dashboard. Omnara is also listed in the official MCP Registry as `com.omnara/omnara`. See [Connecting MCP clients](/api/authentication#connecting-mcp-clients).
