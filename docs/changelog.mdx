...
<Update
  ...
  description="Use Omnara from your coding agent"
  rss={{
    title: "Use Omnara from your coding agent",
    description: "Omnara is now an MCP server. Also new: Modal sandboxes, Amazon Bedrock with AWS credentials, editable secrets, and reliability improvements."
  }}
>

## Use Omnara from your coding agent

Omnara is now an MCP server, Modal joins the sandbox providers, and Amazon Bedrock works with AWS credentials.

### Connect Claude Code, Cursor, and other MCP clients

Add `https://app.omnara.com/mcp` to your MCP client and sign in through the browser — no tokens to paste. Your coding agent can then launch Omnara agents, send them messages, follow their progress, and manage profiles, secrets, and machines, with the same permissions you have in the dashboard. Omnara is also listed in the official MCP Registry as `com.omnara/omnara`. See [Connecting MCP clients](/api/authentication#connecting-mcp-clients).
