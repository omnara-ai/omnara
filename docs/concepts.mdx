...
An app can supply tools for taking actions, listeners for receiving events, and
an interaction handler for presenting questions and approvals. Agent configs
choose these independently. A launcher can add them when it starts an agent, or
you can select them directly in a config without enabling a launcher. Removing a
tool does not stop listeners; removing a listener stops its subscriptions.
