...
[Apps](/integrations/apps) connect one project to a provider such as Slack, Discord
or GitHub. Create an app from the project's Apps page, configure its credentials,
and optionally choose which profiles it launches from provider events.

An app supplies tools for taking actions, subscription types for receiving events,
and an interaction handler for presenting questions and approvals. Agent configs
select tools and handlers; launch requests or the app subscriptions API attach
conversations to agents. Changing config or removing a sending tool does not
change subscriptions. Stop incoming forwarding from the app's **Conversations**
