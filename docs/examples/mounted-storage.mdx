...
The daemon connects outbound to the control plane, so the container needs no inbound ports. If it disconnects, the [agent remains durable](/concepts); commands against the machine return retryable errors until it reconnects.
...
Launch an agent from this config ([dashboard or API](/agents/overview#launch-an-agent)) and verify:
