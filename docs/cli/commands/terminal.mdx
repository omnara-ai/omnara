...
**Agent Flag Pass-through**: Any flags specific to your chosen agent (Claude Code, Codex, Amp) are automatically passed to the underlying agent CLI. This allows you to use native agent features while maintaining Omnara integration.
...
```bash Codex Flags
# Use Codex with its native flags
omnara terminal --agent codex [CODEX_FLAGS]
```

```bash Amp Flags
# Use Amp with its native flags
