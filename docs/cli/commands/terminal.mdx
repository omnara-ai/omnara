...
```bash
omnara terminal [OPTIONS]
```

Starts an AI coding agent with WebSocket relay streaming enabled. This is the explicit command for the relay-enabled mode that's also available via the default `omnara` command.

<Note>
`omnara terminal` is equivalent to running `omnara` (without subcommand). Both launch agents with WebSocket relay streaming by default.
</Note>
