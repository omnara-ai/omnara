...
These shapes describe credentials Omnara issues today. Treat every bearer token
as an opaque string: store and send the complete value, but do not parse it,
validate its checksum, or branch on its prefix or version. The server owns
format validation and may introduce additional formats later.
...
Omnara currently issues personal tokens in the versioned `omnara_pat_v1_<secret>_<checksum>` format. The secret has at least 256 bits of cryptographic entropy; the final six characters are a typo-detecting checksum, not a cryptographic integrity check. Only a SHA-256 hash of the complete token is stored server-side, so treat the one-time display as the only copy that will ever exist. This format is server-owned; consumers must still treat the complete token as opaque.
...
Personal and organization token records also expose a `token_id`. This is a
stable, non-secret management/display identifier; it is deliberately separate
from the bearer value and is not embedded in it.
