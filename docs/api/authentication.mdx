...
`token_name` is the human-facing name of the personal access token. When provided, it must contain 1–64 Unicode characters and at most 256 UTF-8 bytes, use only ordinary spaces, contain no control or invisible format characters, and have no leading or trailing whitespace. Omitting it or sending an empty value uses `Device login`.
