...
| --- | --- | --- |
| Personal API key | `omnara_pat_…` | You — your roles, your project access |
| Org API key | `omnara_org_…` | The org itself — a service account for integrations and automations |
...
The token format is `omnara_pat_<token_id>_<secret>`. Only a hash of the secret is stored server-side, so treat the one-time display as the only copy that will ever exist.
...
```json
{ "access_token": "omnara_pat_hcyd5n6a2…", "token_type": "Bearer" }
```
