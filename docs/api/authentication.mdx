...
The Omnara API is also exposed as an MCP server at `https://app.omnara.com/api/mcp`. MCP clients that support the [MCP authorization spec](https://modelcontextprotocol.io/specification/draft/basic/authorization) discover the OAuth endpoints themselves and sign you in through the browser, so no token needs to be pasted into the client.
...
```json
{
  "mcpServers": {
    "omnara": {
      "type": "http",
      "url": "https://app.omnara.com/api/mcp"
    }
  }
}
```

On first use the client opens the dashboard's **Authorize access to Omnara** page. Approve it and the client stores its own short-lived tokens, refreshing them in the background.

Clients without OAuth support can pass a personal access token or org API key instead:
...
```json
{
  "mcpServers": {
    "omnara": {
      "type": "http",
      "url": "https://app.omnara.com/api/mcp",
      "headers": { "Authorization": "Bearer omnara_pat_v1_…" }
    }
  }
}
```
