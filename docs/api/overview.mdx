...
- **Retry `424`, `429` and `5xx` with backoff, reusing the same `Idempotency-Key`** so a retry can never act twice. `424` with code `upstream_unavailable` means a server you configured, such as an MCP server, did not respond or failed temporarily. A `409` from MCP tool discovery means another refresh of the same server or auth secret is in progress; retry it shortly. Other `4xx` errors won't change on retry — fix the request instead.
