...
An integration can supply tools, named subscription types and one interaction handler.
...
For example, a profile selected by a GitHub integration named `reviews` can also use the
interaction handler of a Slack integration named `engineering`. Its config can include:
...
A subscription connects one integration, one agent, a named type and one concrete provider
...
The integration and integration-definition catalog expose
`capabilities.subscriptions[type].conversation_schema` and `.events`. The type is
local to the integration, such as `thread_messages` or `pull_request`; do not prefix it
with the integration name. The conversation is one flat provider address, not a config
...
Each attachment has `integration_id`, `type`, `conversation` and optional `events`.
...
includes its `isub_…` ID, integration, project, agent ID/name, type, concrete conversation,
...
DELETE cannot remove the new attachment. Integration disconnection preserves subscriptions;
...
integration's sending context. For example:

```json
{"handler":"engineering","args":{"channel_id":"C123","thread_ts":"111.222333"}}
```

A handler may use another conversation accessible to the integration; it is not restricted
...
disconnecting its integration, or replacing the pinned integration invalidates old external
