...
<Tab title="CLI">
  One command creates the app and opens the install link in your browser:
  ...
  Approve the Slack install within 10 minutes while signed in to Omnara as the user who started setup, and the browser returns to the console. `--no-browser` prints the authorization URL instead of opening it.
</Tab>
<Tab title="API">
...
If you already have a Slack app, provide its credentials instead. This path is not available in the console:
...
<Tab title="CLI">
  ...
  The command opens the approval page in your browser and waits until the integration is created. Your Slack app must already be configured with Omnara's event, action, and redirect URLs — the API response in the next tab returns them.
</Tab>
<Tab title="API">
  ```bash
  ...
  Configure your app with the returned event, action, and redirect URLs, then open `oauth_url` to approve the install.
</Tab>
</Tabs>
