...
Channel customer data follows the project tenancy boundary. Connections, routes, channels, agent bindings, message receipts, external requests, and project-scoped runtime units all carry `project_id`, so they can move with that project's agents to the same database shard. Provider-app registrations, app-scoped runtime units, and provider-control receipts are control-plane records because one company-owned app can serve several projects. Control receipts track resumable connection updates; each affected project's state still changes through its scoped API operation. The private connector API hides database placement from the gateway; the gateway never chooses a database or connects to one directly.
