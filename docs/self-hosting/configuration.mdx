...
The API commits the organization and a durable provisioning job. Maintenance
then sends `POST /model-provider-credentials` to the configured base URL with
the organization ID, creator user ID, and its configured template. The
credential service returns `201` with
