...
`default_max_output_tokens` to use the model's capacity. Templates using
`anthropic-messages` must supply `max_output_tokens` for every model; other formats
may omit it when capacity is unknown. See [model output limits](/organization/model-providers)
