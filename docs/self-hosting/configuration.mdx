...
| `tenki` | Default and maximums required; minimum optional | Optional HTTPS `api_base_url` and `allowed_images` | Optional Tenki registry `image`, `disk_size_gb` (default 20), and `startup_script` |
...
- Tenki uses sticky sessions with Omnara-owned cleanup. Leave `image` empty for the base image. `allowed_images` defaults to the pool image; use `["*"]` to allow overrides, or include `""` in an explicit allowlist to allow the base image. Tenki creation is attempted at most once per machine; ambiguous responses are reconciled by ownership metadata. See [Tenki pools](/machines/pools#tenki-pools).
