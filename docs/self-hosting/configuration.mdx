...
| `OMNARA_MIGRATION_TIMEOUT` | Connect-and-migrate deadline for the one-shot process (Go duration); unlock cleanup has a separate 5s ceiling | `30m` | No |

PostgreSQL 18 or newer is required. Omnara uses PostgreSQL 18's built-in UUIDv7
generation, and `migrate` checks the server version before changing the schema.

The long-running API, worker, and maintenance processes use these pool defaults
when the corresponding setting is absent from `OMNARA_DATABASE_URL`:
`pool_max_conns=10`, `pool_min_conns=1`, and
`pool_max_conn_lifetime=30m`, with
`pool_max_conn_lifetime_jitter=5m` to stagger connection recycling. Add any of
those pgxpool settings to a URL or keyword DSN to override it; an explicit
`application_name` also wins. Otherwise the services identify themselves as `omnara-api`,
`omnara-worker`, or `omnara-maintenance` (the defaults reconciliation command
uses `omnara-api-reconcile-defaults`). Size the database connection budget by
summing the maximum pool sizes across every service replica, with headroom for
the migrator, administration, and failover.

The one-shot migrator is not a long-lived application pool, so `pool_*` values
are accepted and stripped rather than used as migration concurrency settings.
Its connections default to `application_name=omnara-migrate`,
`lock_timeout=30s`, and `statement_timeout=15min`; explicit DSN values override
those defaults. `OMNARA_MIGRATION_TIMEOUT` bounds connection and migration
work. Advisory-lock cleanup has its own five-second ceiling, so a dead
connection cannot make the one-shot process hang indefinitely during unlock.
