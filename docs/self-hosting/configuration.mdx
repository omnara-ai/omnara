...
| `OMNARA_WORKER_INBOX_CAPACITY` | Maximum app inbox receipts processed concurrently per worker process; integer from 1 through 100 | `4` | No |
| `OMNARA_WORKER_DISCORD_CAPACITY` | Maximum concurrent Discord app Gateway sessions per worker process; positive integer | `64` | No |
...
Workers sample `omnara_app_inbox_oldest_ready_lag_seconds` about every 30 seconds,
including while every consumer is busy. This measures time since `available_at`
for the oldest due pending receipt, including inactive apps awaiting recovery;
future retry delays and processing leases are excluded. Scrapes read cached
values and never query the inbox. The metric has no app or project labels and
reports zero for an empty ready queue, or `NaN` before sampling or after a failed
sample. Check `omnara_app_inbox_lag_sample_last_success_timestamp_seconds` for
...
clock. Locks or cleanup backlogs can delay deletion. Deleting the app, project or
organization can purge receipts earlier; disconnecting an app preserves normal
retention.
...
The opt-in `TestAppInboxLocalLoad` experiment runs with `OMNARA_INBOX_LOAD=1`
