...
the upstream credential in configuration. This is optional; most self-hosted
installations can leave all three variables unset and let organization owners
configure providers normally.

| Variable | Purpose | Default | Required |
| --- | --- | --- | --- |
| `OMNARA_DEFAULT_MODEL_PROVIDER_TEMPLATE` | Path to the provider and model template on the API. See `default-model-provider-example.yaml`. | — | No |
| `OMNARA_HOSTED_API_URL` | Base URL of the operator credential service. Used only by maintenance. | — | With a default provider template |
| `OMNARA_HOSTED_API_TOKEN` | Bearer token shared with the operator credential service; at least 32 bytes. Used only by maintenance. | — | With a default provider template |

The API commits the organization and its provider template in a durable
provisioning job. Maintenance then sends `POST /model-provider-credentials` to
the configured base URL with the organization ID, creator user ID, and stored
template. The credential service returns `201` with
`{"credential_value":"..."}`, `202` while an earlier issuance is still pending,
`409` for a durable identity conflict, or `5xx` for a retryable failure. Omnara
encrypts a successful credential and creates the provider, configured models,
and default-project grants in one transaction.

If that local transaction does not commit after a `201`, maintenance repeats
the same request. The credential service must use the organization ID as the
stable issuance identity: it can return `202` while recovery is in progress,
but must eventually return a usable `201` credential that supersedes the
uncommitted issuance. It should reserve `409` for durable identity or
provisioner mismatches that require operator action.

Organization creation does not depend on that network call. If the credential
service is unavailable, the organization remains usable and maintenance retries
with backoff. Run migrations before deploying the API and maintenance binaries;
maintenance also needs the normal secret-encryption key ring so it can store the
credential.
