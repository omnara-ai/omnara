...
The API commits the organization and a durable provisioning job, then
immediately sends `POST /model-provider-credentials` with the organization ID,
creator user ID, and configured template. The credential service returns `201` with
...
If the initial request fails, or if the local transaction does not commit after
a `201`, maintenance repeats the same operation with backoff. Both processes use
the same configured template and provisioning path. The credential service must
use the organization ID as the stable issuance identity: it can return `202`
while recovery is in progress, but must eventually return a usable `201`
credential that supersedes the uncommitted issuance. It should reserve `409` for
durable identity or provisioner mismatches that require operator action.

Organization creation does not depend on that network call succeeding. If the
credential service is unavailable, the organization remains usable and
maintenance retries with backoff. Run migrations before deploying the API and
maintenance binaries; both need the normal secret-encryption key ring so they
can store the credential.
