...
| `boxd` | Maximums required; default and minimum optional | Optional `api_url`, `auth_url`, and `allowed_snapshots` | Optional boxd `snapshot`, `startup_script`, `sleep_after_ms`, and `auto_suspend_secs` |
...
- boxd pools authenticate with a boxd API key (`bxd_...`), which is fenced to one boxd org; machines are created in that org. A configured size must be one of boxd's classes (1 vCPU with 4096 MB, 2 with 8192 MB, or 4 with 16384 MB); an omitted size uses the snapshot's captured size or the org default. `snapshot` names a snapshot saved with `boxd snapshots save` and the base image is used when it is omitted. boxd counts guest TCP/UDP traffic as activity and the machine daemon heartbeats every 10 seconds, so a machine with a live daemon never reaches an idle window. Without `sleep_after_ms`, Omnara disables boxd's auto-suspend on its machines so a machine whose daemon has stopped stays observable to runtime protection rather than suspending; auto-hibernate follows the boxd org's own setting. A configured size and a `snapshot` must agree, because boxd restores a snapshot at the size it was captured at.
- For Unikraft, Blaxel, and boxd, `sleep_after_ms` enables scale-to-zero after that many milliseconds of daemon inactivity; omit or `0` disables it, and enabled values must be at least `30000`.
- On boxd, `sleep_after_ms` also sets the machine's auto-suspend window, which is how long boxd waits for the parked daemon to go quiet before suspending. Suspend is warm: memory, processes, and open sockets survive and resume takes under a millisecond. When an agent needs the machine again, Omnara runs a command in it through the boxd API that pokes the daemon's wake listener; boxd resumes a suspended or hibernated machine before running the command, so the same signal reaches a daemon whether or not boxd had suspended the machine yet.
- `auto_suspend_secs` tunes that window, trading cost against wake latency; it defaults to `60` and must be at least `30`, which keeps it clear of the daemon's ten-second heartbeat so boxd never suspends a machine that is still working. It requires `sleep_after_ms`, because a machine that suspends without the daemon parking first is one Omnara cannot wake.
