...
| `OMNARA_API_URL` | Control-plane origin; HTTP is accepted only for loopback or `host.docker.internal` during local development | Existing `daemon.json` value, otherwise `https://api.omnara.com` |
