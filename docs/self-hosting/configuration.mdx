...
Inbox retention runs last in each core maintenance tick. Completed/discarded
receipts remain for seven days after completion, measured by the database clock.
...
`budget_exhausted=true`. Failed receipts in live or disconnected scopes
remain available for operator recovery. See the
