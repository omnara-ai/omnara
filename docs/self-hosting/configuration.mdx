...
Templates also define model capacity and optional request defaults. Omit
`default_max_output_tokens` to use the model's capacity. Every format may omit
`max_output_tokens` when capacity is unknown. If both settings are absent, Messages
looks up the provider's output limit and falls back to 64,000 tokens; other formats
omit the request allowance. See [model output limits](/organization/model-providers)
for provider requirements and context budgeting.
...
Both commands print changes, warnings, and counts. Review the plan before applying.
Reconciliation creates immutable model revisions, preserves organization-owned
models and explicit grant overrides, and reports models that remain referenced
when removed from a template. Provider templates accept `request_timeout_ms` and
`idle_timeout_ms`, defaulting to 3600000 (60 minutes total) and 300000 (5 minutes
idle). Reconciliation applies these values to managed providers; upgrades alone
preserve existing total deadlines, and organization-owned providers retain their
settings.
