...
| --- | --- | --- | --- |
| `freestyle` | Default and maximums required; minimum optional | Optional `api_base_url` and `allowed_snapshots` | `snapshot`, plus optional `startup_script` and `idle_timeout_seconds` |
...
- Freestyle pool machines use full hardware virtualization, providing complete Linux environments for system-level workloads. Snapshots must run systemd and include a POSIX shell, `curl`, `base64`, and `sha256sum` or `shasum`. Omnara grows the VM to the configured CPU and memory; it rejects a snapshot that already exceeds either value. `idle_timeout_seconds` uses Freestyle's native pause and resume lifecycle and accepts values from `1` through `31536000`; omit it to disable idle pausing.
