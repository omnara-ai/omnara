...
`OMNARA_PUBLIC_URL` is the public app origin used for email links, OAuth callbacks, browser flows, and installer links. The API defaults to the same deployment at `/api/v1`; set `OMNARA_PUBLIC_API_URL` when it is exposed at a different public URL. These features will not function unless the configured URLs are publicly accessible.
