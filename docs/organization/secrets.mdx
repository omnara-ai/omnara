...
<Tabs>
<Tab title="Console">
  Under org, project or user secrets, click **New secret**, name it, and choose **Generic**, **OAuth token set**, or **MCP OAuth secret**. For MCP OAuth, click **Authorize and Create Secret** to run the [OAuth flow](/tools/mcp#connect-with-oauth). You can also add **Project grants** in the same dialog.
</Tab>
<Tab title="API">
  ```bash
