...
<Tab title="CLI">
  ...
  For MCP OAuth, use the built-in [OAuth flow](/tools/mcp#connect-with-oauth) — `omnara secrets mcp-oauth` runs it end to end from the terminal. If you need to import OAuth tokens yourself, see the [secret schema](/api-reference/endpoints/secrets/create-secret).
</Tab>
<Tab title="API">
  ```bash
...
Rotation creates a new version while keeping the same secret ID. It is **API-only for now**; the console and CLI show the current version but cannot rotate it.
...
<Tab title="CLI">
  ```bash
  omnara grant secrets add sec_7jehcyd5n6a2bfgik7mv4qtrwz \
  ...
  `omnara grant secrets list <secret-id>` shows a secret's project grants, and `omnara grant secrets delete <secret-id> <grant-id>` removes one.
</Tab>
<Tab title="API">
  ```bash
