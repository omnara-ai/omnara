...
You cannot delete a secret while a model provider, machine pool, or integration uses it as its credential. Replace the secret on that resource — or delete the resource — first.
...
| Surface | Field |
| --- | --- |
| [Model provider config](/organization/model-providers) | `credential_secret_id`, `secret_headers` |
