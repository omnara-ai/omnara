...
A **secret** stores a credential such as a model API key, machine environment value, MCP token, or app credential. Other resources reference the secret by its `sec_…` ID instead of storing the value themselves.
...
| `github_app_credentials` | GitHub App ID, RSA private key and webhook secret | You; see [GitHub setup](/integrations/github#connect-your-github-app) |
...
You cannot delete a secret while a model provider, machine pool, or app uses it. Replace the secret on that resource — or delete the resource — first.
