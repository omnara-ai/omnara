...
For Messages with neither setting, Omnara uses a 64,000-token allowance. This
fallback may exceed the limit of an unknown model; configure an explicit capacity
or allowance in that case. The fallback does not update either stored setting.
Existing context fitting and reasoning-budget validation still apply.
