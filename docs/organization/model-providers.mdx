...
- `max_output_tokens` is the model's known output capacity. Models using Anthropic Messages require it on creation and updates, including Bedrock variants. A best-effort lookup in the cached model catalog fills an omitted creation value when compatible metadata is available; a stale cache can require a catalog fetch. Enter the capacity manually when discovery cannot supply it, such as for a custom or Bedrock model. Other API formats allow unknown capacity, returned as `null`, and support clearing it with a `null` update.
- `default_max_output_tokens` is an optional request allowance, set only when explicitly configured. Discovery populates capacity and leaves this default unset. Without an override, Omnara uses the known capacity and fits the allowance to the remaining context. When neither is known, Chat Completions and Responses use their provider default. An explicit allowance does not replace the Messages capacity requirement.
