...
- `max_output_tokens` is the model's known output capacity. It is optional on creation; a best-effort lookup in the cached model catalog fills an omitted value when available. A stale cache can require a catalog fetch during creation. An unknown capacity is returned as `null`. Updating it to `null` clears the stored capacity.
- `default_max_output_tokens` is an optional request allowance. Without an override, Omnara uses the known capacity and fits the allowance to the remaining context. When neither is known, compatible APIs use their provider default. Anthropic Messages requires an allowance before a call can be sent; supply capacity or a default on the model, project grant, or agent.
...
Normal context admission reserves generation headroom independently of a large output capacity. If a response reaches its output limit, Omnara discards all tool calls from that response, records feedback, and lets the model continue with smaller calls. After two consecutive automatic continuations, another truncated response ends the run with an error message. Partial tool calls are never executed.
