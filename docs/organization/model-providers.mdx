...
Compaction summarizes older history with the same configured model. Its output allowance is at most 16,384 tokens, further limited by the effective default allowance, known output ceiling, and half the context window. Anthropic Messages summary requests omit explicitly enabled manual thinking; ordinary requests retain it. Adaptive thinking and other providers' reasoning settings remain unchanged. Only completed summaries that reduce the history are adopted. If a summary is truncated, Omnara retains the original history and retries with a smaller eligible portion where possible.
