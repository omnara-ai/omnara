...
async function pickByNumber(title: string, items: SelectItem[], options: PickOptions): Promise<number[]> {
  ...
  try {
    for (;;) {
      ...
      if (indices.length > 0 && indices.every((index) => Number.isInteger(index) && index >= 0 && index < items.length)) {
        return options.multiple === true ? [...new Set(indices)] : [indices[0]!]
      }
...
export async function pickOne(title: string, items: SelectItem[]): Promise<number> {
  const [index] = await pick(title, items)
  return index!
}
