...
`OMNARA_API_KEY` takes a personal access token. The token must already belong
to at least one organization. With multiple organizations, the CLI shows an
arrow-key picker (or set `OMNARA_ORG_ID`). With the local machine option, the
agent is told the CLI's working directory and
