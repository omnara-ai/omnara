...
2. Look up the user's org, its default project, and the granted model and
   ...
   - For app capabilities, follow https://docs.omnara.com/integrations/apps and the provider's setup guide. Create a project app with its own credentials. Select its tools under `tools` using `app__<app-name>__<tool-name>`; optional `config` fixes destination fields that the model cannot override. Select named listeners under `listeners` and handlers under `interaction_handlers`. App launchers add missing entries to a derived config; existing profile entries win unchanged.
   - `list_interaction_handlers` and `set_interaction_handler` are implicit tools for tool-capable models. A handler, listener and sending tool are independently selected. Removing a sending tool does not remove subscriptions; removing a listener stops its configured and runtime subscriptions. The dashboard remains available for questions and approvals.
...
4. (optional: interact via Slack) If the user wants to setup Omnara to interact via Slack, create a project app with definition `omnara.slack` through `npx omnara apps create`, and select the profile in its launcher. Use `-h` for current command arguments. Then connect that saved app with `npx omnara apps slack {app_id} ...`. Follow the Slack guide for either creating a Slack registration with a configuration token from https://api.slack.com/apps or connecting an existing registration.

   Successful Slack setup connects the saved project app to the bot. Its newly launched agents receive scoped Slack tools, a message listener and the Slack interaction handler; the profile's base config is unchanged. It does not retroactively attach the earlier dashboard-launched agent to Slack. For GitHub or Discord, use the current provider guide and respect its documented setup/availability limits.
